...
type BatchEnableServicesRequest struct {
	...
	// service. Two or more services must be specified. To enable a single service,
	// use the `EnableService` method instead. A single request can enable a
	// maximum of 20 services at a time. If more than 20 services are specified,
	// the request will fail, and no state changes will occur.
...
type ConsumerQuotaLimit struct {
	...
	// Metric: The name of the parent metric of this limit. An example name would
	...
	// ogleapis.com%2Fcpus/limits/%2Fproject%2Fregion` The resource name is
	...
	// dimension, ordered from least specific to most specific (for example, the
	// global default bucket, with no quota dimensions, will always appear first).
	...
	// ForceSendFields is a list of field names (e.g. "AllowsAdminOverrides") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AllowsAdminOverrides") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ConsumerQuotaMetric struct {
	// ConsumerQuotaLimits: The consumer quota for each quota limit defined on the
	// metric.
	ConsumerQuotaLimits []*ConsumerQuotaLimit `json:"consumerQuotaLimits,omitempty"`
	// DescendantConsumerQuotaLimits: The quota limits targeting the descendant
	// containers of the consumer in request. If the consumer in request is of type
	// `organizations` or `folders`, the field will list per-project limits in the
	// metric; if the consumer in request is of type `project`, the field will be
	// empty. The `quota_buckets` field of each descendant consumer quota limit
	...
	// `projects/123/services/compute.googleapis.com/consumerQuotaMetrics/compute.go
	// ogleapis.com%2Fcpus` The resource name is intended to be opaque and should
	// not be parsed for its component strings, since its representation could
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "ConsumerQuotaLimits") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ConsumerQuotaLimits") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type Endpoint struct {
	...
	// google.api.Endpoint for each of the intended aliases. Additional names that
	// this endpoint will be hosted on.
...
type ImportAdminOverridesRequest struct {
	// Force: Whether to force the creation of the quota overrides. Setting the
	// force parameter to 'true' ignores all quota safety checks that would fail
	// the request. QuotaSafetyCheck lists all such validations. If force is set to
	// true, it is recommended to include a case id in "X-Goog-Request-Reason"
	// header when sending the request.
	Force bool `json:"force,omitempty"`
	// ForceOnly: The list of quota safety checks to ignore before the override
	// mutation. Unlike 'force' field that ignores all the quota safety checks, the
	// 'force_only' field ignores only the specified checks; other checks are still
	...
	// cause the consumer's effective limit to be lower than the consumer's quota
	// usage.
	//   "LIMIT_DECREASE_PERCENTAGE_TOO_HIGH" - Validates that a quota mutation
	// would not cause the consumer's effective limit to decrease by more than 10
	// percent.
...
func (s *ImportConsumerOverridesRequest) MarshalJSON() ([]byte, error) {
	type NoMethod ImportConsumerOverridesRequest
	return gensupport.MarshalJSON(NoMethod(*s), s.ForceSendFields, s.NullFields)
}
...
type OverrideInlineSource struct {
	...
	// 'metric' and 'unit', to specify which metric and which limit the override
	// should be applied to. The 'name' field of the override does not need to be
	// set; it is ignored.
	Overrides []*QuotaOverride `json:"overrides,omitempty"`
	// ForceSendFields is a list of field names (e.g. "Overrides") to
	// unconditionally include in API requests. By default, fields with empty or
...
func (s *OverrideInlineSource) MarshalJSON() ([]byte, error) {
	type NoMethod OverrideInlineSource
	return gensupport.MarshalJSON(NoMethod(*s), s.ForceSendFields, s.NullFields)
}
...
type ProducerQuotaPolicy struct {
	...
	// keys. Any string appearing in {brackets} in the unit (besides {project} or
	...
	// the API does not support quota policies that apply only to a specific user.
	...
	// when the policy is created. Example names would be:
	// `organizations/123/services/compute.googleapis.com/consumerQuotaMetrics/compu
	// te.googleapis.com%2Fcpus/limits/%2Fproject%2Fregion/producerQuotaPolicies/4a3
	// f2c1d`
	Name string `json:"name,omitempty"`
	// PolicyValue: The quota policy value. Can be any nonnegative integer, or -1
	// (unlimited quota).
...
func (s *ProducerQuotaPolicy) MarshalJSON() ([]byte, error) {
	type NoMethod ProducerQuotaPolicy
	return gensupport.MarshalJSON(NoMethod(*s), s.ForceSendFields, s.NullFields)
}
...
// QuotaBucket: A quota bucket is a quota provisioning unit for a specific set
// of dimensions.
type QuotaBucket struct {
	...
	// Dimensions: The dimensions of this quota bucket. If this map is empty, this
	// is the global bucket, which is the default quota value applied to all
	// requests that do not have a more specific override. If this map is nonempty,
	// the default limit, effective limit, and quota overrides apply only to
	// requests that have the dimensions given in the map. For example, if the map
	// has key `region` and value `us-east-1`, then the specified effective limit
	// is only effective in that region, and the specified overrides apply only in
	// that region.
	Dimensions map[string]string `json:"dimensions,omitempty"`
	...
	// ProducerQuotaPolicy: Producer policy inherited from the closet ancestor of
	// the current consumer.
	ProducerQuotaPolicy *ProducerQuotaPolicy `json:"producerQuotaPolicy,omitempty"`
	// RolloutInfo: Rollout information of this quota bucket. This field is present
	// only if the effective limit will change due to the ongoing rollout of the
	// service config.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AdminOverride") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type RolloutInfo struct {
	// DefaultLimitOngoingRollout: Whether there is an ongoing rollout for the
	// default limit or not.
	DefaultLimitOngoingRollout bool `json:"defaultLimitOngoingRollout,omitempty"`
	// ForceSendFields is a list of field names (e.g. "DefaultLimitOngoingRollout")
	// to unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DefaultLimitOngoingRollout") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// atomic: if enabling any service fails, then the entire batch fails, and no
// state changes occur. Operation response type: `google.protobuf.Empty`
//
...
// Disable: Disables a service so that it can no longer be used with a project.
...
// status if the target service is not currently enabled. Operation response
...
// Enable: Enables a service so that it can be used with a project. Operation
...
type ServicesGenerateServiceIdentityCall struct {
	s          *APIService
	parent     string
	urlParams_ gensupport.URLParams
	ctx_       context.Context
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ServicesGenerateServiceIdentityCall) Fields(s ...googleapi.Field) *ServicesGenerateServiceIdentityCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ServicesGenerateServiceIdentityCall) Context(ctx context.Context) *ServicesGenerateServiceIdentityCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ServicesGenerateServiceIdentityCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
// Do executes the "serviceusage.services.generateServiceIdentity" call.
// Any non-2xx status code is an error. Response headers are in either
// *Operation.ServerResponse.Header or (if a response was returned at all) in
// error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
func (c *ServicesGenerateServiceIdentityCall) Do(opts ...googleapi.CallOption) (*Operation, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
...
// List: Lists all services available to the specified project, and the current
...
// View sets the optional parameter "view": Specifies the level of detail for
// quota information in the response.
//
// Possible values:
//
...
// will include buckets for all regions or zones that could support overrides,
// even if none are currently present. In some cases this will cause the
// response to become very large; callers that do not need this extra
// information should use the BASIC view instead.
...
// Any non-2xx status code is an error. Response headers are in either
// *ConsumerQuotaMetric.ServerResponse.Header or (if a response was returned at
...
// atomically, all on the same consumer, but on many different metrics or
// limits. The name field in the quota override message should not be set.
//
...
// ImportConsumerOverrides: Creates or updates multiple consumer overrides
// atomically, all on the same consumer, but on many different metrics or
// limits. The name field in the quota override message should not be set.
//
//   - parent: The resource name of the consumer. An example name would be:
...
func (r *ServicesConsumerQuotaMetricsService) ImportConsumerOverrides(parent string, importconsumeroverridesrequest *ImportConsumerOverridesRequest) *ServicesConsumerQuotaMetricsImportConsumerOverridesCall {
	...
	c.parent = parent
	c.importconsumeroverridesrequest = importconsumeroverridesrequest
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ServicesConsumerQuotaMetricsImportConsumerOverridesCall) Fields(s ...googleapi.Field) *ServicesConsumerQuotaMetricsImportConsumerOverridesCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ServicesConsumerQuotaMetricsImportConsumerOverridesCall) Context(ctx context.Context) *ServicesConsumerQuotaMetricsImportConsumerOverridesCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ServicesConsumerQuotaMetricsImportConsumerOverridesCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *ServicesConsumerQuotaMetricsImportConsumerOverridesCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1beta1/{+parent}/consumerQuotaMetrics:importConsumerOverrides")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
...
// Do executes the "serviceusage.services.consumerQuotaMetrics.importConsumerOverrides" call.
// Any non-2xx status code is an error. Response headers are in either
// *Operation.ServerResponse.Header or (if a response was returned at all) in
// error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
func (c *ServicesConsumerQuotaMetricsImportConsumerOverridesCall) Do(opts ...googleapi.CallOption) (*Operation, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
...
// List: Retrieves a summary of all quota information visible to the service
// consumer, organized by service metric. Each metric includes information
// about all of its defined limits. Each limit includes the limit configuration
// (quota unit, preciseness, default value), the current effective limit value,
// and all of the overrides applied to the limit.
//
...
// Any non-2xx status code is an error. Response headers are in either
// *ListConsumerQuotaMetricsResponse.ServerResponse.Header or (if a response
...
// Get: Retrieves a summary of quota information for a specific quota limit.
//
...
// Any non-2xx status code is an error. Response headers are in either
// *ConsumerQuotaLimit.ServerResponse.Header or (if a response was returned at
// all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Create: Creates an admin override. An admin override is applied by an
// administrator of a parent folder or parent organization of the consumer
// receiving the override. An admin override is intended to limit the amount of
// quota the consumer can use out of the total quota pool allocated to all
...
// the quota override. Setting the force parameter to 'true' ignores all quota
// safety checks that would fail the request. QuotaSafetyCheck lists all such
// validations. If force is set to true, it is recommended to include a case id
// in "X-Goog-Request-Reason" header when sending the request.
...
// ForceOnly sets the optional parameter "forceOnly": The list of quota safety
// checks to ignore before the override mutation. Unlike 'force' field that
// ignores all the quota safety checks, the 'force_only' field ignores only the
// specified checks; other checks are still enforced. The 'force' and
// 'force_only' fields cannot both be set. If force_only is specified, it is
...
// Any non-2xx status code is an error. Response headers are in either
// *ListAdminOverridesResponse.ServerResponse.Header or (if a response was
...
// specified fields of the override. If unset, all fields will be updated.
...
// Create: Creates a consumer override. A consumer override is applied to the
// consumer on its own authority to limit its own quota usage. Consumer
// overrides cannot be used to grant more quota than would be allowed by admin
// overrides, producer overrides, or the default limit of the service.
//
//   - parent: The resource name of the parent quota limit, returned by a
//     ListConsumerQuotaMetrics or GetConsumerQuotaMetric call. An example name
//     would be:
...
// Any non-2xx status code is an error. Response headers are in either
// *ListConsumerOverridesResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
