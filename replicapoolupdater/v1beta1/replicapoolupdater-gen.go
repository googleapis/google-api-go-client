...
const basePathTemplate = "https://www.UNIVERSE_DOMAIN/replicapoolupdater/v1beta1/projects/"
...
type Operation struct {
	...
	// Error: [Output Only] If errors occurred during processing of this operation,
	...
	// Status: [Output Only] Status of the operation. Can be one of the following:
...
// OperationError: [Output Only] If errors occurred during processing of this
...
// RollingUpdate: The following represents a resource describing a single
// update (rollout) of a group of instances to the given template.
type RollingUpdate struct {
	// ActionType: Specifies the action to take for each instance within the
	// instance group. This can be RECREATE which will recreate each instance and
	// is only available for managed instance groups. It can also be REBOOT which
	...
	// InstanceGroupManager: Fully-qualified URL of an instance group manager being
	// updated. Exactly one of instanceGroupManager and instanceGroup must be set.
	...
	// of operations. This should not be used to guess at when the update will be
...
// RollingUpdateError: [Output Only] Errors that occurred during the rolling
// update.
...
type RollingUpdatePolicy struct {
	// AutoPauseAfterInstances: Number of instances to update before the updater
	// pauses the rolling update.
	AutoPauseAfterInstances int64 `json:"autoPauseAfterInstances,omitempty"`
	// InstanceStartupTimeoutSec: The maximum amount of time that the updater waits
	// for a HEALTHY state after all of the update steps are complete. If the
	// HEALTHY state is not received before the deadline, the instance update is
	...
	// updated simultaneously. An instance update is considered complete only after
	// the instance is restarted and initialized.
	MaxNumConcurrentInstances int64 `json:"maxNumConcurrentInstances,omitempty"`
	// MaxNumFailedInstances: The maximum number of instance updates that can fail
	// before the group update is considered a failure. An instance update is
	// considered failed if any of its update actions (e.g. Stop call on Instance
	...
	// MinInstanceUpdateTimeSec: The minimum amount of time that the updater spends
	// to update each instance. Update time is the time it takes to complete all
	// update actions (e.g. Stop call on Instance resource in Rolling Reboot),
	// reboot, and initialize. If the instance update finishes early, the updater
	...
	// ForceSendFields is a list of field names (e.g. "AutoPauseAfterInstances") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AutoPauseAfterInstances") to
...
// Any non-2xx status code is an error. Response headers are in either
// *RollingUpdate.ServerResponse.Header or (if a response was returned at all)
...
// List: Lists recent updates for a given managed instance group, in reverse
...
// Filter sets the optional parameter "filter": Filter expression for filtering
// listed resources.
...
// results to be returned. Maximum value is 500 and default value is 500.
...
// previous list request truncated by maxResults. Used to continue a previous
// list request.
...
// Any non-2xx status code is an error. Response headers are in either
// *RollingUpdateList.ServerResponse.Header or (if a response was returned at
...
// ListInstanceUpdates: Lists the current status for each instance within a
// given update.
//
// - project: The Google Developers Console project name.
// - rollingUpdate: The name of the update.
...
// Any non-2xx status code is an error. Response headers are in either
// *InstanceUpdateList.ServerResponse.Header or (if a response was returned at
// all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Pause: Pauses the update in state from ROLLING_FORWARD or ROLLING_BACK. Has
...
// Resume: Continues an update in PAUSED state. Has no effect if invoked when
...
// Rollback: Rolls back the update in state from ROLLING_FORWARD or PAUSED. Has
