...
func (a *API) GenerateCode() ([]byte, error) {
	...
	pn(" u[key] = []string{value}")
	pn("}")
	pn("\nfunc (u urlParams) setMulti(key string, values []string) {")
	pn(" u[key] = append([]string{}, values...)")
	pn("}")
	p("\n")
...
func (meth *Method) generateCode() {
	...
	pn(" c := &%s{s: %s, urlParams_: make(urlParams)}", callName, servicePtr)
	for _, arg := range args.l {
	...
	for _, opt := range meth.OptParams() {
		...
		if opt.IsRepeated() {
			if goType == "[]string" {
				pn("c.urlParams_.setMulti(%q, %v)", paramName, paramName)
			} else {
				pn("var %v_ []string", paramName)
				pn("for _, v := range %v {", paramName)
				pn(` %v_ = append(%v_, fmt.Sprintf("%%v", v))`, paramName, paramName)
				pn("}")
				pn("c.urlParams_.setMulti(%q, %v_)", paramName, paramName)
			}
		} else {
			if goType == "string" {
	...
	for _, p := range meth.RequiredQueryParams() {
		pn("c.urlParams_.set(%q, fmt.Sprintf(\"%%v\", c.%s))", p.name, p.goCallFieldName())
	}
	for _, p := range meth.RequiredRepeatedQueryParams() {
		goType := p.GoType()
		if goType == "string" {
			pn("c.urlParams_.setMulti(%q, c.%v)", p.name, p.name)
		} else {
			pn("var %v_ []string", p.name)
			pn("for _, v := range c.%v {", p.name)
			pn(" %v_ = append(%v_, fmt.Sprintf(\"%%v\", v))", p.name, p.name, p.name)
			pn("}")
			pn("c.urlParams_.setMulti(%q, %v_)", p.name, p.name)
		}
	}
	...
	pn("urls += \"?\" + url.Values(c.urlParams_).Encode()")
