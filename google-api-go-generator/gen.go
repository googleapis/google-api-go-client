...
func (meth *Method) generateCode() {
	...
	if meth.supportsMediaUpload() {
		...
		pn("// At most one of Media and ResumableMedia may be set.")
		pn("// Parameter size specifies the full content size in bytes, and should be set to -1 if")
		pn("// content size is not known (e.g. for dynamically generated content).")
		pn("// Parameter chunkSize specifies the size in bytes of individual upload chunk, and should")
		pn("// be typically set to -1 to allow the library to automatically select chunk size.")
		pn("// When explicitly selecting chunkSize, note the potential memory usage implications:")
		pn("// one full chunk may have to be buffered in memory.")
		pn(`// Parameter mediaType identifies the MIME media type of the upload, such as "image/png".`)
		pn(`// If mediaType is "", it will be auto-detected.`)
		pn("func (c *%s) ResumableMedia(ctx context.Context, r io.Reader, size int64, chunkSize int64, mediaType string) *%s {", callName, callName)
		pn("c.ctx_ = ctx")
		pn("c.media_ = r")
		pn("c.mediaType_ = mediaType")
		pn("c.size_ = size")
		pn("c.chunkSize_ = chunkSize")
	...
	if meth.supportsMediaUpload() {
		pn("var fakeRdr bool")
		pn("var resumable io.ReaderAt")
		pn(`if c.protocol_ == "resumable" {`)
		pn(" var ok bool")
		pn(" if resumable, ok = c.media_.(io.ReaderAt); !ok || c.size_ <= 0 {")
		pn("  fakeRdr = true")
		pn("  resumable = googleapi.NewFakeReaderAt(c.media_)")
		pn(" }")
		pn(` if c.mediaType_ == "" {`)
		pn("  c.mediaType_ = googleapi.DetectMediaType(resumable)")
		pn(" }")
	...
	if meth.supportsMediaUpload() {
		...
		pn("  ContentLength: c.size_,")
		pn("  ChunkSize:     googleapi.CalcChunkSize(c.chunkSize_),")
