...
const apiId = "servicedirectory:v1"
...
type Endpoint struct {
	...
	// pairs. Annotations that go beyond this limit are rejected * Valid annotation
	// keys have two segments: an optional prefix and name, separated by a slash
	// (/). The name segment is required and must be 63 characters or less,
	// beginning and ending with an alphanumeric character ([a-z0-9A-Z]) with
	// dashes (-), underscores (_), dots (.), and alphanumerics between. The prefix
	// is optional. If specified, the prefix must be a DNS subdomain: a series of
...
type ResolveServiceRequest struct {
	...
	// that have this address * `port>8080` returns endpoints that have port number
	// larger than 8080 *
	...
	// equal to `ep-1` * `annotations.owner!=sd AND annotations.foo=bar` returns
...
type Service struct {
	...
	// consumed by service clients. Restrictions: * The entire annotations
	// dictionary may contain up to 2000 characters, spread accoss all key-value
	// pairs. Annotations that go beyond this limit are rejected * Valid annotation
	// keys have two segments: an optional prefix and name, separated by a slash
	// (/). The name segment is required and must be 63 characters or less,
	// beginning and ending with an alphanumeric character ([a-z0-9A-Z]) with
	...
	Name string `json:"name,omitempty"`
	// Uid: Output only. The globally unique identifier of the service in the UUID4
	// format.
	Uid string `json:"uid,omitempty"`
...
// GetIamPolicy: Gets the IAM Policy for a resource (namespace or service
// only).
//
...
// General `filter` string syntax: ` ()` * “ can be `name` or `labels.` for
...
// this is the same as `labels:owner` * `labels.owner=sd` returns namespaces
// that have key/value `owner=sd` *
...
// SetIamPolicy: Sets the IAM Policy for a resource (namespace or service
// only).
//
...
// TestIamPermissions: Tests IAM permissions for a resource (namespace or
// service only).
//
...
// Filter sets the optional parameter "filter": The filter to list results by.
// General `filter` string syntax: ` ()` * “ can be `name` or `annotations.`
...
func (c *ProjectsLocationsNamespacesServicesResolveCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1/{+name}:resolve")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
...
// /my-service/endpoints/endpoint-c` returns endpoints that have name that is
// alphabetically later than the string, so "endpoint-e" is returned but
// "endpoint-a" is not * `annotations.owner!=sd AND annotations.foo=bar`
// returns endpoints that have `owner` in annotation key but value is not `sd`
// AND have key/value `foo=bar` * `doesnotexist.foo=bar` returns an empty list.
