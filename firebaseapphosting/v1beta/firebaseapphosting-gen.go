...
func NewProjectsLocationsService(s *Service) *ProjectsLocationsService {
	...
	rs.SupportedRuntimes = NewProjectsLocationsSupportedRuntimesService(s)
	return rs
}
...
type ProjectsLocationsService struct {
	...
	SupportedRuntimes *ProjectsLocationsSupportedRuntimesService
}
...
func NewProjectsLocationsSupportedRuntimesService(s *Service) *ProjectsLocationsSupportedRuntimesService {
	rs := &ProjectsLocationsSupportedRuntimesService{s: s}
	return rs
}

type ProjectsLocationsSupportedRuntimesService struct {
	s *Service
}
...
// ListSupportedRuntimesResponse: Response message for a list of supported
// runtimes.
type ListSupportedRuntimesResponse struct {
	// SupportedRuntimes: The list of supported runtimes.
	SupportedRuntimes []*SupportedRuntime `json:"supportedRuntimes,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "SupportedRuntimes") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "SupportedRuntimes") to include in
...
func (s ListSupportedRuntimesResponse) MarshalJSON() ([]byte, error) {
	type NoMethod ListSupportedRuntimesResponse
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}
...
// SupportedRuntime: Represents a single FAH supported runtime. Although
// instances of this resource are parented by a project and location, the set
// of available runtimes may vary across projects and locations, for example,
// during staged rollouts of new runtime support.
type SupportedRuntime struct {
	// AutomaticBaseImageUpdatesSupported: Output only. True if Automatic Base
	// Image Updates (ABIU) is supported for this runtime.
	AutomaticBaseImageUpdatesSupported bool `json:"automaticBaseImageUpdatesSupported,omitempty"`
	// DecommissionTime: Output only. The time at which this runtime will be
	// decommissioned. After this date, the runtime can no longer be used for new
	// builds.
	DecommissionTime string `json:"decommissionTime,omitempty"`
	// DeprecateTime: Output only. The time at which this runtime will effectively
	// be deprecated. After this date, the runtime is still usable but may not
	// receive new features or updates.
	DeprecateTime string `json:"deprecateTime,omitempty"`
	// Name: Identifier. The resource name of the supported runtime. Format:
	// projects/{project}/locations/{location}/supportedRuntimes/{runtime_id}
	Name string `json:"name,omitempty"`
	// RuntimeId: Output only. The identifier of the runtime, e.g., "nodejs22".
	RuntimeId string `json:"runtimeId,omitempty"`
	// ForceSendFields is a list of field names (e.g.
	// "AutomaticBaseImageUpdatesSupported") to unconditionally include in API
	...
	// "AutomaticBaseImageUpdatesSupported") to include in API requests with the
...
func (s SupportedRuntime) MarshalJSON() ([]byte, error) {
	type NoMethod SupportedRuntime
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}
...
type ProjectsLocationsSupportedRuntimesListCall struct {
	s            *Service
	parent       string
	urlParams_   gensupport.URLParams
	ifNoneMatch_ string
...
// List: Lists the runtimes supported by the backend. The list of runtimes may
// vary across projects and locations, for example, during staged rollouts of
// new runtime support.
//
//   - parent: The parent, which owns this collection of SupportedRuntime.
//     Format: projects/{project}/locations/{location}.
func (r *ProjectsLocationsSupportedRuntimesService) List(parent string) *ProjectsLocationsSupportedRuntimesListCall {
	c := &ProjectsLocationsSupportedRuntimesListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.parent = parent
	return c
}

// PageSize sets the optional parameter "pageSize": The suggested number of
// runtimes to return. This field is ignored. We return all runtimes in a
// single page regardless of the page size.
func (c *ProjectsLocationsSupportedRuntimesListCall) PageSize(pageSize int64) *ProjectsLocationsSupportedRuntimesListCall {
	c.urlParams_.Set("pageSize", fmt.Sprint(pageSize))
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse for more
// details.
func (c *ProjectsLocationsSupportedRuntimesListCall) Fields(s ...googleapi.Field) *ProjectsLocationsSupportedRuntimesListCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}
...
// object's ETag matches the given value. This is useful for getting updates
// only after the object has changed since the last request.
func (c *ProjectsLocationsSupportedRuntimesListCall) IfNoneMatch(entityTag string) *ProjectsLocationsSupportedRuntimesListCall {
	c.ifNoneMatch_ = entityTag
	return c
}

// Context sets the context to be used in this call's Do method.
func (c *ProjectsLocationsSupportedRuntimesListCall) Context(ctx context.Context) *ProjectsLocationsSupportedRuntimesListCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns a http.Header that can be modified by the caller to add
// headers to the request.
func (c *ProjectsLocationsSupportedRuntimesListCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *ProjectsLocationsSupportedRuntimesListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1beta/{+parent}/supportedRuntimes")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("GET", urls, nil)
	if err != nil {
		return nil, err
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "firebaseapphosting.projects.locations.supportedRuntimes.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "firebaseapphosting.projects.locations.supportedRuntimes.list" call.
// Any non-2xx status code is an error. Response headers are in either
// *ListSupportedRuntimesResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
func (c *ProjectsLocationsSupportedRuntimesListCall) Do(opts ...googleapi.CallOption) (*ListSupportedRuntimesResponse, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
	...
	ret := &ListSupportedRuntimesResponse{
		ServerResponse: googleapi.ServerResponse{
			Header:         res.Header,
			HTTPStatusCode: res.StatusCode,
		},
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "firebaseapphosting.projects.locations.supportedRuntimes.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
