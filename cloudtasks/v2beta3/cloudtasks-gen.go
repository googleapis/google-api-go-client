...
// Using AppEngineHttpQueue requires `appengine.applications.get` Google
...
// app_engine_routing. The `url` that the task will be sent to is: *
// `url =` host `+` relative_uri Tasks can be dispatched to secure app
// handlers, unsecure app handlers, and URIs restricted with `login:
// admin`. Because tasks are not run as any user, they cannot be
// dispatched to URIs restricted with `login: required` Task dispatches
// also do not follow redirects. The task attempt has succeeded if the
// app's request handler returns an HTTP response code in the range
// [`200` - `299`]. The task attempt has failed if the app's handler
// returns a non-2xx response code or Cloud Tasks does not receive
// response before the deadline. Failed tasks will be retried according
// to the retry configuration. `503` (Service Unavailable) is considered
// an App Engine system error instead of an application error and will
// cause Cloud Tasks' traffic congestion control to temporarily throttle
...
type PullMessage struct {
	...
	// bytes, only UTF-8 encoded tags can be used in Cloud Tasks. If a tag
	// isn't UTF-8 encoded, the tag will be empty when the task is returned
	// by Cloud Tasks.
...
type RateLimits struct {
	...
	// time. The token bucket algorithm is used to control the rate of task
	// dispatches. Each queue has a token bucket that holds tokens, up to
	// the maximum specified by `max_burst_size`. Each time a task is
	// dispatched, a token is removed from the bucket. Tasks will be
	...
	// picked by Cloud Tasks based on the value of
	...
	// `queue.yaml/xml`, `max_burst_size` is equal to bucket_size. If
	// UpdateQueue is called on a queue without explicitly setting a value
	// for `max_burst_size`, `max_burst_size` value will get updated if
	// UpdateQueue is updating max_dispatches_per_second.
...
type Task struct {
	...
	// Engine Task Queue pull queues to provide a way to inspect contents of
	// pull tasks through the CloudTasks.
