...
// "marlin". * `deviceBrand`: Matches error issues that occurred in the
// requested device brands. Example: `deviceBrand = "Google". *
// `deviceType`: Matches error issues that occurred in the requested
// device types. Example: `deviceType = "PHONE". * `errorIssueType`:
// Matches error issues of the requested types only. Valid candidates:
// `CRASH`, `ANR`. Example: `errorIssueType = CRASH OR errorIssueType =
// ANR`. * `appProcessState`: Matches error issues on the process state
// of an app, indicating whether an app runs in the foreground
// (user-visible) or background. Valid candidates: `FOREGROUND`,
