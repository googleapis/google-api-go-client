...
type IPAllocationPolicy struct {
	...
	// choose a range with a specific netmask. Set to a CIDR notation (e.g.
...
type MaintenanceWindow struct {
	...
	// subset of RFC-5545 `RRULE`. The only allowed values for `FREQ` field
	// are `FREQ=DAILY` and `FREQ=WEEKLY;BYDAY=...` Example values:
...
type NodeConfig struct {
	...
	// Location: Optional. The Compute Engine zone in which to deploy the
	// VMs used to run the Apache Airflow software, specified as a relative
	// resource name. For example: "projects/{projectId}/zones/{zoneId}".
	// This `location` must belong to the enclosing environment's project
	// and location. If both this field and `nodeConfig.machineType` are
	// specified, `nodeConfig.machineType` must belong to this `location`;
	// if both are unspecified, the service will pick a zone in the Compute
	// Engine region corresponding to the Cloud Composer location, and
	// propagate that choice to both fields. If only one field (`location`
	// or `nodeConfig.machineType`) is specified, the location information
	// from the specified field will be propagated to the unspecified field.
	Location string `json:"location,omitempty"`

	// MachineType: Optional. The Compute Engine machine type used for
	// cluster instances, specified as a relative resource name. For
	// example:
	...
	// machine type. If this field is unspecified, the `machineTypeId`
	...
	// communications, specified as a relative resource name. For example:
	...
	// Subnet Network is provided, `nodeConfig.subnetwork` must also be
	...
	// machine communications, specified as a relative resource name. For
	// example:
	...
	// firewalls. Each tag within the list must comply with RFC1035. Cannot
	// be updated.
...
type SoftwareConfig struct {
	...
	// property names must be written in snake_case. Property values can
	...
	// version or `latest`. When the patch version is omitted, the current
	// Cloud Composer patch version is selected. When `latest` is provided
	// instead of an explicit version number, the server replaces `latest`
	// with the current Cloud Composer version and stores that version
	// number in the same field. The portion of the image version that
	// follows *airflow-* is an official Apache Airflow repository release
	// name. See also Version List.
