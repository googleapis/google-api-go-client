...
func (meth *Method) generateCode() {
	...
	if meth.supportsMedia() {
		...
		pn("// Choose to use either Media or ResumableMedia, but not both.")
		pn(`// mediaType identifies the MIME media type of the upload (e.g. "image/png").`)
		pn("func (c *%s) ResumableMedia(ctx context.Context, r googleapi.SizeReaderAt, mediaType string) *%s {", callName, callName)
		pn("c.ctx_ = ctx")
		pn("c.resumable_ = r")
