...
type Aes128Encryption struct {
	// KeyUri: Required. URI of the key delivery service. This URI is inserted into
	// the M3U8 header.
	KeyUri string `json:"keyUri,omitempty"`
	// ForceSendFields is a list of field names (e.g. "KeyUri") to unconditionally
...
// AnimationEnd, the overlay object will keep the state of previous animation
...
type AudioAtom struct {
	...
	// Key: Required. The `EditAtom.key` that references the atom with audio inputs
...
// AudioChannelInput: Identifies which input file, track, and channel should be
// used.
...
// Crop: Video cropping configuration for the input video. The cropped input
...
// track. Elementary streams must be packaged before mapping and sharing
...
type Job struct {
	...
	// contain additional information about the failure when `failure_reason` is
	// present. *Note*: This feature is not yet available.
	...
	// Priority: Specify the priority of the job. Enter a value between 0 and 100,
	// where 0 is the lowest priority and 100 is the highest priority. The default
	// is 0.
	...
	// `Job.config`. The default is `preset/web-hd`. Preset Transcoder templates: -
...
type Manifest struct {
	...
	// MuxStreams: Required. List of user given `MuxStream.key`s that should appear
	// in this manifest. When `Manifest.type` is `HLS`, a media manifest with name
...
type MpegCommonEncryption struct {
	// KeyId: Required. 128 bit Key ID represented as lowercase hexadecimal digits
	// for use with common encryption.
...
type OriginUri struct {
	...
	// Hls: HLS manifest URI per https://tools.ietf.org/html/rfc8216#section-4.3.4.
...
type Output struct {
	...
	// empty the value is populated from `Job.output_uri`.
	Uri string `json:"uri,omitempty"`
...
type Progress struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Analyzed") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type TextAtom struct {
	// Inputs: List of `Job.inputs` that should be embedded in this atom. Only one
	// input is supported.
...
type VideoStream struct {
	...
	// equal to zero. Must be less than `VideoStream.gop_frame_count` if set. The
	// default is 0.
	...
	// value is 1,000. The maximum value for H264/H265 is 800,000,000. The maximum
	...
	// 'yuv444p' pixel format. - 'yuv420p10' 10-bit HDR pixel format. - 'yuv422p10'
	// 10-bit HDR pixel format. - 'yuv444p10' 10-bit HDR pixel format. -
	// 'yuv420p12' 12-bit HDR pixel format. - 'yuv422p12' 12-bit HDR pixel format.
	...
	// are FFmpeg-compatible. Note that certain values for this field may cause the
	...
	// Supported rate control modes: - 'vbr' - variable bitrate - 'crf' - constant
	// rate factor
...
// JobTemplateId sets the optional parameter "jobTemplateId": Required. The ID
// to use for the job template, which will become the final component of the
// job template's resource name. This value should be 4-63 characters, and
// valid characters must match the regular expression `a-zA-Z*`.
...
// *JobTemplate.ServerResponse.Header or (if a response was returned at all) in
...
//
//   - parent: The parent location from which to retrieve the collection of job
