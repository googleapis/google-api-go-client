...
func (s *Schema) writeSchemaStruct(api *API) {
	...
	comment := fmt.Sprintf("%s is a list of field names (e.g. %q) to unconditionally include in API requests. By default, fields with empty values are ommitted from API requests. However, any non-pointer, non-interface field appearing in %s will be sent to the server regardless of whether the field is empty or not. This may be used to include empty fields in Patch requests.", forceSendName, firstFieldName, forceSendName)
	s.api.p("\n")
