...
type Deployment struct {
	...
	// same target. Validation regex for label names: `^a-z0-9?$`. Value for
	// an individual label must be <= 512 bytes, the total size of all label
	// names and values must be <= 1024 bytes. Label named "language" can be
	// used to record the programming language of the profiled deployment.
	// The standard choices for the value include "java", "go", "python",
	// "ruby", "nodejs", "php", "dotnet". For deployments running on Google
	// Cloud Platform, "zone" or "region" label should be present describing
	// the deployment location. An example of a zone is "us-central1-a", an
