...
const basePathTemplate = "https://genomics.UNIVERSE_DOMAIN/"
...
// ComputeEngine: Describes a Compute Engine resource that is being managed by
// a running pipeline.
...
// ControllerConfig: Stores the information that the controller will fetch from
// the server in order to run. Should only be used by VMs created by the
...
type ControllerConfig struct {
	...
	// ForceSendFields is a list of field names (e.g. "Cmd") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Cmd") to include in API requests
...
type Disk struct {
	...
	// Specifies the path in the docker container where files on this disk should
	// be located. For example, if `mountPoint` is `/mnt/disk`, and the parameter
	// has `localPath` `inputs/file.txt`, the docker container can access the data
	...
	// ReadOnly: Specifies how a sourced-base persistent disk will be mounted. See
	...
	//
	// Possible values:
	//   "TYPE_UNSPECIFIED" - Default disk type. Use one of the other options
	// below.
	...
	// solid-state disk. See https://cloud.google.com/compute/docs/disks/#pdspecs
	// for details.
...
type DockerExecutor struct {
	...
	// non-zero exit code, output parameter de-localization will be skipped and the
	// pipeline operation's `error` field will be populated. Maximum command string
	// length is 16384.
	Cmd string `json:"cmd,omitempty"`
...
// `pageSize` pipelines. If it contains `pageSize` pipelines, and more
// pipelines exist, then `nextPageToken` will be populated and should be used
// as the `pageToken` argument to a subsequent ListPipelines request.
...
// LocalCopy: LocalCopy defines how a remote file should be copied to and from
// the VM.
type LocalCopy struct {
	// Disk: Required. The name of the disk where this parameter is located. Can be
	// the name of one of the disks specified in the Resources field, or "boot",
	// which represents the Docker instance's boot disk and has a mount point of
	// `/`.
	Disk string `json:"disk,omitempty"`
	// Path: Required. The path within the user's docker container where this input
...
type LoggingOptions struct {
	...
	// which case logs will be output with a unique identifier as the filename in
	// that directory, or as a fully specified path, which must end in `.log`, in
	// which case that path will be used, and the user must ensure that logs are
	// not overwritten. Stdout and stderr logs from the run are also generated and
...
type OperationMetadata struct {
	...
	// CreateTime: The time at which the job was submitted to the Genomics service.
	...
	// execution. This also contains any warnings that were generated during import
	// or export.
	...
	// v1beta2 API and a GetOperation is performed on v1 API, a v1 request will be
	// returned.
...
// Pipeline: The pipeline object. Represents a transformation from a set of
// input parameters to a set of output parameters. The transformation is
// defined as a docker image and command to run within that image. Each
// pipeline is run on a Google Compute Engine VM. A pipeline can be created
// with the `create` method and then later run with the `run` method, or a
// pipeline can be defined and run all at once with the `run` method.
type Pipeline struct {
	...
	// CreatePipeline is called. Cannot be specified in the Pipeline used in the
	// CreatePipelineRequest, and will be populated in the response to
	// CreatePipeline and all subsequent Get and List calls. Indicates that the
	// service has registered this pipeline.
	PipelineId string `json:"pipelineId,omitempty"`
	// ProjectId: Required. The project in which to create the pipeline. The caller
...
// PipelineParameter: Parameters facilitate setting and delivering data into
// the pipeline's execution environment. They are defined at create time, with
// optional defaults, and can be overridden at run time. If `localCopy` is
// unset, then the parameter specifies a string that is passed as-is into the
// pipeline, as the value of the environment variable with the given name. A
// default value can be optionally specified at create time. The default can be
// overridden at run time using the inputs map. If no default is given, a value
// must be supplied at runtime. If `localCopy` is defined, then the parameter
...
// direction of the copy is reversed: ``` gsutil cp /mnt/disk/file.txt
// gs://my-bucket/bar.txt ``` Acceptable paths are: Local pathGoogle Cloud
// Storage path file file file directory - directory must already exist glob
// directory - directory will be created if it doesn't exist One restriction
// due to docker limitations, is that for outputs that are found on the boot
...
type PipelineParameter struct {
	...
	// runtime. If `localCopy` is present, then this must be a Google Cloud Storage
	// path beginning with `gs://`.
	DefaultValue string `json:"defaultValue,omitempty"`
	// Description: Human-readable description.
	Description string `json:"description,omitempty"`
	// LocalCopy: If present, this parameter is marked for copying to and from the
	// VM. `LocalCopy` indicates where on the VM the file should be. The value
	// given to this parameter (either at runtime or using `defaultValue`) must be
	// the remote path where the file should be.
	LocalCopy *LocalCopy `json:"localCopy,omitempty"`
	// Name: Required. Name of the parameter - the pipeline runner uses this string
...
type PipelineResources struct {
	// AcceleratorCount: Optional. The number of accelerators of the specified type
	// to attach. By specifying this parameter, you will download and install the
	// following third-party software onto your managed Compute Engine instances:
	...
	// Tesla® drivers and NVIDIA® CUDA toolkit. Please see
	...
	// (https://cloud.google.com/sdk/gcloud/reference/compute/instances/create). In
	// order to use this, must be true for both create time and run time. Cannot be
	// true at run time if false at create time. If you need to ssh into a private
	// IP VM for debugging, you can ssh to a public VM and then ssh into the
	// private VM's Internal IP. If noAddress is set, this pipeline run may only
	// load docker images from Google Container Registry and not Docker Hub. Before
	// using this, you must configure access to Google services from internal IPs
	...
	// to use this, must be true for both create time and run time. Cannot be true
...
type RunPipelineArgs struct {
	...
	// documentation. All input parameters that do not have default values must be
	// specified. If parameters with defaults are specified here, the defaults will
	// be overridden.
	Inputs map[string]string `json:"inputs,omitempty"`
	// KeepVmAliveOnFailureDuration: How long to keep the VM up after a failure
	// (for example docker command failed, copying input or output files failed,
	// etc). While the VM is up, one can ssh into the VM to debug. Default is 0;
	...
	Resources *PipelineResources `json:"resources,omitempty"`
	// ServiceAccount: The Google Cloud Service Account that will be used to access
	// data and services. By default, the compute service account associated with
	// `projectId` is used.
...
// specified, it refers to a saved pipeline created with CreatePipeline and set
// as the `pipelineId` of the returned Pipeline object. If `ephemeralPipeline`
// is specified, that pipeline is run once with the given args and not saved.
// It is an error to specify both `pipelineId` and `ephemeralPipeline`.
// `pipelineArgs` must be specified.
type RunPipelineRequest struct {
	...
	PipelineId string `json:"pipelineId,omitempty"`
	// ForceSendFields is a list of field names (e.g. "EphemeralPipeline") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "EphemeralPipeline") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type RuntimeMetadata struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ComputeEngine") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ServiceAccount struct {
	...
	// Scopes: List of scopes to be enabled for this service account on the VM. The
...
// SetOperationStatusRequest: Request to set operation status. Should only be
// used by VMs created by the Pipelines Service and not by end users.
...
// TimestampEvent: Stores the list of events and times they occured for major
// events in job execution.
...
// (https://cloud.google.com/iam) permission: * `genomics.operations.get`
//
...
// (https://cloud.google.com/iam) permission: * `genomics.operations.list`
//
...
// key is a label key. * done: If the pipeline is running, this value is false.
// Once the pipeline finishes, the value is true. In v1 and v1alpha2, the
...
// where key is a label key. Examples: * `projectId = my-project AND createTime
...
// Create: Creates a pipeline that can be run later. Create takes a Pipeline
...
// Delete: Deletes a pipeline based on ID. Caller must have WRITE permission to
// the project.
//
...
// Get: Retrieves a pipeline based on ID. Caller must have READ permission to
// the project.
//
...
// GetControllerConfig: Gets controller configuration information. Should only
// be called by VMs created by the Pipelines Service and not by end users.
...
// OperationId sets the optional parameter "operationId": The operation to
// retrieve controller configuration for.
...
// Any non-2xx status code is an error. Response headers are in either
// *ControllerConfig.ServerResponse.Header or (if a response was returned at
...
// List: Lists pipelines. Caller must have READ permission to the project.
func (r *PipelinesService) List() *PipelinesListCall {
	c := &PipelinesListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	return c
}

// NamePrefix sets the optional parameter "namePrefix": Pipelines with names
// that match this prefix should be returned. If unspecified, all pipelines in
...
// PageSize sets the optional parameter "pageSize": Number of pipelines to
// return at once. Defaults to 256, and max is 2048.
func (c *PipelinesListCall) PageSize(pageSize int64) *PipelinesListCall {
	c.urlParams_.Set("pageSize", fmt.Sprint(pageSize))
	return c
}

// PageToken sets the optional parameter "pageToken": Token to use to indicate
// where to start getting results. If unspecified, returns the first page of
// results.
...
// ProjectId sets the optional parameter "projectId": Required. The name of the
// project to search for pipelines. Caller must have READ access to this
// project.
...
// Run: Runs a pipeline. If `pipelineId` is specified in the request, then run
// a saved pipeline. If `ephemeralPipeline` is specified, then run that
// pipeline once without saving a copy. The caller must have READ permission to
// the project where the pipeline is stored and WRITE permission to the project
// where the pipeline will be run, as VMs will be created and storage will be
// used. If a pipeline operation is still running after 6 days, it will be
// canceled.
...
// SetOperationStatus: Sets status of a given operation. Any new timestamps (as
// determined by description) are appended to TimestampEvents. Should only be
// called by VMs created by the Pipelines Service and not by end users.
