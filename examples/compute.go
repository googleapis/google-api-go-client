...
func computeMain(client *http.Client, argv []string) {
	...
	h, op, err := service.Instances.Insert(projectId, zone, instance).DoHeader()
	log.Printf("Got compute.Operation, err: %#v, %v", op, err)
	log.Printf("Etag=%v", h.Get("Etag"))

	inst, err := service.Instances.Get(projectId, zone, instanceName).IfNoneMatch(h.Get("Etag")).Do()
