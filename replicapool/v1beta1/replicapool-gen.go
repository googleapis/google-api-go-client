...
const basePathTemplate = "https://www.UNIVERSE_DOMAIN/replicapool/v1beta1/projects/"
...
type AccessConfig struct {
	...
	// Type: Type of this access configuration file. Currently only ONE_TO_ONE_NAT
	// is supported.
...
type Action struct {
	// Commands: A list of commands to run, one per line. If any command fails, the
	// whole action is considered a failure and no further actions are run. This
	// also marks the virtual machine or replica as a failure.
	...
	// TimeoutMilliSeconds: If an action's commands on a particular replica do not
	// finish in the specified timeoutMilliSeconds, the replica is considered to be
...
// ExistingDisk: A pre-existing persistent disk that will be attached to every
// Replica in the Pool in READ_ONLY mode.
...
type HealthCheck struct {
	...
	// HealthyThreshold: The number of consecutive health check requests that need
	// to succeed before the replica is considered healthy again. The default value
	// is 2.
	...
	// TimeoutSec: How long (in seconds) to wait before a timeout failure for this
	// healthcheck. The default value is 5 seconds.
	TimeoutSec int64 `json:"timeoutSec,omitempty"`
	// UnhealthyThreshold: The number of consecutive health check requests that
	// need to fail in order to consider the replica unhealthy. The default value
	// is 2.
...
// Metadata: A Compute Engine metadata entry. Identical to the metadata on the
...
// MetadataItem: A Compute Engine metadata item, defined as a key:value pair.
// Identical to the metadata on the corresponding Compute Engine resource.
...
// NetworkInterface: A Compute Engine NetworkInterface resource. Identical to
// the NetworkInterface on the corresponding Compute Engine resource.
type NetworkInterface struct {
	// AccessConfigs: An array of configurations for this interface. This specifies
...
// NewDisk: A Persistent Disk resource that will be created and attached to
...
type NewDiskInitializeParams struct {
	...
	// create this disk. If you provide a name of the source image, Replica Pool
	// will look for an image with that name in your project. If you are specifying
	// an image provided by Compute Engine, you will need to provide the full URL
	// with the correct project, such
	// as:
...
type Pool struct {
	...
	// pool. This must match the regex a-z ([-a-z0-9]*[a-z0-9])?. If specified, the
	// instances in this replica pool will be named in the format
	// <base-instance-name>-<ID>. The <ID> postfix will be a four character
	// alphanumeric identifier generated by the service.
	//
	...
	// Name: The name of the replica pool. Must follow the regex a-z
	// ([-a-z0-9]*[a-z0-9])? and be 1-28 characters long.
	Name string `json:"name,omitempty"`
	...
	// managed by this pool. If specified, the replicas in this replica pool will
	// be added to the specified target pools for load balancing purposes. The
	// replica pool must live in the same region as the specified target pools.
	// These values must be the target pool resource names, and not fully qualified
	// URLs.
	TargetPools []string `json:"targetPools,omitempty"`
	// Template: The template to use when creating replicas in this pool. This
	// template is used during initial instance creation of the pool, when growing
	// the pool in size, or when a replica restarts.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AutoRestart") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type PoolsDeleteRequest struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AbandonInstances") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Replica: An individual Replica within a Pool. Replicas are automatically
...
type ReplicaStatus struct {
	// Details: [Output Only] Human-readable details about the current state of the
	// replica
...
type ReplicasDeleteRequest struct {
	...
	// but the virtual machine instance will remain. By default, this is set to
	// false and the instance will be deleted along with the replica.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AbandonInstance") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// ServiceAccount: A Compute Engine service account, identical to the Compute
// Engine resource.
...
type Template struct {
	...
	// is run as shell commands which are executed one after the other in the same
	// bash shell, so any state established by one command is inherited by later
	// commands.
	...
	HealthChecks []*HealthCheck `json:"healthChecks,omitempty"`
	// Version: A free-form string describing the version of this template. You can
	...
	// can define settings such as the machine type and the image of replicas in
...
// VmParams: Parameters for creating a Compute Engine Instance resource. Most
...
type VmParams struct {
	...
	// each replica in the pool. Each disk will be attached in read-only mode to
	// every replica.
	DisksToAttach []*ExistingDisk `json:"disksToAttach,omitempty"`
	// DisksToCreate: A list of Disk resources to create and attach to each Replica
	// in the Pool. Currently, you can only define one disk and it must be a root
	// persistent disk. Note that Replica Pool will create a root persistent disk
	// for each replica.
	...
	// NetworkInterfaces: A list of network interfaces for the instance. Currently
	// only one interface is supported by Google Compute Engine, ONE_TO_ONE_NAT.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BaseInstanceName") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// results to be returned. Acceptable values are 0 to 100, inclusive. (Default:
// 50)
...
// nextPageToken value returned by a previous list request to obtain the next
...
// Any non-2xx status code is an error. Response headers are in either
// *PoolsListResponse.ServerResponse.Header or (if a response was returned at
...
// Resize: Resize a pool. This is an asynchronous operation, and multiple
...
// NumReplicas sets the optional parameter "numReplicas": The desired number of
// replicas to resize to. If this number is larger than the existing number of
...
// Any non-2xx status code is an error. Response headers are in either
// *Replica.ServerResponse.Header or (if a response was returned at all) in
// error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *ReplicasListResponse.ServerResponse.Header or (if a response was returned
