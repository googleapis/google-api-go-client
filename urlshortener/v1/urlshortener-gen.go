...
// Exactly one of *Url or error will be non-nil.
// Any non-2xx status code is an error.
// Response headers are in either *Url.ServerResponse.Header
...
// Exactly one of *UrlHistory or error will be non-nil.
// Any non-2xx status code is an error.
// Response headers are in either *UrlHistory.ServerResponse.Header
// or (if a response was returned at all) in error.(*googleapi.Error).Header.
// googleapi.IsNotModified can be called to check if http.StatusNotModified is returned.
