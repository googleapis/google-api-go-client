...
const basePathTemplate = "https://vectortile.UNIVERSE_DOMAIN/"
...
// Area: Represents an area. Used to represent regions such as water, parks,
// etc. Next ID: 10
type Area struct {
	// BasemapZOrder: The z-order of this geometry when rendered on a flat basemap.
	...
	// Line.basemap_z_order, and in fact may yield more accurate rendering (where a
	// line may be rendered beneath an area).
	...
	// feature that it belongs to: that is, some of the edges are bordering another
	// feature.
	HasExternalEdges bool `json:"hasExternalEdges,omitempty"`
	// InternalEdges: When has_external_edges is true, the polygon has some edges
	// that border another feature. This field indicates the internal edges that do
	// not border another feature. Each value is an index into the vertices array,
	// and denotes the start vertex of the internal edge (the next vertex in the
	...
	// starting vertex of the loop is internal. This field may be used for styling.
	// For example, building parapets could be placed only on the external edges of
	// a building polygon, or water could be lighter colored near the external
	// edges of a body of water. If has_external_edges is false, all edges are
	// internal and this field will be empty.
	InternalEdges []int64 `json:"internalEdges,omitempty"`
	// LoopBreaks: Identifies the boundary loops of the polygon. Only set for
	// INDEXED_TRIANGLE polygons. Each value is an index into the vertices array
	// indicating the beginning of a loop. For instance, values of [2, 5] would
	// indicate loop_data contained 3 loops with indices 0-1, 2-4, and 5-end. This
	// may be used in conjunction with the internal_edges field for styling polygon
	// boundaries. Note that an edge may be on a polygon boundary but still
	// internal to the feature. For example, a feature split across multiple tiles
	// will have an internal polygon boundary edge along the edge of the tile.
	LoopBreaks []int64 `json:"loopBreaks,omitempty"`
	// TriangleIndices: When the polygon encoding is of type INDEXED_TRIANGLES,
	// this contains the indices of the triangle vertices in the vertex_offsets
	// field. There are 3 vertex indices per triangle.
	...
	// shape. The following diagram showes a triangle fan polygon with the vertices
	// labelled with their indices in the vertex_offset list. Triangle fan polygons
	// always have a single boundary loop. Vertices may be in either a clockwise or
	// counterclockwise order. (1) / \ / \ / \ (0)-----(2) / \ / / \ / / \ /
	// (4)-----(3)
	//   "INDEXED_TRIANGLES" - The polygon is a set of triangles with three vertex
	// indices per triangle. The vertex indices can be found in the
	...
	// (3)----(5)
	//   "TRIANGLE_STRIP" - A strip of triangles, where each triangle uses the last
	// edge of the previous triangle. Vertices may be in either a clockwise or
	// counterclockwise order. Only polygons without the has_external_edges flag
	// set will use triangle strips. (0) / \ / \ / \ (2)-----(1) / \ / / \ / / \ /
	// (4)-----(3)
	...
	// ZOrder: The z-ordering of this area. Areas with a lower z-order should be
	// rendered beneath areas with a higher z-order. This z-ordering does not imply
	// anything about the altitude of the line relative to the ground, but it can
	// be used to prevent z-fighting during rendering on the client. This
	...
	// NullFields is a list of field names (e.g. "BasemapZOrder") to include in API
...
// BasemapZOrder: Metadata necessary to determine the ordering of a particular
// basemap element relative to others. To render the basemap correctly, sort by
// z-plane, then z-grade, then z-within-grade.
type BasemapZOrder struct {
	// ZGrade: The second most significant component of the ordering of a component
	// to be rendered onto the basemap.
	ZGrade int64 `json:"zGrade,omitempty"`
	// ZPlane: The most significant component of the ordering of a component to be
	// rendered onto the basemap.
	ZPlane int64 `json:"zPlane,omitempty"`
	// ZWithinGrade: The least significant component of the ordering of a component
	// to be rendered onto the basemap.
	ZWithinGrade int64 `json:"zWithinGrade,omitempty"`
	// ForceSendFields is a list of field names (e.g. "ZGrade") to unconditionally
...
// ExtrudedArea: Represents a height-extruded area: a 3D prism with a constant
// X-Y plane cross section. Used to represent extruded buildings. A single
// building may consist of several extruded areas. The min_z and max_z fields
// are scaled to the size of the tile. An extruded area with a max_z value of
// 4096 has the same height as the width of the tile that it is on.
type ExtrudedArea struct {
	// Area: The area representing the footprint of the extruded area.
	Area *Area `json:"area,omitempty"`
	// MaxZ: The z-value in local tile coordinates where the extruded area ends.
	MaxZ int64 `json:"maxZ,omitempty"`
	// MinZ: The z-value in local tile coordinates where the extruded area begins.
	// This is non-zero for extruded areas that begin off the ground. For example,
	// a building with a skybridge may have an extruded area component with a
	// non-zero min_z.
...
type Feature struct {
	// DisplayName: The localized name of this feature. Currently only returned for
	// roads.
	DisplayName string `json:"displayName,omitempty"`
	// Geometry: The geometry of this feature, representing the space that it
	// occupies in the world.
	Geometry *Geometry `json:"geometry,omitempty"`
	// PlaceId: Place ID of this feature, suitable for use in Places API details
	// requests.
	...
	// goods.
	//   "SCHOOL" - Institution where young people receive general (not vocation or
	// professional) education.
	//   "SEGMENT" - Segments such as roads and train lines.
	//   "ROAD" - A way leading from one place to another intended for use by
	// vehicles.
	//   "LOCAL_ROAD" - A small city street, typically for travel in a residential
	// neighborhood.
...
// FeatureTile: A tile containing information about the map features located in
// the region it covers.
type FeatureTile struct {
	// Coordinates: The global tile coordinates that uniquely identify this tile.
	Coordinates *TileCoordinates `json:"coordinates,omitempty"`
	// Features: Features present on this map tile.
	Features []*Feature `json:"features,omitempty"`
	// Name: Resource name of the tile. The tile resource name is prefixed by its
	...
	// determines how long this Tile response may be cached by the client. See also
	// version_id and STATUS_OK_DATA_UNCHANGED.
	...
	// particular, the tile's features and providers will not be populated when the
	// tile data is identical. However, the cache-control header and version_id can
	// still change even when the tile contents itself does not, so clients should
	// always use the most recent values returned by the API.
	Status string `json:"status,omitempty"`
	// VersionId: An opaque value, usually less than 30 characters, that contains
	// version info about this tile and the data that was used to generate it. The
	// client should store this value in its tile cache and pass it back to the API
	// in the client_tile_version_id field of subsequent tile requests in order to
	// enable the API to detect when the new tile would be the same as the one the
	// client already has in its cache. Also see STATUS_OK_DATA_UNCHANGED.
...
// uniformly spaced points containing elevation data. Each point within the
// grid represents the altitude in meters above average sea level at that
// location within the tile. Elevations provided are (generally) relative to
// the EGM96 geoid, however some areas will be relative to NAVD88. EGM96 and
// NAVD88 are off by no more than 2 meters. The grid is oriented north-west to
// south-east, as illustrated: rows[0].a[0] rows[0].a[m] +-----------------+ |
// | | N | | ^ | | | | | W <-----> E | | | | | v | | S | | |
// +-----------------+ rows[n].a[0] rows[n].a[m] Rather than storing the
// altitudes directly, we store the diffs between them as integers at some
// requested level of precision to take advantage of integer packing. The
// actual altitude values a[] can be reconstructed using the scale and each
// row's first_altitude and altitude_diff fields. More details in
// go/elevation-encoding-options-for-enduro under "Recommended implementation".
type FirstDerivativeElevationGrid struct {
	// AltitudeMultiplier: A multiplier applied to the altitude fields below to
	// extract the actual altitudes in meters from the elevation grid.
	...
	// ForceSendFields is a list of field names (e.g. "AltitudeMultiplier") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AltitudeMultiplier") to include
...
// Geometry: Represents the geometry of a feature, that is, the shape that it
// has on the map. The local tile coordinate system has the origin at the
// north-west (upper-left) corner of the tile, and is scaled to 4096 units
// across each edge. The height (Z) axis has the same scale factor: an extruded
// area with a max_z value of 4096 has the same height as the width of the tile
// that it is on. There is no clipping boundary, so it is possible that some
// coordinates will lie outside the tile boundaries.
type Geometry struct {
	// Areas: The areas present in this geometry.
	Areas []*Area `json:"areas,omitempty"`
	// ExtrudedAreas: The extruded areas present in this geometry. Not populated if
	...
	// ModeledVolumes: The modeled volumes present in this geometry. Not populated
	// unless enable_modeled_volumes has been set in GetFeatureTileRequest.
...
// Line: Represents a 2D polyline. Used to represent segments such as roads,
// train tracks, etc.
type Line struct {
	// BasemapZOrder: The z-order of this geometry when rendered on a flat basemap.
	// Geometry with a lower z-order should be rendered beneath geometry with a
	...
	// Area.basemap_z_order, and in fact may yield more accurate rendering (where a
	// line may be rendered beneath an area).
	BasemapZOrder *BasemapZOrder `json:"basemapZOrder,omitempty"`
	// VertexOffsets: The vertices present in the polyline.
	VertexOffsets *Vertex2DList `json:"vertexOffsets,omitempty"`
	// ZOrder: The z-order of the line. Lines with a lower z-order should be
	// rendered beneath lines with a higher z-order. This z-ordering does not imply
	// anything about the altitude of the area relative to the ground, but it can
	// be used to prevent z-fighting during rendering on the client. In general,
	// larger and more important road features will have a higher z-order line
	// associated with them. This z-ordering can only be used to compare lines, and
	// cannot be compared with the z_order field in the Area message. The z-order
	// may be negative or zero. Prefer Line.basemap_z_order.
	ZOrder int64 `json:"zOrder,omitempty"`
	// ForceSendFields is a list of field names (e.g. "BasemapZOrder") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
...
// ModeledVolume: Represents a modeled volume in 3D space. Used to represent 3D
// buildings.
type ModeledVolume struct {
	// Strips: The triangle strips present in this mesh.
	Strips []*TriangleStrip `json:"strips,omitempty"`
	// VertexOffsets: The vertices present in the mesh defining the modeled volume.
	VertexOffsets *Vertex3DList `json:"vertexOffsets,omitempty"`
	// ForceSendFields is a list of field names (e.g. "Strips") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
// ProviderInfo: Information about the data providers that should be included
// in the attribution string shown by the client.
...
// Relation: Represents a relation to another feature in the tile. For example,
// a building might be occupied by a given POI. The related feature can be
// retrieved using the related feature index.
type Relation struct {
	// RelatedFeatureIndex: Zero-based index to look up the related feature from
	// the list of features in the tile.
	...
	//   "PRIMARILY_OCCUPIED_BY" - The origin feature is primarily occupied by the
	// related feature.
	RelationType string `json:"relationType,omitempty"`
	// ForceSendFields is a list of field names (e.g. "RelatedFeatureIndex") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "RelatedFeatureIndex") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type RoadInfo struct {
	// IsPrivate: Road has signage discouraging or prohibiting use by the general
	// public. E.g., roads with signs that say "Private", or "No trespassing."
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "IsPrivate") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Row: A row of altitude points in the elevation grid, ordered from west to
// east.
type Row struct {
	// AltitudeDiffs: The difference between each successive pair of altitudes,
	// from west to east. The first, westmost point, is just the altitude rather
	// than a diff. The units are specified by the altitude_multiplier parameter
	...
	// reconstructed with: a[0] = altitude_diffs[0] * altitude_multiplier when n >
	// 0, a[n] = a[n-1] + altitude_diffs[n-1] * altitude_multiplier.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AltitudeDiffs") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// SecondDerivativeElevationGrid: A packed representation of a 2D grid of
// uniformly spaced points containing elevation data. Each point within the
// grid represents the altitude in meters above average sea level at that
// location within the tile. Elevations provided are (generally) relative to
// the EGM96 geoid, however some areas will be relative to NAVD88. EGM96 and
...
// Note that the data is packed in such a way that is fast to decode in Unity
// and that further optimizes wire size.
type SecondDerivativeElevationGrid struct {
	// AltitudeMultiplier: A multiplier applied to the elements in the encoded data
	// to extract the actual altitudes in meters.
	AltitudeMultiplier float64 `json:"altitudeMultiplier,omitempty"`
	// ColumnCount: The number of columns included in the encoded elevation data
	// (i.e. the horizontal resolution of the grid).
	ColumnCount int64 `json:"columnCount,omitempty"`
	// EncodedData: A stream of elements each representing a point on the tile
	// running across each row from left to right, top to bottom. There will be
	// precisely horizontal_resolution * vertical_resolution elements in the
	// stream. The elements are not the heights, rather the second order derivative
	// of the values one would expect in a stream of height data. Each element is a
	// varint with the following encoding:
	// ------------------------------------------------------------------------| |
	// Head Nibble |
	// ------------------------------------------------------------------------| |
	...
	// RowCount: The number of rows included in the encoded elevation data (i.e.
...
type SegmentInfo struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "RoadInfo") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// TerrainTile: A tile containing information about the terrain located in the
// region it covers.
type TerrainTile struct {
	...
	// SecondDerivativeElevationGrid. cs/symbol:SecondDerivativeElevationGrid. See
	// go/byte-encoded-terrain for more details.
...
// TileCoordinates: Global tile coordinates. Global tile coordinates reference
// a specific tile on the map at a specific zoom level. The origin of this
// coordinate system is always at the northwest corner of the map, with x
// values increasing from west to east and y values increasing from north to
// south. Tiles are indexed using x, y coordinates from that origin. The zoom
// level containing the entire world in a tile is 0, and it increases as you
// zoom in. Zoom level n + 1 will contain 4 times as many tiles as zoom level
// n. The zoom level controls the level of detail of the data that is returned.
// In particular, this affects the set of feature types returned, their
// density, and geometry simplification. The exact tile contents may change
// over time, but care will be taken to keep supporting the most important use
// cases. For example, zoom level 15 shows roads for orientation and planning
// in the local neighborhood and zoom level 17 shows buildings to give users on
// foot a sense of situational awareness.
...
// TriangleStrip: Represents a strip of triangles. Each triangle uses the last
// edge of the previous one. The following diagram shows an example of a
// triangle strip, with each vertex labeled with its index in the vertex_index
// array. (1)-----(3) / \ / \ / \ / \ / \ / \ (0)-----(2)-----(4) Vertices may
// be in either clockwise or counter-clockwise order.
type TriangleStrip struct {
	// VertexIndices: Index into the vertex_offset array representing the next
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "VertexIndices") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Vertex2DList: 2D vertex list used for lines and areas. Each entry represents
// an offset from the previous one in local tile coordinates. The first entry
// is offset from (0, 0). For example, the list of vertices [(1,1), (2, 2), (1,
// 2)] would be encoded in vertex offsets as [(1, 1), (1, 1), (-1, 0)].
type Vertex2DList struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "XOffsets") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Vertex3DList: 3D vertex list used for modeled volumes. Each entry represents
// an offset from the previous one in local tile coordinates. The first
// coordinate is offset from (0, 0, 0).
...
// "alwaysIncludeBuildingFootprints": Flag indicating whether the returned tile
// will always contain 2.5D footprints for structures. If
...
// ClientInfoApiClient sets the optional parameter "clientInfo.apiClient": API
// client name and version. For example, the SDK calling the API. The exact
...
// "clientInfo.applicationId": Application ID, such as the package name on
...
// ClientInfoDeviceModel sets the optional parameter "clientInfo.deviceModel":
...
// "clientInfo.operatingSystem": Operating system name and version as reported
// by the OS. For example, "Mac OS X 10.10.4". The exact format is
// platform-dependent.
...
// ClientInfoUserId sets the optional parameter "clientInfo.userId": Required.
// A client-generated user ID. The ID should be generated and persisted during
// the first user session or whenever a pre-existing ID is not found. The exact
// format is up to the client. This must be non-empty in a
...
// ClientTileVersionId sets the optional parameter "clientTileVersionId":
...
// "enableDetailedHighwayTypes": Flag indicating whether detailed highway types
// should be returned. If this is set, the CONTROLLED_ACCESS_HIGHWAY type may
// be returned. If not, then these highways will have the generic HIGHWAY type.
...
// EnableFeatureNames sets the optional parameter "enableFeatureNames": Flag
// indicating whether human-readable names should be returned for features. If
// this is set, the display_name field on the feature will be filled out.
...
// EnableModeledVolumes sets the optional parameter "enableModeledVolumes":
...
// "enablePoliticalFeatures": Flag indicating whether political features should
// be returned.
...
// EnablePrivateRoads sets the optional parameter "enablePrivateRoads": Flag
// indicating whether the returned tile will contain road features that are
// marked private. Private roads are indicated by the
...
// the tile boundary. Buildings will only be returned on the tile that contains
// their centroid.
...
// LanguageCode sets the optional parameter "languageCode": Required. The
// BCP-47 language code corresponding to the language in which the name was
...
// RegionCode sets the optional parameter "regionCode": Required. The Unicode
// country/region code (CLDR) of the location from which the request is coming
...
// *FeatureTile.ServerResponse.Header or (if a response was returned at all) in
...
//
//   - name: Resource name of the tile. The tile resource name is prefixed by its
//     collection ID `terraintiles/` followed by the resource ID, which encodes
...
// and 1024 (and not more than max_elevation_resolution_cells). Zero is
...
// Non-uniformity is chosen to maximise the amount of preserved data. For
// example: Original resolution: 30px (width) * 10px (height)
...
// TerrainFormats sets the optional parameter "terrainFormats": Terrain formats
...
// Any non-2xx status code is an error. Response headers are in either
// *TerrainTile.ServerResponse.Header or (if a response was returned at all) in
// error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
