...
const basePathTemplate = "https://www.UNIVERSE_DOMAIN/fusiontables/v1/"
...
type Column struct {
	// BaseColumn: Optional identifier of the base column. If present, this column
	...
	// GraphPredicate: Optional column predicate. Used to map table to graph data
	// model (subject,predicate,object) See
...
// ColumnBaseColumn: Optional identifier of the base column. If present, this
...
type StyleSetting struct {
	...
	// style defintions for points, lines, and polygons in a table. Since a table
	// can have any one or all of them, a style definition can have point, line and
...
// Table: Represents a table. Specifies the name, whether it is exportable,
...
type Task struct {
	...
	// Type: Type of background task. One of  DELETE_ROWS Deletes one or more rows
...
type Template struct {
	...
	// either include an HTML body or a list of columns from which the template is
...
// Hdrs sets the optional parameter "hdrs": Should column names be included (in
// the first row)?. Default is true.
func (c *QuerySqlCall) Hdrs(hdrs bool) *QuerySqlCall {
	c.urlParams_.Set("hdrs", fmt.Sprint(hdrs))
	return c
}

// Typed sets the optional parameter "typed": Should typed values be returned
// in the (JSON) response -- numbers for numeric values and parsed geometries
...
// EndLine sets the optional parameter "endLine": The index of the last line
// from which to start importing, exclusive. Thus, the number of imported lines
// is endLine - startLine. If this parameter is not provided, the file will be
// imported until the last line of the file. If endLine is negative, then the
// imported content will exclude the last endLine lines. That is, if endline is
// negative, no line will be imported whose index is greater than N + endLine
// where N is the number of lines in the file, and the number of imported lines
// will be N + endLine - startLine.
func (c *TableImportRowsCall) EndLine(endLine int64) *TableImportRowsCall {
	c.urlParams_.Set("endLine", fmt.Sprint(endLine))
	return c
}

// IsStrict sets the optional parameter "isStrict": Whether the CSV must have
// the same number of values for each row. If false, rows with fewer values
