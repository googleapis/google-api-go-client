...
// `appengine.applications.get` Google IAM permission for the project
...
// routing is affected by dispatch files. Traffic is encrypted during
// transport and never leaves Google datacenters. Because this traffic
// is carried over a communication mechanism internal to Google, you
// cannot explicitly set the protocol (for example, HTTP or HTTPS). The
// request to the handler, however, will appear to have used the HTTP
// protocol. The AppEngineRouting used to construct the URL that the
// task is delivered to can be set at the queue-level or task-level: *
// If app_engine_routing_override is set on the queue, this value is
...
// with `login: admin`. Because tasks are not run as any user, they
// cannot be dispatched to URIs restricted with `login: required` Task
// dispatches also do not follow redirects. The task attempt has
...
type AppEngineHttpRequest struct {
	...
	// task; see request headers. These headers are set only when the task
	...
	// Writing a push task request handler and the App Engine documentation
	// for your runtime on How Requests are Handled.
	//
	// Possible values:
...
// of App Engine, Microservices Architecture on Google App Engine, App
// Engine Standard request routing, and App Engine Flex request routing.
// Using AppEngineRouting requires `appengine.applications.get` Google
...
type OAuthToken struct {
	...
	// the queue. The caller must have iam.serviceAccounts.actAs permission
...
type Queue struct {
	...
	// Console. Purge time will be truncated to the nearest microsecond.
	...
	// Stackdriver Logging. If this field is unset, then no logs are
	// written.
	...
	// queue.yaml/xml. UpdateQueue cannot be used to change `state`.
	//
	// Possible values:
...
type RateLimits struct {
	...
	// period of time. The token bucket algorithm is used to control the
	// rate of task dispatches. Each queue has a token bucket that holds
	// tokens, up to the maximum specified by `max_burst_size`. Each time a
	...
	// `max_burst_size` is equal to bucket_size. Since `max_burst_size` is
	// output only, if UpdateQueue is called on a queue created by
	// `queue.yaml/xml`, `max_burst_size` will be reset based on the value
	// of max_dispatches_per_second, regardless of whether
...
type StackdriverLoggingConfig struct {
	...
	// Stackdriver Logging. This field may contain any value between 0.0 and
	// 1.0, inclusive. 0.0 is the default and means that no operations are
	// logged.
...
type Task struct {
	...
	// default deadline depends on the scaling type of the service: 10
	// minutes for standard apps with automatic scaling, 24 hours for
	// standard apps with manual and basic scaling, and 60 minutes for flex
	...
	// than the service's timeout. We recommend setting the
	...
	// Identifying projects * `LOCATION_ID` is the canonical ID for the
	// task's location. The list of available locations can be obtained by
	// calling ListLocations. For more information, see
	// https://cloud.google.com/about/locations/. * `QUEUE_ID` can contain
	// letters ([A-Za-z]), numbers ([0-9]), or hyphens (-). The maximum
...
// your queues. Read Overview of Queue Management and queue.yaml before
// using this method.
...
// of Queue Management and queue.yaml before using this method.
...
// Authorization requires the following Google IAM permission on the
...
// Advanced Logs Filters. Sample filter "state: PAUSED". Note that using
// filters might cause fewer queues than the requested page_size to be
// returned.
...
// Management and queue.yaml before using this method.
...
// following Google IAM permission on the specified resource parent: *
...
// `cloudtasks.tasks.fullView` Google IAM permission on the Task
// resource.
//
// Possible values:
//   "VIEW_UNSPECIFIED" - Unspecified. Defaults to BASIC.
