...
const basePathTemplate = "https://www.UNIVERSE_DOMAIN/qpxExpress/v1/trips/"
...
type AircraftData struct {
	// Code: The aircraft code. For example, for a Boeing 777 the code would be
	// 777.
	Code string `json:"code,omitempty"`
	// Kind: Identifies this as an aircraftData object. Value: the fixed string
...
type AirportData struct {
	// City: The city code an airport is located in. For example, for JFK airport,
	// this is NYC.
	...
	Kind string `json:"kind,omitempty"`
	// Name: The name of an airport. For example, for airport BOS the name is
...
type BagDescriptor struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CommercialName") to include in
...
type CarrierData struct {
	// Code: The IATA designator of a carrier (airline, etc). For example, for
...
// CityData: Information about a city that might be useful to an end-user;
// typically the city of an airport.
type CityData struct {
	// Code: The IATA character ID of a city. For example, for Boston this is BOS.
	Code string `json:"code,omitempty"`
	// Country: The two-character country code of the country the city is located
	// in. For example, US for the United States of America.
	Country string `json:"country,omitempty"`
	// Kind: Identifies this as a city, typically with one or more airports. Value:
...
// Data: Detailed information about components found in the solutions of this
...
type Data struct {
	...
	// Carrier: The airline carrier of the aircraft flying between an origin and
	...
	// Kind: Identifies this as QPX Express response resource, including a trip's
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Aircraft") to include in API
...
// low-fare search query. In the airline industry a fare is a price an airline
// charges for one-way travel between two points. A fare typically contains a
// carrier code, two city codes, a price, and a fare basis. (A fare basis is a
...
type FareInfo struct {
	...
	// Carrier: The carrier of the aircraft or other vehicle commuting between two
	// points.
	...
	// Private: Whether this is a private fare, for example one offered only to
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BasisCode") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// FlightInfo: A flight is a sequence of legs with the same airline carrier and
// flight number. (A leg is the smallest unit of travel, in the case of a
// flight a takeoff immediately followed by a landing at two set points on a
// particular carrier with a particular flight number.) The naive view is that
// a flight is scheduled travel of an aircraft between two points, with
// possibly intermediate stops, but carriers will frequently list flights that
// require a change of aircraft between legs.
...
// FreeBaggageAllowance: Information about free baggage allowed on one segment
// of a trip.
type FreeBaggageAllowance struct {
	// BagDescriptor: A representation of a type of bag, such as an ATPCo subcode,
	...
	// KilosPerPiece: The maximum number of kilos any one piece of baggage may
	// weigh.
	KilosPerPiece int64 `json:"kilosPerPiece,omitempty"`
	// Kind: Identifies this as free baggage object, allowed on one segment of a
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BagDescriptor") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// LegInfo: Information about a leg. (A leg is the smallest unit of travel, in
...
type LegInfo struct {
	// Aircraft: The aircraft (or bus, ferry, railcar, etc) travelling between the
	// two points of this leg.
	Aircraft string `json:"aircraft,omitempty"`
	// ArrivalTime: The scheduled time of arrival at the destination of the leg,
	// local to the point of arrival.
	ArrivalTime string `json:"arrivalTime,omitempty"`
	// ChangePlane: Whether you have to change planes following this leg. Only
	// applies to the next leg.
	...
	// DepartureTime: The scheduled departure time of the leg, local to the point
	// of departure.
	...
	Id string `json:"id,omitempty"`
	// Kind: Identifies this as a leg object. A leg is the smallest unit of travel,
	// in the case of a flight a takeoff immediately followed by a landing at two
	// set points on a particular carrier with a particular flight number. Value:
	// the fixed string qpxexpress#legInfo.
	Kind string `json:"kind,omitempty"`
	// Meal: A simple, general description of the meal(s) served on the flight, for
	// example: "Hot meal".
	Meal string `json:"meal,omitempty"`
	// Mileage: The number of miles in this leg.
	Mileage int64 `json:"mileage,omitempty"`
	// OnTimePerformance: In percent, the published on time performance on this
	// leg.
	OnTimePerformance int64 `json:"onTimePerformance,omitempty"`
	// OperatingDisclosure: Department of Transportation disclosure information on
	// the actual operator of a flight in a code share. (A code share refers to a
	// marketing agreement between two carriers, where one carrier will list in its
	// schedules (and take bookings for) flights that are actually operated by
	// another carrier.)
	...
	// Secure: Whether passenger information must be furnished to the United States
...
// definition of an infant, child, adult, and senior citizen varies across
// carriers and reservation systems.
type PassengerCounts struct {
	...
	// InfantInLapCount: The number of passengers that are infants travelling in
	// the lap of an adult.
	InfantInLapCount int64 `json:"infantInLapCount,omitempty"`
	// InfantInSeatCount: The number of passengers that are infants each assigned a
	// seat.
	InfantInSeatCount int64 `json:"infantInSeatCount,omitempty"`
	// Kind: Identifies this as a passenger count object, representing the number
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AdultCount") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// PricingInfo: The price of one or more travel segments. The currency used to
// purchase tickets is usually determined by the sale/ticketing city or the
// sale/ticketing country, unless none are specified, in which case it defaults
// to that of the journey origin country.
type PricingInfo struct {
	// BaseFareTotal: The total fare in the base fare currency (the currency of the
	// country of origin). This element is only present when the sales currency and
	...
	// FareCalculation: The horizontal fare calculation. This is a field on a
	// ticket that displays all of the relevant items that go into the calculation
	// of the fare.
	FareCalculation string `json:"fareCalculation,omitempty"`
	// Kind: Identifies this as a pricing object, representing the price of one or
	// more travel segments. Value: the fixed string qpxexpress#pricingInfo.
	Kind string `json:"kind,omitempty"`
	// LatestTicketingTime: The latest ticketing time for this pricing assuming the
	// reservation occurs at ticketing time and there is no change in fares/rules.
	// The time is local to the point of sale (POS).
	...
	// Ptc: The passenger type code for this pricing. An alphanumeric code used by
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BaseFareTotal") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// SegmentInfo: Details of a segment of a flight; a segment is one or more
// consecutive legs on the same flight. For example a hypothetical flight
// ZZ001, from DFW to OGG, would have one segment with two legs: DFW to HNL
// (leg 1), HNL to OGG (leg 2), and DFW to OGG (legs 1 and 2).
type SegmentInfo struct {
	// BookingCode: The booking code or class for this segment.
	BookingCode string `json:"bookingCode,omitempty"`
	// BookingCodeCount: The number of seats available in this booking code on this
	// segment.
	BookingCodeCount int64 `json:"bookingCodeCount,omitempty"`
	// Cabin: The cabin booked for this segment.
	Cabin string `json:"cabin,omitempty"`
	// ConnectionDuration: In minutes, the duration of the connection following
	// this segment.
	ConnectionDuration int64 `json:"connectionDuration,omitempty"`
	...
	// (leg 1), HNL to OGG (leg 2). Value: the fixed string qpxexpress#segmentInfo.
	Kind string `json:"kind,omitempty"`
	// Leg: The legs composing this segment.
	Leg []*LegInfo `json:"leg,omitempty"`
	// MarriedSegmentGroup: The solution-based index of a segment in a married
	// segment group. Married segments can only be booked together. For example, an
	// airline might report a certain booking code as sold out from Boston to
	// Pittsburgh, but as available as part of two married segments Boston to
	// Chicago connecting through Pittsburgh. For example content of this field,
	// consider the round-trip flight ZZ1 PHX-PHL ZZ2 PHL-CLT ZZ3 CLT-PHX. This has
	// three segments, with the two outbound ones (ZZ1 ZZ2) married. In this case,
	// the two outbound segments belong to married segment group 0, and the return
	// segment belongs to married segment group 1.
	MarriedSegmentGroup string `json:"marriedSegmentGroup,omitempty"`
	// SubjectToGovernmentApproval: Whether the operation of this segment remains
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BookingCode") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SegmentPricing struct {
	// FareId: A segment identifier unique within a single solution. It is used to
	...
	// Kind: Identifies this as a segment pricing object, representing the price of
	...
	// ForceSendFields is a list of field names (e.g. "FareId") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
// SliceInfo: Information about a slice. A slice represents a traveller's
...
// using 1 slice, round-trips using 2. For example, if a traveler specifies the
...
type SliceInfo struct {
	...
	Duration int64 `json:"duration,omitempty"`
	// Kind: Identifies this as a slice object. A slice represents a traveller's
	// intent, the portion of a low-fare search corresponding to a traveler's
	// request to get between two points. One-way journeys are generally expressed
	// using 1 slice, round-trips using 2. Value: the fixed string
...
type SliceInput struct {
	// Alliance: Slices with only the carriers in this alliance should be returned;
	// do not use this field with permittedCarrier. Allowed values are ONEWORLD,
	// SKYTEAM, and STAR.
	...
	Destination string `json:"destination,omitempty"`
	// Kind: Identifies this as a slice input object, representing the criteria a
	// desired slice must satisfy. Value: the fixed string qpxexpress#sliceInput.
	Kind string `json:"kind,omitempty"`
	// MaxConnectionDuration: The longest connection between two legs, in minutes,
	// you are willing to accept.
	MaxConnectionDuration int64 `json:"maxConnectionDuration,omitempty"`
	// MaxStops: The maximum number of stops you are willing to accept in this
	// slice.
	MaxStops int64 `json:"maxStops,omitempty"`
	// Origin: Airport or city IATA designator of the origin.
	Origin string `json:"origin,omitempty"`
	// PermittedCarrier: A list of 2-letter IATA airline designators. Slices with
	// only these carriers should be returned.
	PermittedCarrier []string `json:"permittedCarrier,omitempty"`
	// PermittedDepartureTime: Slices must depart in this time of day range, local
	// to the point of departure.
	PermittedDepartureTime *TimeOfDayRange `json:"permittedDepartureTime,omitempty"`
	// PreferredCabin: Prefer solutions that book in this cabin for this slice.
	// Allowed values are COACH, PREMIUM_COACH, BUSINESS, and FIRST.
	PreferredCabin string `json:"preferredCabin,omitempty"`
	// ProhibitedCarrier: A list of 2-letter IATA airline designators. Exclude
	// slices that use these carriers.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Alliance") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type TaxData struct {
	...
	Id string `json:"id,omitempty"`
	// Kind: Identifies this as a tax data object, representing some tax. Value:
...
type TaxInfo struct {
	...
	Country string `json:"country,omitempty"`
	// Id: Identifier uniquely identifying this tax in a response. Not present for
	// unnamed carrier surcharges.
	Id string `json:"id,omitempty"`
	// Kind: Identifies this as a tax information object. Value: the fixed string
	// qpxexpress#taxInfo.
	Kind string `json:"kind,omitempty"`
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ChargeType") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type TimeOfDayRange struct {
	...
	// Kind: Identifies this as a time of day range object, representing two times
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "EarliestTime") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type TripOption struct {
	...
	Id string `json:"id,omitempty"`
	// Kind: Identifies this as a trip information object. Value: the fixed string
	// qpxexpress#tripOption.
	Kind string `json:"kind,omitempty"`
	// Pricing: Per passenger pricing information.
	Pricing []*PricingInfo `json:"pricing,omitempty"`
	// SaleTotal: The total price for all passengers on the trip, in the form of a
...
// TripOptionsRequest: A QPX Express search request, which will yield one or
// more solutions.
type TripOptionsRequest struct {
	...
	// corresponding to a traveler's request to get between two points. One-way
	// journeys are generally expressed using one slice, round-trips using two. An
	// example of a one slice trip with three segments might be BOS-SYD, SYD-LAX,
	...
	// TicketingCountry: IATA country code representing the point of ticketing.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "MaxPrice") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type TripOptionsResponse struct {
	...
	// Kind: Identifies this as a QPX Express trip response object, which consists
	// of zero or more solutions. Value: the fixed string qpxexpress#tripOptions.
	Kind string `json:"kind,omitempty"`
...
type TripsSearchResponse struct {
	// Kind: Identifies this as a QPX Express API search response resource. Value:
...
// *TripsSearchResponse.ServerResponse.Header or (if a response was returned at
