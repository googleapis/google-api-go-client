...
type TrainedmodelsServicer interface {
	Delete(id string) *TrainedmodelsDeleteCall
	Get(id string) *TrainedmodelsGetCall
	Insert(training *Training) *TrainedmodelsInsertCall
	Predict(id string, input *Input) *TrainedmodelsPredictCall
	Update(id string, update *Update) *TrainedmodelsUpdateCall
}
