...
// Done is returned by an iterator's Next method to indicate that the iterator
// has no more items.
...
type PageInfo struct {
	...
	// Set to true on first call to PageInfo.next or Pager.NextPage.
	nextCalled, nextPageCalled bool
}

// NewPageInfo is for use only by iterator implementations.
func NewPageInfo(fetch func(int, string) (string, error), bufLen func() int, takeBuf func() interface{}) (*PageInfo, func(error) (error, bool)) {
	// fetch, bufLen and takeBuf are as described in PageInfo, above.
	// The return value is the PageInfo.next method bound to the returned PageInfo value.
	// (Doing it this way avoids exporting PageInfo.next.)
	pi := &PageInfo{
		fetch:   fetch,
		bufLen:  bufLen,
		takeBuf: takeBuf,
...
// Ensure there is at least one element in the buffer.
// Take the iterator's error state on input, and return the new error state on output.
// Also report whether the buffer has at least one element in it.
