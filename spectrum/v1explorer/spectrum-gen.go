...
const basePathTemplate = "https://www.UNIVERSE_DOMAIN/spectrum/v1explorer/paws/"
...
// information, such as the antenna height, antenna type, etc. Whether antenna
// characteristics must be provided in a request depends on the device type and
// regulatory domain.
type AntennaCharacteristics struct {
	// Height: The antenna height in meters. Whether the antenna height is required
	// depends on the device type and the regulatory domain. Note that the height
	// may be negative.
	Height float64 `json:"height,omitempty"`
	// HeightType: If the height is required, then the height type (AGL for above
	// ground level or AMSL for above mean sea level) is also required. The default
	// is AGL.
...
// DbUpdateSpec: This message is provided by the database to notify devices of
// an upcoming change to the database URI.
type DbUpdateSpec struct {
	// Databases: A required list of one or more databases. A device should update
	// its preconfigured list of databases to replace (only) the database that
...
// DeviceCapabilities: Device capabilities provide additional information that
// may be used by a device to provide additional information to the database
// that may help it to determine available spectrum. If the database does not
// support device capabilities it will ignore the parameter altogether.
type DeviceCapabilities struct {
	// FrequencyRanges: An optional list of frequency ranges supported by the
	// device. Each element must contain start and stop frequencies in which the
	// device can operate. Channel identifiers are optional. When specified, the
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "FrequencyRanges") to include in
...
// DeviceDescriptor: The device descriptor contains parameters that identify
...
type DeviceDescriptor struct {
	// EtsiEnDeviceCategory: Specifies the ETSI white space device category. Valid
	// values are the strings master and slave. This field is case-insensitive.
	...
	// EtsiEnDeviceEmissionsClass: Specifies the ETSI white space device emissions
	// class. The values are represented by numeric strings, such as 1, 2, etc.
	// Consult the ETSI documentation for details about the device types.
	EtsiEnDeviceEmissionsClass string `json:"etsiEnDeviceEmissionsClass,omitempty"`
	// EtsiEnDeviceType: Specifies the ETSI white space device type. Valid values
	// are single-letter strings, such as A, B, etc. Consult the ETSI documentation
	...
	// FccId: Specifies the device's FCC certification identifier. The value is an
	...
	// FccTvbdDeviceType: Specifies the TV Band White Space device type, as defined
	// by the FCC. Valid values are FIXED, MODE_1, MODE_2.
	FccTvbdDeviceType string `json:"fccTvbdDeviceType,omitempty"`
	// ManufacturerId: The manufacturer's ID may be required by the regulatory
	// domain. This should represent the name of the device manufacturer, should be
	// consistent across all devices from the same manufacturer, and should be
	// distinct from that of other manufacturers. The string value must not exceed
	// 64 characters in length.
	ManufacturerId string `json:"manufacturerId,omitempty"`
	// ModelId: The device's model ID may be required by the regulatory domain. The
	// string value must not exceed 64 characters in length.
	ModelId string `json:"modelId,omitempty"`
	// RulesetIds: The list of identifiers for rulesets supported by the device. A
	// database may require that the device provide this list before servicing the
	...
	// SerialNumber: The manufacturer's device serial number; required by the
	// applicable regulatory domain. The length of the value must not exceed 64
	// characters.
	SerialNumber string `json:"serialNumber,omitempty"`
	// ForceSendFields is a list of field names (e.g. "EtsiEnDeviceCategory") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "EtsiEnDeviceCategory") to include
...
// DeviceOwner: This parameter contains device-owner information required as
// part of device registration. The regulatory domains may require additional
// parameters.
//
...
type DeviceOwner struct {
	// Operator: The vCard contact information for the device operator is optional,
	// but may be required by specific regulatory domains.
	Operator *Vcard `json:"operator,omitempty"`
	// Owner: The vCard contact information for the individual or business that
	// owns the device is required.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Operator") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// DeviceValidity: The device validity element describes whether a particular
// device is valid to operate in the regulatory domain.
type DeviceValidity struct {
	// DeviceDesc: The descriptor of the device for which the validity check was
	// requested. It will always be present.
	DeviceDesc *DeviceDescriptor `json:"deviceDesc,omitempty"`
	// IsValid: The validity status: true if the device is valid for operation,
	// false otherwise. It will always be present.
	IsValid bool `json:"isValid,omitempty"`
	// Reason: If the device identifier is not valid, the database may include a
	// reason. The reason may be in any language. The length of the value should
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DeviceDesc") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// EventTime: The start and stop times of an event. This is used to indicate
// the time period for which a spectrum profile is valid.
//
...
// FrequencyRange: A specific range of frequencies together with the associated
// maximum power level and channel identifier.
type FrequencyRange struct {
	// ChannelId: The database may include a channel identifier, when applicable.
	// When it is included, the device should treat it as informative. The length
	...
	// corresponding operating bandwidth—that is permitted within the frequency
	// range. Depending on the context in which the frequency-range element
	// appears, this value may be required. For example, it is required in the
	// available-spectrum response, available-spectrum-batch response, and
	// spectrum-use notification message, but it should not be present (it is not
	// applicable) when the frequency range appears inside a device-capabilities
	// message.
	MaxPowerDBm float64 `json:"maxPowerDBm,omitempty"`
	// StartHz: The required inclusive start of the frequency range (in Hertz).
...
type GeoLocation struct {
	// Confidence: The location confidence level, as an integer percentage, may be
	// required, depending on the regulatory domain. When the parameter is optional
	// and not provided, its value is assumed to be 95. Valid values range from 0
	// to 99, since, in practice, 100-percent confidence is not achievable. The
	...
	// Region: If present, indicates that the geolocation represents a region.
...
// GeoLocationEllipse: A "point" with uncertainty is represented using the
// Ellipse shape.
type GeoLocationEllipse struct {
	// Center: A required geo-spatial point representing the center of the ellipse.
	Center *GeoLocationPoint `json:"center,omitempty"`
	// Orientation: A floating-point number that expresses the orientation of the
	// ellipse, representing the rotation, in degrees, of the semi-major axis from
	// North towards the East. For example, when the uncertainty is greatest along
	...
	// uncertainty along the major axis of the ellipse. May be required by the
	// regulatory domain. When the uncertainty is optional, the default value is 0.
	SemiMajorAxis float64 `json:"semiMajorAxis,omitempty"`
	// SemiMinorAxis: A floating-point number that expresses the location
	// uncertainty along the minor axis of the ellipse. May be required by the
	// regulatory domain. When the uncertainty is optional, the default value is 0.
...
type GeoLocationPoint struct {
	// Latitude: A required floating-point number that expresses the latitude in
	...
	// Longitude: A required floating-point number that expresses the longitude in
	// degrees using the WGS84 datum. For details on this encoding, see the
	// National Imagery and Mapping Agency's Technical Report TR8350.2.
...
type GeoLocationPolygon struct {
	// Exterior: When the geolocation describes a region, the exterior field refers
	// to a list of latitude/longitude points that represent the vertices of a
	// polygon. The first and last points must be the same. Thus, a minimum of four
	// points is required. The following polygon restrictions from RFC5491 apply:
	//
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Exterior") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GeoSpectrumSchedule struct {
	// Location: The geolocation identifies the location at which the spectrum
	// schedule applies. It will always be present.
	Location *GeoLocation `json:"location,omitempty"`
	// SpectrumSchedules: A list of available spectrum profiles and associated
	// times. It will always be present, and at least one schedule must be included
	// (though it may be empty if there is no available spectrum). More than one
...
// PawsGetSpectrumBatchRequest: The request message for a batch available
// spectrum query protocol.
type PawsGetSpectrumBatchRequest struct {
	...
	// Capabilities: The master device may include its device capabilities to limit
	// the available-spectrum batch response to the spectrum that is compatible
	...
	// cases, the requestType parameter must be empty). When a requestType value is
	...
	// its current location plus additional anticipated locations when allowed by
	// the regulatory domain. At least one location must be included. Geolocation
	// must be given as the location of the radiation center of the device's
	// antenna. If a location specifies a region, rather than a point, the database
	// may return an UNIMPLEMENTED error if it does not support query by
	// region.
	//
	...
	// allows the device to register and get spectrum-availability information in a
	// single request.
	Owner *DeviceOwner `json:"owner,omitempty"`
	// RequestType: The request type parameter is an optional parameter that can be
	// used to modify an available spectrum batch request, but its use depends on
	...
	// ForceSendFields is a list of field names (e.g. "Antenna") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Antenna") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// PawsGetSpectrumBatchResponse: The response message for the batch available
// spectrum query contains a schedule of available spectrum for the device at
// multiple locations.
type PawsGetSpectrumBatchResponse struct {
	// DatabaseChange: A database may include the databaseChange parameter to
	// notify a device of a change to its database URI, providing one or more
	// alternate database URIs. The device should use this information to update
	// its list of pre-configured databases by (only) replacing its entry for the
	// responding database with the list of alternate URIs.
	DatabaseChange *DbUpdateSpec `json:"databaseChange,omitempty"`
	// DeviceDesc: The database must return in its available spectrum response the
	// device descriptor information it received in the master device's available
	// spectrum batch request.
	DeviceDesc *DeviceDescriptor `json:"deviceDesc,omitempty"`
	// GeoSpectrumSchedules: The available spectrum batch response must contain a
	// geo-spectrum schedule list, The list may be empty if spectrum is not
	// available. The database may return more than one geo-spectrum schedule to
	// represent future changes to the available spectrum. How far in advance a
	// schedule may be provided depends upon the applicable regulatory domain. The
	...
	// entries in the list, and must use the geolocation value in each geo-spectrum
	// schedule entry to match available spectrum to a location.
	...
	// MaxContiguousBwHz: The database may return a constraint on the allowed
	// maximum contiguous bandwidth (in Hertz). A regulatory domain may require the
	// database to return this parameter. When this parameter is present in the
	// response, the device must apply this constraint to its spectrum-selection
	// logic to ensure that no single block of spectrum has bandwidth that exceeds
	// this value.
	MaxContiguousBwHz float64 `json:"maxContiguousBwHz,omitempty"`
	// MaxTotalBwHz: The database may return a constraint on the allowed maximum
	// total bandwidth (in Hertz), which does not need to be contiguous. A
	// regulatory domain may require the database to return this parameter. When
	// this parameter is present in the available spectrum batch response, the
	...
	// NeedsSpectrumReport: For regulatory domains that require a spectrum-usage
	// report from devices, the database must return true for this parameter if the
	// geo-spectrum schedules list is not empty; otherwise, the database should
	// either return false or omit this parameter. If this parameter is present and
	// its value is true, the device must send a spectrum use notify message to the
	...
	// identifies the applicable regulatory authority and ruleset for the available
	// spectrum batch response. If included, the device must use the corresponding
	// ruleset to interpret the response. Values provided in the returned ruleset
	// information, such as maxLocationChange, take precedence over any conflicting
	// values provided in the ruleset information returned in a prior
	...
	// available spectrum batch response. The timestamp should be used by the
	// device as a reference for the start and stop times specified in the response
	// spectrum schedules.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DatabaseChange") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// PawsGetSpectrumRequest: The request message for the available spectrum query
// protocol which must include the device's geolocation.
type PawsGetSpectrumRequest struct {
	// Antenna: Depending on device type and regulatory domain, the characteristics
	// of the antenna may be required.
	Antenna *AntennaCharacteristics `json:"antenna,omitempty"`
	// Capabilities: The master device may include its device capabilities to limit
	...
	// DeviceDesc: When the available spectrum request is made on behalf of a
	// specific device (a master or slave device), device descriptor information
	// for that device is required (in such cases, the requestType parameter must
	...
	// capability that makes an available spectrum request) is required whether the
	// master device is making the request on its own behalf or on behalf of a
	// slave device (one without geolocation capability). The location must be the
	...
	// device (a device with geolocation capability) on behalf of a slave device (a
	...
	// descriptor information (in addition to device descriptor information for the
	// slave device, which is provided in a separate parameter).
	MasterDeviceDesc *DeviceDescriptor `json:"masterDeviceDesc,omitempty"`
	// Owner: Depending on device type and regulatory domain, device owner
	// information may be included in an available spectrum request. This allows
	// the device to register and get spectrum-availability information in a single
	// request.
	Owner *DeviceOwner `json:"owner,omitempty"`
	// RequestType: The request type parameter is an optional parameter that can be
	// used to modify an available spectrum request, but its use depends on
	// applicable regulatory rules. It may be used, for example, to request generic
	// slave device parameters without having to specify the device descriptor for
	// a specific device. When the requestType parameter is missing, the request is
...
// PawsGetSpectrumResponse: The response message for the available spectrum
// query which contains a schedule of available spectrum for the device.
type PawsGetSpectrumResponse struct {
	...
	// responding database with the list of alternate URIs.
	DatabaseChange *DbUpdateSpec `json:"databaseChange,omitempty"`
	// DeviceDesc: The database must return, in its available spectrum response,
	// the device descriptor information it received in the master device's
	...
	MaxContiguousBwHz float64 `json:"maxContiguousBwHz,omitempty"`
	// MaxTotalBwHz: The database may return a constraint on the allowed maximum
	// total bandwidth (in Hertz), which need not be contiguous. A regulatory
	// domain may require the database to return this parameter. When this
	// parameter is present in the available spectrum response, the device must
	// apply this constraint to its spectrum-selection logic to ensure that total
	// bandwidth does not exceed this value.
	MaxTotalBwHz float64 `json:"maxTotalBwHz,omitempty"`
	// NeedsSpectrumReport: For regulatory domains that require a spectrum-usage
	// report from devices, the database must return true for this parameter if the
	// spectrum schedule list is not empty; otherwise, the database will either
	// return false or omit this parameter. If this parameter is present and its
	// value is true, the device must send a spectrum use notify message to the
	// database; otherwise, the device must not send the notification.
	NeedsSpectrumReport bool `json:"needsSpectrumReport,omitempty"`
	// RulesetInfo: The database should return ruleset information, which
	// identifies the applicable regulatory authority and ruleset for the available
	// spectrum response. If included, the device must use the corresponding
	// ruleset to interpret the response. Values provided in the returned ruleset
	// information, such as maxLocationChange, take precedence over any conflicting
	// values provided in the ruleset information returned in a prior
	// initialization response sent by the database to the device.
	RulesetInfo *RulesetInfo `json:"rulesetInfo,omitempty"`
	// SpectrumSchedules: The available spectrum response must contain a spectrum
	// schedule list. The list may be empty if spectrum is not available. The
	// database may return more than one spectrum schedule to represent future
	// changes to the available spectrum. How far in advance a schedule may be
	// provided depends on the applicable regulatory domain.
	SpectrumSchedules []*SpectrumSchedule `json:"spectrumSchedules,omitempty"`
	// Timestamp: The database includes a timestamp of the form
	// YYYY-MM-DDThh:mm:ssZ (Internet timestamp format per RFC3339) in its
	// available spectrum response. The timestamp should be used by the device as a
	// reference for the start and stop times specified in the response spectrum
	// schedules.
	Timestamp string `json:"timestamp,omitempty"`
	// Type: The message type (e.g., INIT_REQ, AVAIL_SPECTRUM_REQ, ...).
	//
...
// PawsInitRequest: The initialization request message allows the master device
// to initiate exchange of capabilities with the database.
type PawsInitRequest struct {
	// DeviceDesc: The DeviceDescriptor parameter is required. If the database does
	// not support the device or any of the rulesets specified in the device
...
// PawsInitResponse: The initialization response message communicates database
// parameters to the requesting device.
type PawsInitResponse struct {
	...
	Kind string `json:"kind,omitempty"`
	// RulesetInfo: The rulesetInfo parameter must be included in the response.
	// This parameter specifies the regulatory domain and parameters applicable to
...
// PawsNotifySpectrumUseRequest: The spectrum-use notification message which
// must contain the geolocation of the Device and parameters required by the
// regulatory domain.
type PawsNotifySpectrumUseRequest struct {
	...
	// list specifies the spectrum that the device expects to use, which includes
...
// PawsRegisterRequest: The registration request message contains the required
...
// PawsRegisterResponse: The registration response message simply acknowledges
// receipt of the request and is otherwise empty.
...
type PawsVerifyDeviceRequest struct {
	// DeviceDescs: A list of device descriptors, which specifies the slave devices
	// to be validated, is required.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DeviceDescs") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type PawsVerifyDeviceResponse struct {
	...
	// validation response to report whether each slave device listed in a previous
	// device validation request is valid. The number of entries must match the
...
// RulesetInfo: This contains parameters for the ruleset of a regulatory domain
// that is communicated using the initialization and available-spectrum
// processes.
type RulesetInfo struct {
	// Authority: The regulatory domain to which the ruleset belongs is required.
	// It must be a 2-letter country code. The device should use this to determine
	// additional device behavior required by the associated regulatory domain.
	Authority string `json:"authority,omitempty"`
	// MaxLocationChange: The maximum location change in meters is required in the
	// initialization response, but optional otherwise. When the device changes
	// location by more than this specified distance, it must contact the database
	// to get the available spectrum for the new location. If the device is using
	// spectrum that is no longer available, it must immediately cease use of the
	// spectrum under rules for database-managed spectrum. If this value is
	...
	// MaxPollingSecs: The maximum duration, in seconds, between requests for
	// available spectrum. It is required in the initialization response, but
	// optional otherwise. The device must contact the database to get available
	// spectrum no less frequently than this duration. If the new spectrum
	// information indicates that the device is using spectrum that is no longer
	// available, it must immediately cease use of those frequencies under rules
	// for database-managed spectrum. If this value is provided within the context
	// of an available-spectrum response, it takes precedence over the value within
	// the initialization response.
	MaxPollingSecs int64 `json:"maxPollingSecs,omitempty"`
	// RulesetIds: The identifiers of the rulesets supported for the device's
	// location. The database should include at least one applicable ruleset in the
...
// SpectrumMessage: Available spectrum can be logically characterized by a list
// of frequency ranges and permissible power levels for each range.
type SpectrumMessage struct {
	// Bandwidth: The bandwidth (in Hertz) for which permissible power levels are
	...
	// specifications, at 0.1MHz and 8MHz. This parameter may be empty if there is
	// no available spectrum. It will be present otherwise.
	Bandwidth float64 `json:"bandwidth,omitempty"`
	// FrequencyRanges: The list of frequency ranges and permissible power levels.
	// The list may be empty if there is no available spectrum, otherwise it will
	// be present.
	FrequencyRanges []*FrequencyRange `json:"frequencyRanges,omitempty"`
...
// SpectrumSchedule: The spectrum schedule element combines an event time with
// spectrum profile to define a time period in which the profile is valid.
type SpectrumSchedule struct {
	// EventTime: The event time expresses when the spectrum profile is valid. It
	// will always be present.
	EventTime *EventTime `json:"eventTime,omitempty"`
	// Spectra: A list of spectrum messages representing the usable profile. It
	// will always be present, but may be empty when there is no available
	// spectrum.
	Spectra []*SpectrumMessage `json:"spectra,omitempty"`
...
// Vcard: A vCard-in-JSON message that contains only the fields needed for
// PAWS:
// - fn: Full name of an individual
// - org: Name of the organization
// - adr: Address fields
...
type Vcard struct {
	...
	// ForceSendFields is a list of field names (e.g. "Adr") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Adr") to include in API requests
	// with the JSON null value. By default, fields with empty values are omitted
	// from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type VcardAddress struct {
	// Code: The postal code associated with the address. For example: 94423.
	Code string `json:"code,omitempty"`
	// Country: The country name. For example: US.
	Country string `json:"country,omitempty"`
	// Locality: The city or local equivalent portion of the address. For example:
	// San Jose.
	Locality string `json:"locality,omitempty"`
	// Pobox: An optional post office box number.
	Pobox string `json:"pobox,omitempty"`
	// Region: The state or local equivalent portion of the address. For example:
	// CA.
...
// VcardTypedText: The structure used to represent an organization and an email
// address.
type VcardTypedText struct {
	// Text: The text string associated with this item. For example, for an org
...
// GetSpectrum: Requests information about the available spectrum for a device
// at a location. Requests from a fixed-mode device must include owner
...
// Any non-2xx status code is an error. Response headers are in either
// *PawsGetSpectrumBatchResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *PawsInitResponse.ServerResponse.Header or (if a response was returned at
...
// certain frequency ranges for transmission. Only to be invoked when required
// by the regulator. The Google Spectrum Database does not operate in domains
...
// Any non-2xx status code is an error. Response headers are in either
// *PawsNotifySpectrumUseResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *PawsRegisterResponse.ServerResponse.Header or (if a response was returned
...
// VerifyDevice: Validates a device for white space use in accordance with
// regulatory rules. The Google Spectrum Database does not support master/slave
