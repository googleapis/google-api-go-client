...
func (a *API) GenerateCode() ([]byte, error) {
	...
	pn(` elts := []string{"gl-go/" + strings.Replace(runtime.Version(), " ", "_")}`)
	pn(` if s.GoogleClientHeaderElement != "" {`)
	pn(`   elts = append(elts, s.GoogleClientHeaderElement)`)
	pn(` }`)
	pn(`   elts = append(elts, "gdcl/%s")`, generatorVersion)
	pn(` return strings.Join(elts, " ")`)
	pn("}\n")
