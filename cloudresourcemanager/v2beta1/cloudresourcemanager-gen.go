...
type Folder struct {
	...
	// unique amongst its siblings, e.g. no two folders with the same parent can
	...
	// underscores and can be no longer than 30 characters. This is captured by the
	// regular expression: `[\p{L}\p{N}]([\p{L}\p{N}_- ]{0,28}[\p{L}\p{N}])?`.
	DisplayName string `json:"displayName,omitempty"`
	// LifecycleState: Output only. The lifecycle state of the folder. Updates to
	// the lifecycle_state must be performed via DeleteFolder and UndeleteFolder.
	//
	// Possible values:
...
type MoveFolderRequest struct {
	// DestinationParent: Required. The resource name of the Folder or Organization
	// to reparent the folder under. Must be of the form `folders/{folder_id}` or
...
type SearchFoldersRequest struct {
	// PageSize: Optional. The maximum number of folders to return in the response.
	...
	// whitespace to prevent unexpected behavior. Some example queries are: * Query
...
// Create: Creates a Folder in the resource hierarchy. Returns an Operation
// which can be used to track the progress of the folder creation workflow.
// Upon success the Operation.response field will be populated with the created
...
// display_name must be distinct from all other Folders that share its parent.
// + The addition of the Folder must not cause the active Folder hierarchy to
// exceed a height of 10. Note, the full active + deleted Folder hierarchy is
// allowed to reach a height of 20; this provides additional headroom when
// moving folders that contain deleted folders. + The addition of the Folder
// must not cause the total number of Folders under its parent to exceed 300.
// If the operation fails due to a folder constraint violation, some errors may
// be returned by the CreateFolder request, with status code
...
// Parent sets the optional parameter "parent": Required. The resource name of
// the new Folder's parent. Must be of the form `folders/{folder_id}` or
// `organizations/{org_id}`.
...
// state, where a Folder is empty if it doesn't contain any Folders or Projects
// in the ACTIVE state. The caller must have `resourcemanager.folders.delete`
// permission on the identified folder.
//
//   - name: the resource name of the Folder to be deleted. Must be of the form
//     `folders/{folder_id}`.
...
// Get: Retrieves a Folder identified by the supplied resource name. Valid
...
// permission on the identified folder.
//
//   - name: The resource name of the Folder to retrieve. Must be of the form
//     `folders/{folder_id}`.
...
// GetIamPolicy: Gets the access control policy for a Folder. The returned
// policy may be empty if no such policy or resource exists. The `resource`
// field should be the Folder's resource name, e.g. "folders/1234". The caller
...
// List: Lists the Folders that are direct descendants of supplied parent
// resource. List provides a strongly consistent view of the Folders underneath
// the specified parent resource. List returns Folders sorted based upon the
...
// Folders to return in the response. The server can return fewer folders than
...
// returned from a previous call to `ListFolders` that indicates where this
...
// Parent sets the optional parameter "parent": Required. The resource name of
// the Organization or Folder whose Folders are being listed. Must be of the
// form `folders/{folder_id}` or `organizations/{org_id}`. Access to this
...
// Folders in the DELETE_REQUESTED state should be returned. Defaults to false.
...
// can be used to track the progress of the folder move workflow. Upon success
// the Operation.response field will be populated with the moved Folder. Upon
...
// Patch: Updates a Folder, changing its display_name. Changes to the folder
...
// UpdateMask sets the optional parameter "updateMask": Required. Fields to be
// updated. Only the `display_name` can be updated.
...
// Search: Search for folders that match specific filter criteria. Search
...
func (c *FoldersSearchCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.searchfoldersrequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	req.Header = reqHeaders
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "cloudresourcemanager.folders.search", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "cloudresourcemanager.folders.search" call.
// Any non-2xx status code is an error. Response headers are in either
// *SearchFoldersResponse.ServerResponse.Header or (if a response was returned
...
func (c *FoldersSearchCall) Pages(ctx context.Context, f func(*SearchFoldersResponse) error) error {
	c.ctx_ = ctx
	defer func(pt string) { c.searchfoldersrequest.PageToken = pt }(c.searchfoldersrequest.PageToken)
	for {
		x, err := c.Do()
		if err != nil {
			return err
...
// SetIamPolicy: Sets the access control policy on a Folder, replacing any
// existing policy. The `resource` field should be the Folder's resource name,
...
// Folder. The `resource` field should be the Folder's resource name, e.g.
...
// Undelete: Cancels the deletion request for a Folder. This method may only be
...
//
//   - name: The resource name of the Folder to undelete. Must be of the form
//     `folders/{folder_id}`.
