...
//
// When content size is not explicitly specified via an UploadOption and the supplied reader
// implements Stat() (os.FileInfo, error) method, content size is guessed as Stat().Size().
//
// When supplied reader implements io.ReaderAt and content size is known, ResumableUpload
// is configured to avoid buffering content. Chunking is also avoided under those conditions,
// unless it is explicitly requested via an UploadOption.
//
...
func (rx *ResumableUpload) Configure(r io.Reader, opts ...UploadOption) string {
	...
	if file, ok := r.(statter); ok && opt.contentSize <= 0 {
		if fileinfo, err := file.Stat(); err == nil {
			opt.contentSize = fileinfo.Size()
		}
	}
	if readerAt, ok := r.(io.ReaderAt); ok && opt.contentSize > 0 {
