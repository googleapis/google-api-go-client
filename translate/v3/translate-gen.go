...
type Glossary struct {
	...
	// Name: Identifier. The resource name of the glossary. Glossary names have the
	// form
...
type RomanizeTextRequest struct {
	...
	// known, for example, "hi" or "zh". Supported language codes are listed in
...
func (c *ProjectsLocationsRefineTextCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v3/{+parent}:refineText")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
...
// be updated. Currently, only `display_name` and `input_config` are supported.
