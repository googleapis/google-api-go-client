...
type service struct {
	...
	max int
	// If true, return a two zero pages before each RPC that returns items, and
...
func (s *service) List(pageSize int, pageToken string) ([]int, string, error) {
	...
	numZeroes := 0
	if pageToken != "" {
		// The page token is "START NUM0" where NUM0 is the number of zero
		// pages written since the last non-zero page.
		words := strings.Fields(pageToken)
		var err error
		start, err = strconv.Atoi(words[0])
		if err != nil {
			return nil, "", err
		}
		numZeroes, err = strconv.Atoi(words[1])
		if err != nil {
			return nil, "", err
		}
	}
	var ints []int
	var i, next int
	if s.zeroes && numZeroes != 2 {
		// Return a zero page.  We're not moving forward.
		next = start
	} else {
		// Return a page with at least one item.
		for i = start; i < s.end && len(ints) < pageSize; i++ {
			ints = append(ints, i)
		}
		next = i
	}
	if next == s.end && (!s.zeroes || numZeroes == 1) {
		pageToken = ""
	} else {
		if len(ints) == 0 {
			numZeroes++
		...
		pageToken = fmt.Sprintf("%d %d", next, numZeroes)
	}
...
func TestPager(t *testing.T) {
	...
	for _, svc := range []service{
		...
		var want [][]int
		for start := 0; start < svc.end; start += pageSize {
			end := start + pageSize
			if end > svc.end {
				end = svc.end
			}
