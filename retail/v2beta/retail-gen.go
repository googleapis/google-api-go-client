...
type GoogleCloudRetailV2betaCatalogAttribute struct {
	...
	// CatalogAttribute can be pre-loaded by using AddCatalogAttribute,
	// ImportCatalogAttributes, or UpdateAttributesConfig APIs. This field
	// is `False` for pre-loaded CatalogAttributes. Only CatalogAttributes
	// that are not in use by products can be deleted. CatalogAttributes
	// that are in use by products cannot be deleted; however, their
	// configuration properties will reset to default values upon removal
	// request. After catalog changes, it takes about 10 minutes for this
	// field to update.
...
type GoogleCloudRetailV2betaControl struct {
	...
	// SearchSolutionUseCase: Required. Specifies the use case for the
	// control. Affects what condition fields can be set. Only settable by
	// search controls. Will default to SEARCH_SOLUTION_USE_CASE_SEARCH if
	// not specified. Currently only allow one search_solution_use_case per
	// control.
	//
	// Possible values:
	//   "SEARCH_SOLUTION_USE_CASE_UNSPECIFIED" - The value when it's
	// unspecified. Defaults to SEARCH.
