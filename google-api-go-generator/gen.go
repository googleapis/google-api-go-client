	...
	for _, opt := range meth.OptParams() {
		...
		if opt.IsRepeated() {
			if goType == "[]string" {
				...
				convertMultiParams(a, "", paramName)
			}
		} else {
			if goType == "string" {
