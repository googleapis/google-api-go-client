...
type CommutePreference struct {
	// AllowNonStreetLevelAddress: Optional. If `true`, jobs without street level
	// addresses may also be returned. For city level addresses, the city center is
	// used. For state and coarser level addresses, text matching is used. If this
	// field is set to `false` or is not specified, only jobs that include street
	// level addresses will be returned by commute search.
	AllowNonStreetLevelAddress bool `json:"allowNonStreetLevelAddress,omitempty"`
	// DepartureHourLocal: Optional. The departure hour to use to calculate traffic
	// impact. Accepts an integer between 0 and 23, representing the hour in the
	// time zone of the start_location. Must not be present if road_traffic is
	// specified.
	DepartureHourLocal int64 `json:"departureHourLocal,omitempty"`
	// Method: Required. The method of transportation for which to calculate the
	// commute time.
	//
	// Possible values:
	...
	// commute time. Must not be present if departure_hour_local is specified.
	//
	// Possible values:
	...
	// StartLocation: Required. The latitude and longitude of the location from
	...
	// ForceSendFields is a list of field names (e.g. "AllowNonStreetLevelAddress")
	// to unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AllowNonStreetLevelAddress") to
...
// is the entity that owns job listings, that is, the hiring entity responsible
// for employing applicants for the job position.
type Company struct {
	// CareerPageLink: Optional. The URL to employer's career site or careers page
	// on the employer's web site.
	CareerPageLink string `json:"careerPageLink,omitempty"`
	// CompanyInfoSources: Optional. Identifiers external to the application that
	// help to further identify the employer.
	...
	// DisableLocationOptimization: Deprecated. Do not use this field. Optional.
	...
	// DisplayName: Required. The name of the employer to be displayed with the
	// job, for example, "Google, LLC.".
	...
	// Recommended practice is to use the distributor_company_id. Defaults to same
	...
	// DistributorCompanyId: Required. A client's company identifier, used to
	// uniquely identify the company. If an employer has a subsidiary or sub-brand,
	// such as "Alphabet" and "Google", which the client wishes to use as the
	// company displayed on the job. Best practice is to create a distinct company
	// identifier for each distinct brand displayed. The maximum number of allowed
	// characters is 255.
	...
	// provided, the logo image should be squared at 80x80 pixels. The url must be
	// a Google Photos or Google Album url. Only images in these Google sub-domains
	// are accepted.
	ImageUrl string `json:"imageUrl,omitempty"`
	// KeywordSearchableCustomAttributes: Optional. A list of keys of filterable
	...
	// values with parenthesis, brackets and special symbols might not be properly
	...
	// keyword_searchable_custom_attributes instead. Optional. A list of filterable
	// custom fields that should be used in keyword search. The jobs of this
	// company are returned if any of these custom fields matches the search
	...
	// is generated by the service when a company is created, for example,
	// "companies/0000aaaa-1111-bbbb-2222-cccc3333dddd".
	Name string `json:"name,omitempty"`
	// StructuredCompanyHqLocation: Output only. A structured headquarters location
	// of the company, resolved from hq_location if possible.
	...
	// Title: Deprecated. Use display_name instead. Required. The name of the
	// employer to be displayed with the job, for example, "Google, LLC.".
	Title string `json:"title,omitempty"`
	// Website: Optional. The URL representing the company's primary web site or
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CareerPageLink") to include in
...
// CompanyInfoSource: A resource that represents an external Google identifier
// for a company, for example, a Google+ business page or a Google Maps
// business page. For unsupported types, use `unknown_type_id`.
type CompanyInfoSource struct {
	// FreebaseMid: Optional. The Google's Knowledge Graph value for the employer's
	// company.
	...
	// MapsCid: Optional. The numeric identifier for the employer's headquarters on
	// Google Maps, namely, the Google Maps CID (cell id).
	MapsCid string `json:"mapsCid,omitempty"`
	// UnknownTypeId: Optional. A Google identifier that does not match any of the
	// other types.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "FreebaseMid") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type CompensationFilter struct {
	// IncludeJobsWithUnspecifiedCompensationRange: Optional. Whether to include
	// jobs whose compensation range is unspecified.
...
type CompensationInfo struct {
	...
	// compensation or pay for the job. As an alternative, compensation_amount_min
	// and compensation_amount_max may be used to define a range of compensation.
	Amount *Money `json:"amount,omitempty"`
	...
	// Max: Deprecated. Use entries instead. Optional. An upper bound on a range
	// for compensation or pay for the job. The currency type is specified in
	// compensation_amount.
	Max *Money `json:"max,omitempty"`
	// Min: Deprecated. Use entries instead. Optional. A lower bound on a range for
	// compensation or pay for the job. The currency type is specified in
	// compensation_amount.
	Min *Money `json:"min,omitempty"`
	// Type: Deprecated. Use entries instead. Optional. Type of job compensation.
	//
	// Possible values:
	//   "JOB_COMPENSATION_TYPE_UNSPECIFIED" - The default value if the type is not
	// specified.
	...
	//   "COMMISSION" - The job compensation is quoted based solely on commission.
	//   "OTHER_TYPE" - The job compensation is not quoted according to the listed
...
type CompensationRange struct {
	// Max: Optional. The maximum amount of compensation. If left empty, the value
	// is set to a maximal compensation value and the currency code is set to match
	// the currency code of min_compensation.
	Max *Money `json:"max,omitempty"`
	// Min: Optional. The minimum amount of compensation. If left empty, the value
...
type CreateJobRequest struct {
	// DisableStreetAddressResolution: Deprecated. Please use processing_options.
...
type CustomAttribute struct {
	// Filterable: Optional. If the `filterable` flag is true, custom field values
	// are searchable. If false, values are not searchable. Default is false.
	Filterable bool `json:"filterable,omitempty"`
	// LongValue: Optional but at least one of string_values or long_value must be
	...
	// StringValues: Optional but at least one of string_values or long_value must
	...
	// `string_values`, a maximum total number of 200 values is allowed, with each
	// `string_value` has a byte size of no more than 255B. For unfilterable
...
// error will be thrown if neither string_value_histogram or
...
type CustomAttributeHistogramRequest struct {
	...
	// StringValueHistogram: Optional. If set to true, the response will include
...
// CustomField: Resource that represents the custom data not captured by the
// standard fields.
...
type Date struct {
	...
	// 0 if specifying a year by itself or a year and month where the day is not
	// significant.
...
// DeleteJobsByFilterRequest: Deprecated. Use BatchDeleteJobsRequest instead.
...
type DeleteJobsByFilterRequest struct {
	...
	// processing steps to complete before the job is cleaned up. Otherwise, the
	// call returns while some steps are still taking place asynchronously, hence
	// faster.
	...
	// ForceSendFields is a list of field names (e.g. "DisableFastProcess") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DisableFastProcess") to include
...
// DeviceInfo: Input only. Device information collected from the job seeker,
...
// ExtendedCompensationFilter: Deprecated. Always use CompensationFilter. Input
// only. Filter on job compensation type and amount.
type ExtendedCompensationFilter struct {
	...
	// Currency: Optional. Specify currency in 3-letter ISO 4217
	// (https://www.iso.org/iso-4217-currency-codes.html) format. If unspecified,
	// jobs are returned regardless of currency.
	Currency string `json:"currency,omitempty"`
	// IncludeJobWithUnspecifiedCompensationRange: Optional. Whether to include
	// jobs whose compensation range is unspecified.
	...
	//   "UNIT_ONLY" - Filter by `base compensation entry's` unit. A job is a match
	// if and only if the job contains a base CompensationEntry and the base
	// CompensationEntry's unit matches provided compensation_units. Populate one
	// or more compensation_units. See ExtendedCompensationInfo.CompensationEntry
	...
	// or more compensation_units.
	Type string `json:"type,omitempty"`
	// ForceSendFields is a list of field names (e.g. "CompensationRange") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CompensationRange") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// ExtendedCompensationInfo: Deprecated. Use CompensationInfo. Describes job
// compensation.
type ExtendedCompensationInfo struct {
	...
	// AnnualizedBaseCompensationUnspecified: Output only. Indicates annualized
	// base compensation range cannot be derived, due to the job's base
	// compensation entry cannot be annualized. See CompensationEntry for
	// explanation on annualization and base compensation entry.
	...
	// AnnualizedTotalCompensationUnspecified: Output only. Indicates annualized
	// total compensation range cannot be derived, due to the job's all
	// CompensationEntry cannot be annualized. See CompensationEntry for
	// explanation on annualization and base compensation entry.
	AnnualizedTotalCompensationUnspecified bool `json:"annualizedTotalCompensationUnspecified,omitempty"`
	// Currency: Optional. A 3-letter ISO 4217
	// (https://www.iso.org/iso-4217-currency-codes.html) currency code.
	Currency string `json:"currency,omitempty"`
	// Entries: Optional. Job compensation information. At most one entry can be of
...
// ExtendedCompensationInfoCompensationEntry: Deprecated. See CompensationInfo.
...
type ExtendedCompensationInfoCompensationEntry struct {
	...
	//   "OTHER_COMPENSATION_UNIT" - Other compensation units.
	Unit string `json:"unit,omitempty"`
	// Unspecified: Optional. Indicates compensation amount and range are unset.
...
// ExtendedCompensationInfoCompensationRange: Deprecated. See CompensationInfo.
// Compensation range.
...
// ExtendedCompensationInfoDecimal: Deprecated. See CompensationInfo. Decimal
// number.
type ExtendedCompensationInfoDecimal struct {
	// Micros: Micro (10^-6) units. The value must be between -999,999 and +999,999
	// inclusive. If `units` is positive, `micros` must be positive or zero. If
	// `units` is zero, `micros` can be positive, zero, or negative. If `units` is
	...
	// ForceSendFields is a list of field names (e.g. "Micros") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
// Filter: Deprecated. Use BatchDeleteJobsRequest instead. Input only. Filter
// for jobs to be deleted.
type Filter struct {
	// RequisitionId: Required. The requisition ID (or posting ID) assigned by the
	// client to identify a job. This is intended for client identification and
	// tracking of listings. name takes precedence over this field The maximum
	// number of allowed characters is 225.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "RequisitionId") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// GetHistogramRequest: Deprecated. Use SearchJobsRequest.histogram_facets
// instead to make a single call with both search and histogram. Input only. A
// request for the `GetHistogram` method.
type GetHistogramRequest struct {
	// AllowBroadening: Optional. Controls whether to broaden the search to avoid
	// too few results for a given query in instances where a search has sparse
	// results. Results from a broadened query is a superset of the results from
	// the original query. Defaults to false.
	AllowBroadening bool `json:"allowBroadening,omitempty"`
	// Filters: Deprecated. Use query instead. Optional. Restrictions on the scope
	// of the histogram.
	...
	// RequestMetadata: Meta information, such as `user_id`, collected from the job
	// searcher or other entity conducting a job search, is used to improve the
	// service's search quality. Users determine identifier values, which must be
	// unique and consist.
	RequestMetadata *RequestMetadata `json:"requestMetadata,omitempty"`
	// SearchTypes: Required. A list of facets that specify the histogram data to
	// be calculated against and returned. Histogram response times can be slow,
	...
	//   "ADMIN1_COUNTRY" - A combination of state or province code with a country
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AllowBroadening") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// GetHistogramResponse: Deprecated. Use SearchJobsRequest.histogram_facets
// instead to make a single call with both search and histogram. Output only.
...
// GoogleCloudTalentV4BatchOperationMetadata: Metadata used for long running
// operations returned by CTS batch APIs. It's used to replace
...
// GoogleCloudTalentV4CompensationInfoCompensationEntry: A compensation entry
// that represents one component of compensation, such as base pay, bonus, or
// other compensation type. Annualization: One compensation entry can be
// annualized if - it contains valid amount or range. - and its
// expected_units_per_year is set or can be derived. Its annualized range is
// determined as (amount or range) times expected_units_per_year.
...
// GoogleCloudTalentV4CompensationInfoCompensationRange: Compensation range.
...
// GoogleCloudTalentV4CustomAttribute: Custom attribute values that are either
...
// referred to as a "job listing" or "job requisition"). A job belongs to a
// Company, which is the hiring entity responsible for the job.
type GoogleCloudTalentV4Job struct {
	...
	// pattern: a-zA-Z*. For example, key0LikeThis or KEY_1_LIKE_THIS. At most 100
...
// GoogleCloudTalentV4JobApplicationInfo: Application related details of a job
// posting.
...
// GoogleCloudTalentV4JobDerivedInfo: Derived details about the job posting.
...
// GoogleCloudTalentV4Location: A resource that represents a location with full
...
type HistogramFacets struct {
	...
	// facets, for example, `COMPANY_SIZE`, `EMPLOYMENT_TYPE` etc. This field is
	// equivalent to GetHistogramRequest.
	//
	// Possible values:
	//   "JOB_FIELD_UNSPECIFIED" - The default value if search type is not
	// specified.
...
type HistogramResults struct {
	...
	// results that matches HistogramFacets.compensation_histogram_requests.
	...
	// attributes that matches HistogramFacets.custom_attribute_histogram_facets.
...
type Job struct {
	...
	// application_email_list or application_instruction must be specified. Use
	// this field to specify email address(es) to which resumes or applications can
	// be sent. The maximum number of allowed characters is 255.
	ApplicationEmailList []string `json:"applicationEmailList,omitempty"`
	// ApplicationInstruction: Optional but at least one of application_urls,
	// application_email_list or application_instruction must be specified. Use
	// this field to provide instructions, such as "Mail your application to ...",
	...
	// application_email_list or application_instruction must be specified. Use
	// this URL field to direct an applicant to a website, for example to link to
	// an online application form. The maximum number of allowed characters is
	// 2,000.
	...
	//   "CHILD_CARE" - The job includes access to programs that support child
	// care, such as daycare.
	//   "DENTAL" - The job includes dental services that are covered by a dental
	// insurance plan.
	//   "DOMESTIC_PARTNER" - The job offers specific benefits to domestic
	// partners.
	//   "FLEXIBLE_HOURS" - The job allows for a flexible work schedule.
	//   "MEDICAL" - The job includes health services that are covered by a medical
	// insurance plan.
	//   "LIFE_INSURANCE" - The job includes a life insurance plan provided by the
	// employer or available for purchase by the employee.
	//   "PARENTAL_LEAVE" - The job allows for a leave of absence to a parent to
	...
	// by the employer or available for purchase by the employee.
	//   "SICK_DAYS" - The job allows for paid time off due to illness.
	//   "TELECOMMUTE" - Deprecated. Set Region.TELECOMMUTE instead. The job allows
	// telecommuting (working remotely).
	//   "VACATION" - The job includes paid time off for vacation.
	//   "VISION" - The job includes vision services that are covered by a vision
	// insurance plan.
	Benefits []string `json:"benefits,omitempty"`
	// CompanyDisplayName: Output only. The name of the company listing the job.
	CompanyDisplayName string `json:"companyDisplayName,omitempty"`
	// CompanyName: Optional but one of company_name or distributor_company_id must
	// be provided. The resource name of the company listing the job, such as
	...
	// CompanyTitle: Deprecated. Use company_display_name instead. Output only. The
	// name of the company listing the job.
	...
	// structured fields. This field is a more general combination of the
	// deprecated id-based filterable_custom_fields and string-based
	// non_filterable_custom_fields. The keys of the map are strings up to 64 bytes
	// and must match the pattern: a-zA-Z*. At most 100 filterable and at most 100
	// unfilterable keys are supported. For filterable `string_values`, across all
	// keys at most 200 values are allowed, with each string no more than 255
	...
	// distributor_company_id must be provided. A unique company identifier used by
	// job distributors to identify an employer's company entity. company_name
	// takes precedence over this field, and is the recommended field to use to
	// identify companies. The maximum number of allowed characters is 255.
	DistributorCompanyId string `json:"distributorCompanyId,omitempty"`
	// EducationLevels: Optional. The desired education level for the job, such as
	// "Bachelors", "Masters", "Doctorate".
	//
	// Possible values:
	//   "EDUCATION_LEVEL_UNSPECIFIED" - The default value if the level is not
	// specified.
	...
	//   "NO_DEGREE_REQUIRED" - No formal education is required for the position.
	...
	// position.
	//   "VOLUNTEER" - The is an opportunity for an individual to volunteer, where
	// there is no expectation of compensation for the provided services.
	//   "PER_DIEM" - The job requires an employee to work on an as-needed basis
	// with a flexible schedule.
	//   "CONTRACT_TO_HIRE" - The job is offered as a contracted position with the
	// understanding that it is converted into a full-time position at the end of
	// the contract. Jobs of this type are also returned by a search for
	// EmploymentType.CONTRACTOR jobs.
	//   "FLY_IN_FLY_OUT" - The job involves employing people in remote areas and
	// flying them temporarily to the work site instead of relocating employees and
	...
	EmploymentTypes []string `json:"employmentTypes,omitempty"`
	// EndDate: Optional. The end date of the job in UTC time zone. Typically this
	...
	// out. The format of this field is RFC 3339 date strings. Example:
	// 2000-01-01T00:00:00.999999999Z See https://www.ietf.org/rfc/rfc3339.txt
	// (https://www.ietf.org/rfc/rfc3339.txt). A valid date range is between
	// 1970-01-01T00:00:00.0Z and 2100-12-31T23:59:59.999Z. Invalid dates are
	// ignored and treated as expire time not provided. If this value is not
	...
	// job expires after 2017/01/31 13:00AM UTC. If this value is not provided but
	// expiry_date is, expiry_date is used. If this value is not provided on job
	// update, it depends on the field masks set by
	// UpdateJobRequest.update_job_fields. If the field masks include expiry_time,
	// or the masks are empty meaning that every field is updated, the job posting
	// expires after 30 days from the job's last update time. Otherwise the
	// expiration date isn't updated.
	ExpireTime string `json:"expireTime,omitempty"`
	// ExpiryDate: Deprecated. Use expire_time instead. Optional but strongly
	// recommended to be provided for the best service experience. The expiration
	// date of the job in UTC time. After 12 am on this date, the job is marked as
	// expired, and it no longer appears in search results. The expired job can't
	...
	// job can be updated and opened again by using a future expiration date. It
	// can also remain expired. Updating an expired job to be open fails if there
	// is another existing open job with same requisition_id, company_name and
	// language_code. The expired jobs are retained in our system for 90 days.
	// However, the overall expired job count cannot exceed 3 times the maximum of
	// open jobs count over the past week, otherwise jobs with earlier expire time
	// are removed first. Expired jobs are no longer accessible after they are
	// cleaned out. A valid date range is between 1970/1/1 and 2100/12/31. Invalid
	// dates are ignored and treated as expiry date not provided. If this value is
	// not provided on job creation or is invalid, the job posting expires after 30
	// days from the job's creation time. For example, if the job was created on
	// 2017/01/01 13:00AM UTC with an unspecified expiration date, the job expires
	// after 2017/01/31 13:00AM UTC. If this value is not provided on job update,
	// it depends on the field masks set by UpdateJobRequest.update_job_fields. If
	// the field masks include expiry_date, or the masks are empty meaning that
	// every field is updated, the job expires after 30 days from the job's last
	...
	// FilterableCustomFields: Deprecated. Use custom_attributes instead. Optional.
	// A map of fields to hold filterable custom job attributes not captured by the
	// standard fields such as job_title, company_name, or level. These custom
	// fields store arbitrary string values, and can be used for purposes not
	// covered by the structured fields. For the best search experience, use of the
	// structured rather than custom fields is recommended. Data stored in these
	// custom fields fields are indexed and searched against by keyword searches
	// (see SearchJobsRequest.custom_field_filters][]). The map key must be a
	// number between 1-20. If an invalid key is provided on job create or update,
	...
	// The maximum number of allowed characters is 10,000.
	Incentives string `json:"incentives,omitempty"`
	// JobLocations: Output only. Structured locations of the job, resolved from
	// locations.
	JobLocations []*JobLocation `json:"jobLocations,omitempty"`
	// JobTitle: Required. The title of the job, such as "Software Engineer" The
	// maximum number of allowed characters is 500.
	...
	// code based on Job.description is assigned, otherwise defaults to 'en_US'.
	LanguageCode string `json:"languageCode,omitempty"`
	// Level: Optional. The experience level associated with the job, such as
	// "Entry Level".
	//
	// Possible values:
	...
	// experience. Location(s) where the emploeyer is looking to hire for this job
	...
	// Name: Required during job update. Resource name assigned to a job by the
	// API, for example, "/jobs/foo". Use of this field in job queries and API
	...
	// 0.
	PromotionValue int64 `json:"promotionValue,omitempty"`
	// PublishDate: Optional. The date this job was most recently published in UTC
	// format. The default value is the time the request arrives at the server.
	...
	Qualifications string `json:"qualifications,omitempty"`
	// ReferenceUrl: Output only. The URL of a web page that displays job details.
	ReferenceUrl string `json:"referenceUrl,omitempty"`
	// Region: Optional. The job Region (for example, state, country) throughout
	// which the job is available. If this field is set, a LocationFilter in a
	// search query within the job region finds this job if an exact location match
	// is not specified. If this field is set, setting job locations to the same
	...
	// For example, if a `STATE_WIDE` job is posted in "CA, USA", it is returned if
	// LocationFilter has "Mountain View".
	//   "NATION_WIDE" - In addition to exact location matching, job is returned
	// when LocationFilter in search query is in the same country as this job. For
	// example, if a `NATION_WIDE` job is posted in "USA", it is returned if
	// LocationFilter has 'Mountain View'.
	//   "TELECOMMUTE" - Job allows employees to work remotely (telecommute). If
	// locations are provided with this value, the job is considered as having a
	// location, but telecommuting is allowed.
	...
	// used by clients for client identification and tracking of listings. A job is
	...
	Responsibilities string `json:"responsibilities,omitempty"`
	// StartDate: Optional. The start date of the job in UTC time zone. Typically
	// this field is used for contracting engagements. Dates prior to 1970/1/1 and
	// invalid date formats are ignored.
	StartDate *Date `json:"startDate,omitempty"`
	// UnindexedCustomFields: Deprecated. Use custom_attributes instead. Optional.
	...
	// JobVisibility.PRIVATE if not specified. Currently only JobVisibility.PRIVATE
	// is supported.
	//
	// Possible values:
	...
	//   "PUBLIC" - The Job is visible to the owner and may be visible to all other
	// API clients. Not yet supported. Use PRIVATE.
	Visibility string `json:"visibility,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "ApplicationEmailList") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ApplicationEmailList") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type JobFilters struct {
	// Categories: Optional. The category filter specifies the categories of jobs
	// to search against. See Category for more information. If a value is not
	...
	//
	// Possible values:
	//   "JOB_CATEGORY_UNSPECIFIED" - The default value if the category is not
	// specified.
	//   "ACCOUNTING_AND_FINANCE" - An accounting and finance job, such as an
	// Accountant.
	//   "ADMINISTRATIVE_AND_OFFICE" - And administrative and office job, such as
	...
	// different travel methods (e.g. driving or public transit). Note: this only
	// works with COMMUTE MODE. When specified, [JobFilters.location_filters] will
	// be ignored. Currently we do not support sorting by commute time.
	CommuteFilter *CommutePreference `json:"commuteFilter,omitempty"`
	// CompanyNames: Optional. The company names filter specifies the company
	// entities to search against. If a value is not specified, jobs are searched
	// for against all companies. If multiple values are specified, jobs are
	// searched against the specified companies. At most 20 company filters are
	// allowed.
	CompanyNames []string `json:"companyNames,omitempty"`
	// CompanyTitles: Optional. This filter specifies the exact company titles of
	// jobs to search against. If a value is not specified, jobs within the search
	...
	// job with per-hour compensation > $15", only jobs that meet this criteria are
	...
	// to match against the Job.custom_attributes that are marked as `filterable`.
	...
	// maximum of 100 comparisons/functions in the expression. The expression must
	// be < 3000 bytes in length. Sample Query: (key1 = "TEST" OR
	...
	// number between 1-20 (the service supports 20 custom fields) corresponding to
	// the desired custom field map value. If an invalid key is provided or
	...
	//   "OTHER" - The job does not fit any of the other listed types.
	EmploymentTypes []string `json:"employmentTypes,omitempty"`
	// ExtendedCompensationFilter: Deprecated. Always use compensation_filter.
	...
	// against, for example, "en-US". If a value is not specified, the search
	// results may contain jobs in any locale. Language codes should be in BCP-47
	// format, for example, "en-US" or "sr-Latn". For more information, see Tags
	// for Identifying Languages (https://tools.ietf.org/html/bcp47). At most 10
	...
	// information. If a location value is not specified, jobs are retrieved from
	// all locations. If multiple values are specified, jobs are retrieved from any
	// of the specified locations. If different values are specified for the
	...
	// PublishDateRange: Optional. Jobs published within a range specified by this
	// filter are searched against, for example, DateRange.PAST_MONTH. If a value
	// is not specified, all open jobs are searched against regardless of the date
	// they were published.
	//
	// Possible values:
	...
	// Query: Optional. The query filter contains the keywords that match against
	// the job title, description, and location fields. The maximum query size is
	// 255 bytes/characters.
	Query string `json:"query,omitempty"`
	// TenantJobOnly: Deprecated. Do not use this field. This flag controls whether
	// the job search should be restricted to jobs owned by the current user.
	// Defaults to false where all jobs accessible to the user are searched
	// against.
...
// JobLocation: Output only. A resource that represents a location with full
// geographic information.
type JobLocation struct {
	...
	// other delivery location.
	PostalAddress *PostalAddress `json:"postalAddress,omitempty"`
	// RadiusMeters: Radius in meters of the job location. This value is derived
	...
	// example, currently, "Mountain View, CA, USA" has a radius of 7885.79 meters.
...
type JobProcessingOptions struct {
	...
	// qualifications * responsibilities HTML tags in these fields may be stripped
	// if sanitiazation is not disabled. Defaults to
	// HtmlSanitization.SIMPLE_FORMATTING_ONLY.
	//
	// Possible values:
	//   "HTML_SANITIZATION_UNSPECIFIED" - Default value.
...
type JobQuery struct {
	...
	// by commute time.
	CommuteFilter *CommutePreference `json:"commuteFilter,omitempty"`
	// CompanyDisplayNames: Optional. This filter specifies the exact company
	// display name of the jobs to search against. If a value isn't specified, jobs
	// within the search results are associated with any company. If multiple
	...
	// companies specified. At most 20 company filters are allowed.
	CompanyNames []string `json:"companyNames,omitempty"`
	// CompensationFilter: Optional. This search filter is applied only to
	// Job.compensation_info. For example, if the filter is specified as "Hourly
	...
	// syntax for this expression is a subset of Google SQL syntax. Supported
	// operators are: =, !=, <, <=, >, >= where the left of the operator is a
	// custom field key and the right of the operator is a number or string
	// (surrounded by quotes) value. Supported functions are LOWER() to perform
	// case insensitive match and EMPTY() to filter on the existence of a key.
	...
	// comparisons/functions are allowed in the expression. The expression must be
	// < 2000 characters in length. Sample Query: (key1 = "TEST" OR
	// LOWER(key1)="test" OR NOT EMPTY(key1)) AND key2 > 100
	CustomAttributeFilter string `json:"customAttributeFilter,omitempty"`
	// DisableSpellCheck: Optional. This flag controls the spell-check feature. If
	// false, the service attempts to correct a misspelled query, for example,
	...
	// PublishDateRange: Optional. Jobs published within a range specified by this
	// filter are searched against, for example, DateRange.PAST_MONTH. If a value
	// isn't specified, all open jobs are searched against regardless of their
	// published date.
	//
	// Possible values:
	//   "DATE_RANGE_UNSPECIFIED" - Default value: Filtering on time is not
...
// as a pair of doubles representing degrees latitude and degrees longitude.
...
// ListCompanyJobsResponse: Deprecated. Use ListJobsResponse instead. Output
// only. The List jobs response object.
...
type LocationFilter struct {
	...
	// IsTelecommute: Optional. Allows the client to return jobs without a set
	// location, specifically, telecommuting jobs (telecomuting is considered by
	// the service as a special location. Job.allow_telecommute indicates if a job
	...
	// which to search. This field is ignored if `location_name` is provided.
	LatLng *LatLng `json:"latLng,omitempty"`
	// Name: Optional. The address name, such as "Mountain View" or "Bay Area".
	Name string `json:"name,omitempty"`
	// RegionCode: Optional. CLDR region code of the country/region of the address.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DistanceInMiles") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type NumericBucketingResult struct {
	...
	// MaxValue: Stores the maximum value of the numeric field. Will be populated
...
// an i18n-ready address widget such as
...
type RequestMetadata struct {
	...
	// the career site. If this field is not available for some reason, send
	// "UNKNOWN". Note that any improvements to the service model for a particular
	...
	// API. If this field is not available for some reason, please send "UNKNOWN".
	...
	// this value in order to have the strongest positive impact on search quality.
	...
	// "UNKNOWN". Note that any improvements to the service model for a particular
	// tenant site, rely on this field being set correctly to some unique user_id.
...
type ResponseMetadata struct {
	// ExperimentIdList: Identifiers for the versions of the search algorithm used
	// during this API invocation if multiple algorithms are used. The default
	// value is empty. For search response only.
	ExperimentIdList []int64 `json:"experimentIdList,omitempty"`
	// Mode: For search response only. Indicates the mode of a performed search.
	//
	// Possible values:
	...
	//   "EMAIL_ALERT_SEARCH" - Deprecated. Please use the SearchJobsForAlert API.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ExperimentIdList") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SearchJobsRequest struct {
	...
	// relevance thresholding. Relevance thresholding removes jobs that have low
	// relevance in search results, for example, removing "Assistant to the CEO"
	// positions from the search results of a search for "CEO". Disabling relevance
	// thresholding improves the accuracy of subsequent search requests. Defaults
	// to false.
	...
	// requires the return of a precise count of the first 300 results. Setting
	// this to `true` ensures consistency in the number of results per page. Best
	// practice is to set this value to true if a client allows users to jump
	// directly to a non-sequential search results page. Enabling this flag may
	// adversely impact performance. Defaults to false.
	EnablePreciseResultSize bool `json:"enablePreciseResultSize,omitempty"`
	// Filters: Deprecated. Use query instead. Optional. Restrictions on the scope
	// of the search request, such as filtering by location.
	Filters *JobFilters `json:"filters,omitempty"`
	// HistogramFacets: Optional. Restrictions on what fields to perform histogram
	// on, such as `COMPANY_SIZE` etc.
	HistogramFacets *HistogramFacets `json:"histogramFacets,omitempty"`
	// JobView: Optional. The number of job attributes returned for jobs in the
	...
	// Job.company_name, Job.job_locations, Job.description, Job.visibility. Note:
	// Job.description is deprecated. It is scheduled to be removed from
	...
	// Job.incentives are deprecated. These fields are scheduled to be removed from
	// MatchingJob.Job objects in the SearchJobsResponse results on 12/31/2018. See
	// the alternative MatchingJob.search_text_snippet and MatchingJob.job_summary
	// fields.
	...
	// search results. This field is only considered if page_token is unset. For
	// example, 0 means to return results starting from the first matching job, and
	// 10 means to return from the 11th job. This can be used for pagination, (for
	// example, pageSize = 10 and offset = 10 means to return from the second
	// page).
	Offset int64 `json:"offset,omitempty"`
	// OrderBy: Deprecated. Use sort_by instead. Optional. The criteria determining
	// how search results are sorted. Defaults to SortBy.RELEVANCE_DESC if no value
	// is specified.
	//
	// Possible values:
	...
	//   "ANNUALIZED_BASE_COMPENSATION" - Sort by job annualized base compensation
	// in ascending order. If job's annualized base compensation is unspecified,
	// they are put at the end of search result.
	...
	// compensation in descending order. If job's annualized total compensation is
	// unspecified, they are put at the end of search result.
	...
	// searcher, used to improve the search quality of the service. The
	// identifiers, (such as `user_id`) are provided by users, and must be unique
	// and consistent.
	RequestMetadata *RequestMetadata `json:"requestMetadata,omitempty"`
	// SortBy: Optional. The criteria determining how search results are sorted.
	...
	// ForceSendFields is a list of field names (e.g.
	// "DisableRelevanceThresholding") to unconditionally include in API requests.
	// By default, fields with empty or default values are omitted from API
	// requests. See https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields
	// for more details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DisableRelevanceThresholding") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SearchJobsResponse struct {
	...
	// specified query. This information is only available when query has a valid
	// CommutePreference.
	AppliedCommuteFilter *CommutePreference `json:"appliedCommuteFilter,omitempty"`
	// AppliedJobLocationFilters: The location filters that the service applied to
	// the specified query. If any filters are lat-lng based, the
	...
	// results, seenenable_precise_result_size.
	...
	NextPageToken string `json:"nextPageToken,omitempty"`
	// NumJobsFromBroadenedQuery: If query broadening is enabled, we may append
	...
	// means all the jobs in the jobs list are from the original (without
	...
	// TotalSize: The precise result count, which is available only if the client
	// set enable_precise_result_size to `true` or if the response is the last page
	// of results. Otherwise, the value will be `-1`.
	TotalSize int64 `json:"totalSize,omitempty,string"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "AppliedCommuteFilter") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AppliedCommuteFilter") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type UpdateJobRequest struct {
	// DisableStreetAddressResolution: Deprecated. Please use processing_options.
	// This flag is ignored if processing_options is set. Optional. If set to
	// `true`, the service does not attempt resolve a more precise address for the
	// job.
	DisableStreetAddressResolution bool `json:"disableStreetAddressResolution,omitempty"`
	...
	// is set.
	ProcessingOptions *JobProcessingOptions `json:"processingOptions,omitempty"`
	// UpdateJobFields: Optional but strongly recommended to be provided for the
	// best service experience. If update_job_fields is provided, only the
	// specified fields in job are updated. Otherwise all the fields are updated. A
	// field mask to restrict the fields that are updated. Valid values are: *
	// jobTitle * employmentTypes * description * applicationUrls *
	// applicationEmailList * applicationInstruction * responsibilities *
	// qualifications * educationLevels * level * department * startDate * endDate
...
//
//   - name: The resource name of the company to be deleted, such as,
//     "companies/0000aaaa-1111-bbbb-2222-cccc3333dddd".
...
//
//   - name: Resource name of the company to retrieve, such as
//     "companies/0000aaaa-1111-bbbb-2222-cccc3333dddd".
...
// List: Lists all companies associated with a Cloud Talent Solution account.
func (r *CompaniesService) List() *CompaniesListCall {
	c := &CompaniesListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	return c
}

// MustHaveOpenJobs sets the optional parameter "mustHaveOpenJobs": Set to true
// if the companies request must have open jobs. Defaults to false. If true, at
// most page_size of companies are fetched, among which only those with open
// jobs are returned.
...
// Patch: Updates the specified company. Company names can't be updated. To
// update a company name, delete the company and all jobs associated with it,
// and only then re-create them.
//
...
// UpdateCompanyFields sets the optional parameter "updateCompanyFields":
// Optional but strongly recommended to be provided for the best service
// experience. If update_company_fields is provided, only the specified fields
// in company are updated. Otherwise all the fields are updated. A field mask
// to specify the company fields to update. Valid values are: * displayName *
...
// List: Deprecated. Use ListJobs instead. Lists all jobs associated with a
// company.
//
//   - companyName: The resource name of the company that owns the jobs to be
...
// IdsOnly sets the optional parameter "idsOnly": If set to `true`, only job
// ID, job requisition ID and language code will be returned. A typical use is
// to synchronize job repositories. Defaults to false.
func (c *CompaniesJobsListCall) IdsOnly(idsOnly bool) *CompaniesJobsListCall {
	c.urlParams_.Set("idsOnly", fmt.Sprint(idsOnly))
	return c
}

// IncludeJobsCount sets the optional parameter "includeJobsCount": Deprecated.
// Please DO NOT use this field except for small companies. Suggest counting
// jobs page by page instead.  Set to true if the total number of open jobs is
...
// requisition ID, also known as posting ID, assigned by the company to the
...
// to be returned per page of results. If ids_only is set to true, the maximum
// allowed page size is 1000. Otherwise, the maximum allowed page size is 100.
...
//
// - name: The resource name of the job to be deleted, such as "jobs/11111111".
...
// Deprecated. This field is not working anymore.  If set to true, this call
// waits for all processing steps to complete before the job is cleaned up.
...
// specified job by filter. You can specify whether to synchronously wait for
...
//
// - name: The resource name of the job to retrieve, such as "jobs/11111111".
...
// Histogram: Deprecated. Use SearchJobsRequest.histogram_facets instead to
// make a single call with both search and histogram. Retrieves a histogram for
// the given GetHistogramRequest. This call provides a structured count of jobs
// that match against the search query, grouped by specified facets. This call
// constrains the visibility of jobs present in the database, and only counts
// jobs the caller has permission to search against. For example, use this call
// to generate the number of jobs in the U.S. by state.
...
// Any non-2xx status code is an error. Response headers are in either
// *GetHistogramResponse.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// eligible for filtering are: * `companyName` (Required) * `requisitionId`
// (Optional) Sample Query: * companyName = "companies/123" * companyName =
...
// Job.name, Job.requisition_id and Job.language_code will be returned. A
// typical use case is to synchronize job repositories. Defaults to false.
...
// Complete: Completes the specified prefix with job keyword suggestions.
...
// language of the query. This is the BCP-47 language code, such as "en-US" or
...
// CompletionType.COMPANY_NAME type, only companies having open jobs with same
// language_code are returned. For CompletionType.COMBINED type, only open jobs
// with same language_code or companies having open jobs with same
