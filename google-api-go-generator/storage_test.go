...
func TestRepeatedParams(t *testing.T) {
	...
	_, err = cs.List(10).Active(true).MaxResults(1).Ids([]int64{2, 3}).PageToken("abc").SizeIds([]int64{4, 5}).Types([]string{"def", "ghi"}).Do()
	if err != nil {
	...
	if g, w := handler.reqURIs[0], "/userprofiles/10/creatives?active=true&alt=json&ids=2&ids=3&maxResults=1&pageToken=abc&sizeIds=4&sizeIds=5&types=def&types=ghi"; g != w {
