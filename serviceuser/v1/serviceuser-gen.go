...
const basePathTemplate = "https://serviceuser.UNIVERSE_DOMAIN/"
...
// of
// this message itself. See https://cloud.google.com/apis/design/glossary
// for
...
type AuthProvider struct {
	...
	// AuthorizationUrl: Redirect URL if JWT token is required but no present or is
	// expired.
...
// request.
//
// If a method doesn't have any auth requirements, request credentials will
// be
// ignored.
...
//
// This section determines the authorization provider, if unspecified, then
// no
...
// that
// can be sent in grpc metadata using “x-goog-ext-<extension_id>-bin”
// and
// “x-goog-ext-<extension_id>-jspb” format. For example, list any
// service
// specific protobuf types that can appear in grpc metadata as follows in
// your
// yaml file:
//
// Example:
...
//	     - google.foo.v1.NewExtension
//
// You can also specify extension ID instead of fully qualified extension
// name
// here.
type Context struct {
...
// To override the display text used for the link, this can be
// used:
// <pre><code>&#91;display
...
// Endpoint: `Endpoint` describes a network endpoint that serves a set of
// APIs.
...
// type.
// The path template controls how fields of the request message are mapped
// to
// the URL path.
//
// Example:
...
// as `...?param=A&param=B`. In the case of a message type, each field of
// the
...
// matches
// zero or more URL path segments, which must be the last part of the URL
// path
// except the `Verb`.
//
...
// a
// `HttpRule` that maps the gRPC method to a REST endpoint, achieving the
// same
...
// ListEnabledServicesResponse: Response message for `ListEnabledServices`
// method.
type ListEnabledServicesResponse struct {
	// NextPageToken: Token that can be passed to `ListEnabledServices` to resume a
	// paginated
	// query.
...
//
// In most cases: use one or at most two OAuth scopes for an entire family
// of
...
// enumeration, etc.
type Option struct {
	// Name: The option's name. For protobuf built-in options (options defined
	// in
...
type Page struct {
	// Content: The Markdown content of the page. You can use <code>&#40;== include
	// {path} ==&#41;</code>
...
type QuotaLimit struct {
	...
	// Optional. If not set, the UI will provide a default display name based
	// on
	...
	// We
	// support only "100s" and "1d" for now. Additional support will be added
	// in
...
type SearchServicesResponse struct {
	// NextPageToken: Token that can be passed to `ListAvailableServices` to resume
	// a paginated
	// query.
	NextPageToken string `json:"nextPageToken,omitempty"`
...
type Service struct {
	...
	// ConfigVersion: The semantic version of the service configuration. The config
	// version
...
// SourceContext: `SourceContext` represents information about the source of
// a
...
// Disable: Disable a service so it can no longer be used with a
...
//     consumer.
//
//     The Service User implementation accepts the following forms for
//     consumer:
//
//   - "project:<project_id>"
//
...
// result to start with; returned by a previous list
// call.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListEnabledServicesResponse.ServerResponse.Header or (if a response was
...
// Any non-2xx status code is an error. Response headers are in either
// *SearchServicesResponse.ServerResponse.Header or (if a response was returned
