...
func (meth *Method) generateCode() {
	...
	if meth.supportsMediaUpload() && httpMethod != "GET" {
		...
		pn("contentLength_, hasMediaUpload_ := googleapi.ConditionallyIncludeMedia(c.mediaUpload_, &body, &ctype)")
	}
	...
	if meth.supportsMediaUpload() {
		pn("if hasMediaUpload_ { req.ContentLength = contentLength_ }")
	}
	if hasContentType {
