...
func (s GenerateInstantVerificationTokenRequest) MarshalJSON() ([]byte, error) {
	type NoMethod GenerateInstantVerificationTokenRequest
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}

// GenerateInstantVerificationTokenResponse: Response message for
// Verifications.GenerateInstantVerificationToken.
type GenerateInstantVerificationTokenResponse struct {
	// InstantVerificationToken: The generated instant verification token.
	InstantVerificationToken string `json:"instantVerificationToken,omitempty"`
	// Result: Output only. The result of the instant verification token
	// generation.
	//
	// Possible values:
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "InstantVerificationToken")
	// to unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "InstantVerificationToken") to
...
func (s GenerateInstantVerificationTokenResponse) MarshalJSON() ([]byte, error) {
	type NoMethod GenerateInstantVerificationTokenResponse
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}
...
type VerificationTokensGenerateCall struct {
	s                                       *Service
	generateinstantverificationtokenrequest *GenerateInstantVerificationTokenRequest
	urlParams_                              gensupport.URLParams
	ctx_                                    context.Context
	header_                                 http.Header
}

// Generate: Generate a token for the provided location data to verify the
// location.
func (r *VerificationTokensService) Generate(generateinstantverificationtokenrequest *GenerateInstantVerificationTokenRequest) *VerificationTokensGenerateCall {
	c := &VerificationTokensGenerateCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.generateinstantverificationtokenrequest = generateinstantverificationtokenrequest
	return c
}
...
func (c *VerificationTokensGenerateCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.generateinstantverificationtokenrequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	req.Header = reqHeaders
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "mybusinessverifications.verificationTokens.generate", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "mybusinessverifications.verificationTokens.generate" call.
// Any non-2xx status code is an error. Response headers are in either
// *GenerateInstantVerificationTokenResponse.ServerResponse.Header or (if a
// response was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
func (c *VerificationTokensGenerateCall) Do(opts ...googleapi.CallOption) (*GenerateInstantVerificationTokenResponse, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
	...
	ret := &GenerateInstantVerificationTokenResponse{
		ServerResponse: googleapi.ServerResponse{
			Header:         res.Header,
			HTTPStatusCode: res.StatusCode,
		},
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "mybusinessverifications.verificationTokens.generate", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
