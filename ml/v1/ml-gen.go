...
// GoogleApi__HttpBody: Message that represents an arbitrary HTTP body. It
// should only be used for payload formats that can't be represented as JSON,
...
type GoogleCloudMlV1AutomatedStoppingConfigDecayCurveAutomatedStoppingConfig struct {
	...
	// each Trials Decay Curve. Otherwise, Measurement.steps will be used as the
	// x-axis.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "UseElapsedTime") to include in
...
// median automated stopping rule stops a pending trial if the trial's best
...
type GoogleCloudMlV1HyperparameterOutputHyperparameterMetric struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ObjectiveValue") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// GoogleCloudMlV1MeasurementMetric: A message representing a metric in the
// measurement.
...
type GoogleCloudMlV1StudyConfigParameterSpecDiscreteValueSpec struct {
	// Values: Must be specified if type is `DISCRETE`. A list of feasible points.
...
type GoogleCloudMlV1StudyConfigParameterSpecMatchingParentCategoricalValueSpec struct {
	// Values: Matches values of the parent parameter with type 'CATEGORICAL'. All
...
type GoogleCloudMlV1StudyConfigParameterSpec struct {
	...
	// ChildParameterSpecs: A child node is active if the parameter's value matches
	// the child node's matching_parent_values. If two items in
...
// tuned. Contains the name of the parameter and the suggested value to use for
// this trial.
...
// request config. Note that the AcceleratorConfig can be used in both Jobs and
// Versions. Learn more about accelerators for training
...
// AddTrialMeasurement service method.
...
type GoogleCloudMlV1__AutoScaling struct {
	...
	// MinNodes: Optional. The minimum number of nodes to allocate for this model.
	...
	// cost per node-hour as documented in the pricing guide
	// (/ml-engine/docs/pricing), even if no predictions are performed. There is
	// additional cost for each prediction performed. Unlike manual scaling, if the
	// load gets too heavy for the nodes that are up, the service will
	...
	// not specified and AutoScaling is used with a legacy (MLS1) machine type
	// (/ml-engine/docs/machine-types-online-prediction), `min_nodes` defaults to
	// 0, in which case, when traffic to a model stops (and after a cool-down
	...
	// AutoScaling is used with a Compute Engine (N1) machine type
	// (/ml-engine/docs/machine-types-online-prediction), `min_nodes` defaults to
	// 1. `min_nodes` must be at least 1 for use with a Compute Engine machine
...
// Stopping of Trials. If no implementation_config is set, automated early
...
type GoogleCloudMlV1__AutomatedStoppingConfig struct {
	...
	// ForceSendFields is a list of field names (e.g. "DecayCurveStoppingConfig")
	// to unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DecayCurveStoppingConfig") to
...
type GoogleCloudMlV1__BuiltInAlgorithmOutput struct {
	...
	// RuntimeVersion: AI Platform runtime version on which the built-in algorithm
	// was trained.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Framework") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__Capability struct {
	...
	//   "ONLINE_PREDICTION"
	Type string `json:"type,omitempty"`
	// ForceSendFields is a list of field names (e.g. "AvailableAccelerators") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
// placed in the metadata field of a google.longrunning.Operation associated
...
// for the CheckTrialEarlyStoppingState service method.
type GoogleCloudMlV1__CheckTrialEarlyStoppingStateRequest struct {
}

// GoogleCloudMlV1__CheckTrialEarlyStoppingStateResponse: The message will be
// placed in the response field of a completed google.longrunning.Operation
...
type GoogleCloudMlV1__Config struct {
	...
	// ForceSendFields is a list of field names (e.g. "TpuServiceAccount") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "TpuServiceAccount") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// container. This message is a subset of the Kubernetes ContainerPort v1 core
// specification
...
type GoogleCloudMlV1__ContainerPort struct {
	// ContainerPort: Number of the port to expose on the container. This must be a
	// valid port number: 0 < PORT_NUMBER < 65536.
...
// serving predictions. This message is a subset of the Kubernetes Container v1
// core specification
...
type GoogleCloudMlV1__ContainerSpec struct {
	...
	// Prediction
	// (/ai-platform/prediction/docs/custom-container-requirements#aip-variables)
	...
	// serving predictions. This URI must identify an image in Artifact Registry
	// (/artifact-registry/docs/overview) and begin with the hostname
	// `{REGION}-docker.pkg.dev`, where `{REGION}` is replaced by the region that
	// matches AI Platform Prediction regional endpoint
	// (/ai-platform/prediction/docs/regional-endpoints) that you are using. For
	// example, if you are using the `us-central1-ml.googleapis.com` endpoint, then
	// this URI must begin with `us-central1-docker.pkg.dev`. To use a custom
	// container, the AI Platform Google-managed service account
	// (/ai-platform/prediction/docs/custom-service-account#default) must have
	...
	// requirements for the Docker image itself, read Custom container requirements
	...
	// on this list. AI Platform Prediction also sends liveness and health checks
	// (/ai-platform/prediction/docs/custom-container-requirements#health) to this
	// port. If you do not specify this field, it defaults to following value:
	// ```json [ { "containerPort": 8080 } ] ``` AI Platform Prediction does not
	// use ports other than the first one listed. This field corresponds to the
...
type GoogleCloudMlV1__EncryptionConfig struct {
	...
	// encryption key used to protect a resource, such as a training job. It has
...
// available in a container. This message is a subset of the Kubernetes EnvVar
...
// for explaining model predictions. There are three feature attribution
// methods supported for TensorFlow models: integrated gradients, sampled
// Shapley, and XRAI. Learn more about feature attributions.
...
type GoogleCloudMlV1__ExplanationConfig struct {
	...
	// XraiAttribution: Attributes credit by computing the XRAI taking advantage of
	// the model's fully differentiable structure. Refer to this paper for more
	// details: https://arxiv.org/abs/1906.02825 Currently only implemented for
...
type GoogleCloudMlV1__GetConfigResponse struct {
	...
	// ServiceAccount: The service account Cloud ML uses to access resources in the
	// project.
...
// hyperparameter tuning trial from a training job. The TrainingOutput object
// that is returned on successful completion of a training job with
// hyperparameter tuning includes a list of HyperparameterOutput objects, one
...
type GoogleCloudMlV1__HyperparameterOutput struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AllMetrics") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__HyperparameterSpec struct {
	...
	// tuning job. Uses the default AI Platform hyperparameter tuning algorithm if
	// unspecified.
	//
	// Possible values:
	...
	// should exactly match what is shown in TensorBoard, including all scopes. For
	// versions of TensorFlow prior to 0.12, this should be only the tag passed to
	...
	// MaxFailedTrials: Optional. The number of failed trials that need to be seen
	// before failing the hyperparameter tuning job. You can specify this field to
	// override the default failing criteria for AI Platform hyperparameter tuning
	// jobs. Defaults to zero, which means the service decides when a
	...
	// tuning by adding trials in parallel. However, each trail only benefits from
	// the information gained in completed trials. That means that a trial does not
	// get access to the results of trials running at the same time, which could
	// reduce the quality of the overall optimization. Each trial will use the same
...
// computing the Aumann-Shapley value taking advantage of the model's fully
...
type GoogleCloudMlV1__IntegratedGradientsAttribution struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "NumIntegralSteps") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__Job struct {
	...
	// JobPosition: Output only. It's only effect when the job is in QUEUED state.
	// If it's positive, it indicates the job's position in the job scheduler. It's
	// 0 when the job is already scheduled.
	JobPosition int64 `json:"jobPosition,omitempty,string"`
	// Labels: Optional. One or more labels that you can add, to organize your
	// jobs. Each label is a key-value pair, where both the key and the value are
...
// ListTrials service method.
type GoogleCloudMlV1__ListOptimalTrialsRequest struct {
}

// GoogleCloudMlV1__ListOptimalTrialsResponse: The response message for the
...
type GoogleCloudMlV1__ListOptimalTrialsResponse struct {
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "Trials") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
type GoogleCloudMlV1__ListStudiesResponse struct {
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "Studies") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Studies") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__ManualScaling struct {
	...
	// operating this model will be proportional to `nodes` * number of hours since
...
type GoogleCloudMlV1__Measurement struct {
	// ElapsedTime: Output only. Time that the trial has been running at the point
	// of this measurement.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ElapsedTime") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// to calculate the desired nodes count when autoscaling is enabled.
type GoogleCloudMlV1__MetricSpec struct {
	...
	// metric deviates from the threshold by a certain percentage, the node count
	// changes.
...
// have multiple versions, each of which is a deployed, trained model ready to
// receive prediction requests. The model itself is just a container.
type GoogleCloudMlV1__Model struct {
	// DefaultVersion: Output only. The default version of the model. This version
	// will be used to handle prediction requests that do not specify a version.
	...
	// Etag: `etag` is used for optimistic concurrency control as a way to help
	// prevent simultaneous updates of a model from overwriting each other. It is
	// strongly suggested that systems make use of the `etag` in the
	// read-modify-write cycle to perform model updates in order to avoid race
	// conditions: An `etag` is returned in the response to `GetModel`, and systems
	// are expected to put that etag in the request to `UpdateModel` to ensure that
	// their change will be applied to the model as intended.
	Etag string `json:"etag,omitempty"`
	// Labels: Optional. One or more labels that you can add, to organize your
	// models. Each label is a key-value pair, where both the key and the value are
	// arbitrary strings that you supply. For more information, see the
	...
	// verbose than the standard access logs (see `onlinePredictionLogging`) and
	// can incur higher cost. However, they are helpful for debugging. Note that
	// logs may incur a cost (/stackdriver/pricing), especially if your project
	...
	// are sent to Cloud Logging. These logs are like standard server access logs,
	// containing information like timestamp and latency for each request. Note
	...
	// deployed. Only one region per model is supported. Defaults to 'us-central1'
	// if nothing is set. See the available regions for AI Platform services. Note:
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DefaultVersion") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__OperationMetadata struct {
	...
	// Labels: The user labels, inherited from the model or the model version being
	// operated on.
	Labels map[string]string `json:"labels,omitempty"`
...
type GoogleCloudMlV1__ParameterSpec struct {
	...
	// {`min_value`, `max_value`} will be ignored.
	Type string `json:"type,omitempty"`
	// ForceSendFields is a list of field names (e.g. "CategoricalValues") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CategoricalValues") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__PredictRequest struct {
	// HttpBody:  Required. The prediction request body. Refer to the request body
...
type GoogleCloudMlV1__PredictionInput struct {
	...
	// service will buffer batch_size number of records in memory before invoking
	// one Tensorflow prediction call internally. So take the record size and
	...
	// Region: Required. The Google Compute Engine region to run the prediction job
	// in. See the available regions for AI Platform services.
	Region string `json:"region,omitempty"`
	// RuntimeVersion: Optional. The AI Platform runtime version to use for this
	// batch prediction. If not set, AI Platform will pick the runtime version used
	...
	RuntimeVersion string `json:"runtimeVersion,omitempty"`
	// SignatureName: Optional. The name of the signature defined in the SavedModel
	// to use for this job. Please refer to SavedModel
	// (https://tensorflow.github.io/serving/serving_basic.html) for information
	// about how to use signatures. Defaults to DEFAULT_SERVING_SIGNATURE_DEF_KEY
	...
	// VersionName: Use this field if you want to specify a version of the model to
	// use. The string is formatted the same way as `model_version`, with the
	// addition of the version information:
...
// GoogleCloudMlV1__ReplicaConfig: Represents the configuration for a replica
// in a cluster.
type GoogleCloudMlV1__ReplicaConfig struct {
	...
	// command or args: The defaults defined in the Docker image are used. - If you
	// supply a command but no args: The default EntryPoint and the default Cmd
	// defined in the Docker image are ignored. Your command is run without any
	// arguments. - If you supply only args: The default Entrypoint defined in the
	// Docker image is run with the args that you supplied. - If you supply a
	// command and args: The default Entrypoint and the default Cmd defined in the
	// Docker image are ignored. Your command is run with your args. It cannot be
	// set if custom container image is not provided. Note that this field and
	...
	// run. If provided, it will override default ENTRYPOINT of the docker image.
	// If not provided, the docker image's ENTRYPOINT is used. It cannot be set if
	...
	// Otherwise, do not specify this field. This must be a runtime version that
	// currently supports training with TPUs
	// (/ml-engine/docs/tensorflow/runtime-version-list#tpu-support). Note that the
	...
	// patch version
	// (https://www.tensorflow.org/guide/version_compat#semantic_versioning_20). In
	// this field, you must specify the runtime version (TensorFlow minor version).
	// For example, if your custom container runs TensorFlow `1.x.y`, specify
	// `1.x`.
...
// request-response pairs to a BigQuery table. Online prediction requests to a
// model version and the responses to these requests are converted to raw
// strings and saved to the specified BigQuery table. Logging is constrained by
// BigQuery quotas and limits (/bigquery/quotas). If your project exceeds
// BigQuery quotas or limits, AI Platform Prediction does not log
// request-response pairs, but it continues to serve predictions. If you are
// using continuous evaluation (/ml-engine/docs/continuous-evaluation/), you do
// not need to specify this configuration manually. Setting up continuous
...
type GoogleCloudMlV1__RequestLoggingConfig struct {
	...
	// following format: " project_id.dataset_name.table_name" The specified table
	// must already exist, and the "Cloud ML Service Agent" for your project must
	// have permission to write to it. The table must have the following schema
	// (/bigquery/docs/schemas): Field name Type Mode model STRING REQUIRED
	...
	// fraction from 0 to 1. For example, if you want to log 10% of requests, enter
	// `0.1`. The sampling window is the lifetime of the model version. Defaults to
	// 0.
	SamplingPercentage float64 `json:"samplingPercentage,omitempty"`
	// ForceSendFields is a list of field names (e.g. "BigqueryTableName") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BigqueryTableName") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// container. AI Platform Prediction sends requests to these paths on the
// container; the custom container must run an HTTP server that responds to
// these requests with appropriate responses. Read Custom container
...
type GoogleCloudMlV1__RouteMap struct {
	...
	// Prediction intermittently sends GET requests to this path on the container's
	// IP address and port to check that the container is healthy. Read more about
	// health checks
	// (/ai-platform/prediction/docs/custom-container-requirements#checks). For
	// example, if you set this field to `/bar`, then AI Platform Prediction
	...
	// Platform Prediction forwards requests sent using projects.predict to this
	// path on the container's IP address and port. AI Platform Prediction then
	// returns the container's response in the API response. For example, if you
	// set this field to `/foo`, then when AI Platform Prediction receives a
	...
	// ForceSendFields is a list of field names (e.g. "Health") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
type GoogleCloudMlV1__SampledShapleyAttribution struct {
	// NumPaths: The number of feature permutations to consider when approximating
	// the Shapley values.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "NumPaths") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__Scheduling struct {
	...
	// seconds. The field can contain up to nine fractional digits, terminated by
	// `s`. If not specified, this field defaults to `604800s` (seven days). If the
	// training job is still running after this duration, AI Platform Training
	// cancels it. The duration is measured from when the job enters the `RUNNING`
	...
	// `gcloud` tool, you can specify this field in a `config.yaml` file
	// (/ai-platform/training/docs/training-jobs#formatting_your_configuration_param
	// eters). For example: ```yaml trainingInput: scheduling: maxRunningTime:
	// 7200s ```
	MaxRunningTime string `json:"maxRunningTime,omitempty"`
	// MaxWaitTime: Optional. The maximum job wait time, expressed in seconds. The
	// field can contain up to nine fractional digits, terminated by `s`. If not
	// specified, there is no limit to the wait time. The minimum for this field is
	// `1800s` (30 minutes). If the training job has not entered the `RUNNING`
	// state after this duration, AI Platform Training cancels it. After the job
	// begins running, it can no longer be cancelled due to the maximum wait time.
	...
	// maximum wait time cancellation. However, independently of this constraint,
	...
	// `config.yaml` file
	// (/ai-platform/training/docs/training-jobs#formatting_your_configuration_param
	// eters). For example: ```yaml trainingInput: scheduling: maxWaitTime: 3600s
	// ```
	MaxWaitTime string `json:"maxWaitTime,omitempty"`
	// Priority: Optional. Job scheduling will be based on this priority, which in
	// the range [0, 1000]. The bigger the number, the higher the priority. Default
	// to 0 if not set. If there are multiple jobs requesting same type of
	// accelerators, the high priority job will be scheduled prior to ones with low
	// priority.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "MaxRunningTime") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__StudyConfig struct {
	...
	//   "RANDOM_SEARCH" - Simple random search within the feasible space.
	Algorithm string `json:"algorithm,omitempty"`
	// AutomatedStoppingConfig: Configuration for automated stopping of unpromising
	// Trials.
...
// response field of a completed google.longrunning.Operation associated with a
...
// job. When using the gcloud command to submit your training job, you can
// specify the input parameters as command-line arguments and/or in a YAML
// configuration file referenced from the --config command-line argument. For
// details, see the guide to submitting a training job
// (/ai-platform/training/docs/training-jobs).
type GoogleCloudMlV1__TrainingInput struct {
	// Args: Optional. Command-line arguments passed to the training application
	// when it starts. If your job uses a custom container, then the arguments are
	...
	// keys (CMEK) to protect resources created by a training job, instead of using
	// Google's default encryption. If this is set, then all resources created by
	// the training job will be encrypted with the customer-managed encryption key
	// that you specify. Learn how and when to use CMEK with AI Platform Training
	// (/ai-platform/training/docs/cmek).
	EncryptionConfig *GoogleCloudMlV1__EncryptionConfig `json:"encryptionConfig,omitempty"`
	// EvaluatorConfig: Optional. The configuration for evaluators. You should only
	// set `evaluatorConfig.acceleratorConfig` if `evaluatorType` is set to a
	...
	// training job. Each replica in the cluster will be of the type specified in
	...
	// your training job's evaluator nodes. The supported values are the same as
	// those described in the entry for `masterType`. This value must be consistent
	...
	// TensorFlow program as the '--job-dir' command-line argument. The benefit of
	// specifying this field is that Cloud ML validates the path for use in
	// training.
	JobDir string `json:"jobDir,omitempty"`
	// MasterConfig: Optional. The configuration for your master worker. You should
	// only set `masterConfig.acceleratorConfig` if `masterType` is set to a
	// Compute Engine machine type. Learn about restrictions on accelerator
	// configurations for training.
	// (/ai-platform/training/docs/using-gpus#compute-engine-machine-types-with-gpu)
	...
	// MasterType: Optional. Specifies the type of virtual machine to use for your
	// training job's master worker. You must specify this field when `scaleTier`
	// is set to `CUSTOM`. You can use certain Compute Engine machine types
	...
	// See the list of legacy machine types
	// (/ai-platform/training/docs/machine-types#legacy-machine-types). Finally, if
	// you want to use a TPU for training, specify `cloud_tpu` in this field. Learn
	// more about the special configuration options for training with TPUs
	// (/ai-platform/training/docs/using-tpus#configuring_a_custom_tpu_machine).
	...
	// not peered with any network. Learn about using VPC Network Peering.
	// (/ai-platform/training/docs/vpc-peering).
	...
	// `parameterServerType` is set to a Compute Engine machine type. Learn about
	...
	// use for the training job. Each replica in the cluster will be of the type
	// specified in `parameter_server_type`. This value can only be used when
	// `scale_tier` is set to `CUSTOM`. If you set this value, you must also set
	// `parameter_server_type`. The default value is zero.
	ParameterServerCount int64 `json:"parameterServerCount,omitempty,string"`
	// ParameterServerType: Optional. Specifies the type of virtual machine to use
	// for your training job's parameter server. The supported values are the same
	// as those described in the entry for `master_type`. This value must be
	...
	// legacy machine types. This value must be present when `scaleTier` is set to
	...
	// either specify this field or specify `masterConfig.imageUri`. The following
	...
	// '2.7' is available when `runtime_version` is set to '1.15' or earlier. Read
	// more about the Python versions available for each runtime version
	// (/ml-engine/docs/runtime-version-list).
	PythonVersion string `json:"pythonVersion,omitempty"`
	// Region: Required. The region to run the training job in. See the available
	...
	// `masterConfig.imageUri`. For more information, see the runtime version list
	// (/ai-platform/training/docs/runtime-version-list) and learn how to manage
	// runtime versions (/ai-platform/training/docs/versioning).
	RuntimeVersion string `json:"runtimeVersion,omitempty"`
	// ScaleTier: Required. Specifies the machine types, the number of replicas for
	// workers and parameter servers.
	//
	// Possible values:
	//   "BASIC" - A single worker instance. This tier is suitable for learning how
	// to use Cloud ML, and for experimenting with new models using small datasets.
	...
	// _must_ set `TrainingInput.masterType` to specify the type of machine to use
	// for your master node. This is the only required setting. * You _may_ set
	...
	// ServiceAccount: Optional. The email address of a service account to use when
	// running the training appplication. You must have the
	// `iam.serviceAccounts.actAs` permission for the specified service account. In
	// addition, the AI Platform Training Google-managed service account must have
	// the `roles/iam.serviceAccountAdmin` role for the specified service account.
	// Learn more about configuring a service account.
	// (/ai-platform/training/docs/custom-service-account) If not specified, the AI
	...
	// Defaults to `false`. Learn more about this field.
	// (/ai-platform/training/docs/distributed-training-details#chief-versus-master)
	...
	// WorkerCount: Optional. The number of worker replicas to use for the training
	...
	// WorkerType: Optional. Specifies the type of virtual machine to use for your
	// training job's worker nodes. The supported values are the same as those
	// described in the entry for `masterType`. This value must be consistent with
	// the category of machine type that `masterType` uses. In other words, both
...
type GoogleCloudMlV1__TrainingOutput struct {
	...
	// HyperparameterMetricTag: The TensorFlow summary tag name used for optimizing
	// hyperparameter tuning trials. See
	// `HyperparameterSpec.hyperparameterMetricTag`
	// (#HyperparameterSpec.FIELDS.hyperparameter_metric_tag) for more information.
	...
	WebAccessUris map[string]string `json:"webAccessUris,omitempty"`
	// ForceSendFields is a list of field names (e.g. "BuiltInAlgorithmOutput") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BuiltInAlgorithmOutput") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoogleCloudMlV1__Trial struct {
	...
	// trial is infeasible. This should only be set if trial_infeasible is true.
	InfeasibleReason string `json:"infeasibleReason,omitempty"`
	// Measurements: A list of measurements that are strictly lexicographically
	// ordered by their induced tuples (steps, elapsed_time). These are used for
	...
	// service.
	State string `json:"state,omitempty"`
	// TrialInfeasible: Output only. If true, the parameters in this trial are not
	// attempted again.
	TrialInfeasible bool `json:"trialInfeasible,omitempty"`
...
// a trained model deployed in the cloud, ready to handle prediction requests.
// A model can have multiple versions. You can get information about all of the
...
type GoogleCloudMlV1__Version struct {
	...
	// prediction (beta). Only specify this field if you have specified a Compute
	// Engine (N1) machine type in the `machineType` field. Learn more about using
	// GPUs for online prediction
	// (/ml-engine/docs/machine-types-online-prediction#gpus).
	AcceleratorConfig *GoogleCloudMlV1__AcceleratorConfig `json:"acceleratorConfig,omitempty"`
	// AutoScaling: Automatically scale the number of nodes used to serve the model
	// in response to increases and decreases in traffic. Care should be taken to
	...
	// predictions. If you specify this field, then `machineType` is required. If
	// you specify this field, then `deploymentUri` is optional. If you specify
	// this field, then you must not specify `runtimeVersion`, `packageUris`,
	...
	CreateTime string `json:"createTime,omitempty"`
	// DeploymentUri: The Cloud Storage URI of a directory containing trained model
	// artifacts to be used to create the model version. See the guide to deploying
	...
	// model's version. Some explanation features require additional metadata to be
	...
	// `SCIKIT_LEARN`, `XGBOOST`. If you do not specify a framework, AI Platform
	// will analyze files in the deployment_uri to determine a framework. If you
	// choose `SCIKIT_LEARN` or `XGBOOST`, you must also set the runtime version of
	// the model to 1.4 or greater. Do **not** specify a framework if you're
	// deploying a custom prediction routine
	// (/ai-platform/prediction/docs/custom-prediction-routines) or if you're using
	...
	// Labels: Optional. One or more labels that you can add, to organize your
	// model versions. Each label is a key-value pair, where both the key and the
	// value are arbitrary strings that you supply. For more information, see the
	// documentation on using labels. Note that this field is not updatable for
	// mls1* models.
	Labels map[string]string `json:"labels,omitempty"`
	...
	// LastMigrationTime: Output only. The last time this version was successfully
	// migrated to AI Platform (Unified)
	// (https://cloud.google.com/ai-platform-unified/docs/start/migrating-to-ai-platform-unified).
	LastMigrationTime string `json:"lastMigrationTime,omitempty"`
	// LastUseTime: Output only. The time the version was last used for prediction.
	...
	// values for this field, read Choosing a machine type for online prediction
	// (/ai-platform/prediction/docs/machine-types-online-prediction). If this
	// field is not specified and you are using a regional endpoint
	// (/ai-platform/prediction/docs/regional-endpoints), then the machine type
	...
	// predictable billing. Beware that latency and error rates will increase if
	// the traffic exceeds that capability of the system to serve it based on the
	// selected number of nodes.
	ManualScaling *GoogleCloudMlV1__ManualScaling `json:"manualScaling,omitempty"`
	// Name: Required. The name specified for the version when it was created. The
	...
	// (/ml-engine/docs/tensorflow/custom-prediction-routines) or scikit-learn
	// pipelines with custom code
	// (/ml-engine/docs/scikit/exporting-for-prediction#custom-pipeline-code). For
	...
	// specify this field, you must also set `runtimeVersion`
	// (#Version.FIELDS.runtime_version) to 1.4 or greater.
	PackageUris []string `json:"packageUris,omitempty"`
	// PredictionClass: Optional. The fully qualified name (module_name.class_name)
	// of a class that implements the Predictor interface described in this
	// reference field. The module containing this class should be included in a
	// package provided to the `packageUris` field (#Version.FIELDS.package_uris).
	// Specify this field if and only if you are deploying a custom prediction
	// routine (beta) (/ml-engine/docs/tensorflow/custom-prediction-routines). If
	...
	// (/ml-engine/docs/machine-types-online-prediction). The following code sample
	// provides the Predictor interface: class Predictor(object): """Interface for
	// constructing custom predictors.""" def predict(self, instances, **kwargs):
	...
	// from_path(cls, model_dir): """Creates an instance of Predictor using the
	// given path. Loading of the predictor should be done in this method. Args:
	// model_dir: The local directory that contains the exported model file along
	// with any additional files uploaded when creating the version resource.
	// Returns: An instance implementing this Predictor class. """ raise
	// NotImplementedError() Learn more about the Predictor interface and custom
	...
	// Routes: Optional. Specifies paths on a custom container's HTTP server where
	// AI Platform Prediction sends certain requests. If you specify this field,
	...
	// following: ```json { "predict": "/v1/models/MODEL/versions/VERSION:predict",
	// "health": "/v1/models/MODEL/versions/VERSION" } ``` See RouteMap for more
	...
	// deployment. For more information, see the runtime version list
	// (/ml-engine/docs/runtime-version-list) and how to manage runtime versions
	// (/ml-engine/docs/versioning).
	RuntimeVersion string `json:"runtimeVersion,omitempty"`
	// ServiceAccount: Optional. Specifies the service account for resource access
	// control. If you specify this field, then you must also specify either the
...
// taking advantage of the model's fully differentiable structure. Refer to
// this paper for more details: https://arxiv.org/abs/1906.02825 Currently only
...
// of permissions. Example: { "audit_log_configs": [ { "log_type": "DATA_READ",
...
// GoogleRpc__Status: The `Status` type defines a logical error model that is
...
// Explain: Performs explanation on the data in the request. {% dynamic include
// "/ai-platform/includes/___explain-request" %}
//
//   - name: The resource name of a model or a version. Authorization: requires
...
// *GoogleApi__HttpBody.ServerResponse.Header or (if a response was returned at
...
// GetConfig: Get the service account information associated with your project.
// You need this information in order to grant the service account permissions
// for the Google Cloud Storage location where you put your model training code
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__GetConfigResponse.ServerResponse.Header or (if a response
...
// Predict: Performs online prediction on the data in the request. {% dynamic
// include "/ai-platform/includes/___predict-request" %}
//
//   - name: The resource name of a model or a version. Authorization: requires
//     the `predict` permission on the specified resource.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleProtobuf__Empty.ServerResponse.Header or (if a response was returned
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__Job.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// List: Lists the jobs in the project. If there are no jobs that match the
...
// Filter sets the optional parameter "filter": Specifies the subset of jobs to
// retrieve. You can filter on the value of one or more attributes of the job
// object. For example, retrieve jobs with a job identifier that starts with
// 'census': gcloud ai-platform jobs list --filter='jobId:census*' List all
...
// PageSize sets the optional parameter "pageSize": The number of jobs to
// retrieve per "page" of results. If there are more remaining results than
// this number, the response message will contain a valid value in the
// `next_page_token` field. The default value is 20, and the maximum page size
// is 100.
...
// the next page of results. You get the token from the `next_page_token` field
// of the response from the previous call.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__ListJobsResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Patch: Updates a specific job resource. Currently the only supported fields
// to update are `labels`.
//
// - name: The job name.
...
// path, relative to `Job`, of the field to update. To adopt etag mechanism,
// include `etag` field in the mask, and include the `etag` value in your job
// resource. For example, to change the labels of a job, the `update_mask`
// parameter would be specified as `labels`, `etag`, and the `PATCH` request
// body would specify the new value, as follows: { "labels": { "owner":
...
// Get: Get the complete list of CMLE capabilities in a location, along with
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__Location.ServerResponse.Header or (if a response was
...
//
//   - parent: The name of the project for which available locations are to be
...
// PageSize sets the optional parameter "pageSize": The number of locations to
// retrieve per "page" of results. If there are more remaining results than
// this number, the response message will contain a valid value in the
// `next_page_token` field. The default value is 20, and the maximum page size
// is 100.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__ListLocationsResponse.ServerResponse.Header or (if a
// response was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleLongrunning__Operation.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// StudyId sets the optional parameter "studyId": Required. The ID to use for
// the study, which will become the final component of the study's resource
// name.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__Study.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__ListStudiesResponse.ServerResponse.Header or (if a
// response was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// AddMeasurement: Adds a measurement of the objective metrics to a trial. This
// measurement is assumed to have been taken before the trial is complete.
//
// - name: The trial name.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__Trial.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// CheckEarlyStoppingState: Checks whether a trial should stop or not. Returns
// a long-running operation. When the operation is successful, it will contain
// a CheckTrialEarlyStoppingStateResponse.
//
// - name: The trial name.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__ListTrialsResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__ListOptimalTrialsResponse.ServerResponse.Header or (if a
// response was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// by AI Platform Vizier. Returns a long-running operation associated with the
...
// Create: Creates a model which will later contain one or more versions. You
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__Model.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Get: Gets information about a model, including its name, the description (if
...
// List: Lists the models in a project. Each project can contain multiple
...
// Filter sets the optional parameter "filter": Specifies the subset of models
// to retrieve.
...
// PageSize sets the optional parameter "pageSize": The number of models to
// retrieve per "page" of results. If there are more remaining results than
// this number, the response message will contain a valid value in the
// `next_page_token` field. The default value is 20, and the maximum page size
// is 100.
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudMlV1__ListModelsResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Patch: Updates a specific model resource. Currently the only supported
...
// UpdateMask sets the optional parameter "updateMask": Required. Specifies the
// path, relative to `Model`, of the field to update. For example, to change
// the description of a model to "foo" and set its default version to
...
// { "name":"version_1" } } Currently the supported update masks are
...
// Create: Creates a new version of a model from a trained TensorFlow model. If
...
// When you add a version to a model that already has one or more versions, the
...
// Delete: Deletes a model version. Each model can have multiple versions
...
//
//   - name: The name of the version. You can get the names of all the versions
...
// List: Gets basic information about all the versions of a model. If you
...
// retrieved in batches (called pages). If there are no versions that match the
// request parameters, the list request returns an empty response body: {}.
//
...
// PageSize sets the optional parameter "pageSize": The number of versions to
// retrieve per "page" of results. If there are more remaining results than
// this number, the response message will contain a valid value in the
// `next_page_token` field. The default value is 20, and the maximum page size
// is 100.
...
// UpdateMask sets the optional parameter "updateMask": Required. Specifies the
// path, relative to `Version`, of the field to update. Must be present and
// non-empty. For example, to change the description of a version to "foo", the
// `update_mask` parameter would be specified as `description`, and the `PATCH`
// request body would specify the new value, as follows: ``` { "description":
// "foo" } ``` Currently the only supported update mask fields are
// `description`, `requestLoggingConfig`, `autoScaling.minNodes`, and
// `manualScaling.nodes`. However, you can only update `manualScaling.nodes` if
// the version uses a Compute Engine (N1) machine type
// (/ml-engine/docs/machine-types-online-prediction).
...
// default version is used for prediction requests made against the model that
...
//
//   - name: The name of the version to make the default for the model. You can
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleLongrunning__ListOperationsResponse.ServerResponse.Header or (if a
// response was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
