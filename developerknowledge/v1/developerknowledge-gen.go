...
// Usage example:
//
//	import "google.golang.org/api/developerknowledge/v1"
//	...
...
package developerknowledge // import "google.golang.org/api/developerknowledge/v1"
...
const apiId = "developerknowledge:v1"
...
func (c *DocumentsBatchGetCall) doRequest(alt string) (*http.Response, error) {
	...
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1/documents:batchGet")
...
// Timestamps must be in RFC-3339 format, e.g., "2025-01-01T00:00:00Z". Note:
// Field names must be in `snake_case` (e.g., `data_source`). Values on the
// right-hand side of filtering expressions must be string literals enclosed in
// double quotes (e.g., "docs.cloud.google.com"). You can combine expressions
// using `AND`, `OR`, and `NOT` (or `-`) logical operators. `OR` has higher
// precedence than `AND`. Use parentheses for explicit precedence grouping.
// Examples: * `data_source = "docs.cloud.google.com" OR data_source =
...
func (c *DocumentsSearchDocumentChunksCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1/documents:searchDocumentChunks")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("GET", urls, nil)
	if err != nil {
		return nil, err
