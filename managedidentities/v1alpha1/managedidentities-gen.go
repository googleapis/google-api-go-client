...
// Domain: If the domain is being changed, it will be placed into the UPDATING
...
// Trust: Represents a relationship between two domains which makes it possible
...
// If the trust is being changed, it will be placed into the UPDATING state,
// which indicates that the resource is being reconciled. At this point, Get
// will reflect an intermediate state.
type Trust struct {
	// CreateTime: Output only. The time the instance was created.
...
// CheckMigrationPermission: AuditMigration API gets the current state of
// DomainMigration
//
...
// DomainName sets the optional parameter "domainName": The fully qualified
// domain name. e.g. mydomain.myorganization.com, with the following
...
// * Must end with a number or a letter. * Must not start with period. * Must
// be unique within the project. * First segment length (mydomain form example
...
// PageSize sets the optional parameter "pageSize": If not specified, a default
...
// the repeated paths field may only include these fields from Domain: *
...
// ResetAdminPassword: Resets managed identities admin password identified by
