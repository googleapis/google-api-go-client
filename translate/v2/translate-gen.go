...
func (r *DetectionsService) List(q []string) *DetectionsListCall {
	c := &DetectionsListCall{s: r.s, urlParams_: urlValues()}
	c.q = q
	return c
}
...
func (c *DetectionsListCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Del("q")
	for _, v := range c.q {
		c.urlParams_.Add("q", fmt.Sprintf("%v", v))
	}
...
func (r *LanguagesService) List() *LanguagesListCall {
	c := &LanguagesListCall{s: r.s, urlParams_: urlValues()}
	return c
}
...
func (c *LanguagesListCall) Target(target string) *LanguagesListCall {
	c.urlParams_.Set("target", fmt.Sprintf("%v", target))
	return c
}
...
func (r *TranslationsService) List(q []string, target string) *TranslationsListCall {
	c := &TranslationsListCall{s: r.s, urlParams_: urlValues()}
	c.q = q
	c.target = target
	return c
}
...
func (c *TranslationsListCall) Cid(cid []string) *TranslationsListCall {
	c.urlParams_.Del("cid")
	for _, v := range cid {
		c.urlParams_.Add("cid", fmt.Sprintf("%v", v))
	}
	return c
}
