...
type API struct {
	...
	responseTypes map[string]bool    // Only cache response types of GET methods.
...
func (meth *Method) generateCode() {
	...
	if httpMethod == "GET" {
		...
		pn("// Use googleapi.IsNotModified to check whether the response error from Do")
		pn("// is the result of In-None-Match.")
	...
	if retTypeComma != "" && !mapRetType {
		pn("// Exactly one of %v or error will be non-nil.", retTypeComma)
		pn("// Any non-2xx status code is an error.")
		pn("// Response headers are in either %v.ServerResponse.Header", retTypeComma)
		pn("// or (if a response was returned at all) in error.(*googleapi.Error).Header.")
		pn("// googleapi.IsNotModified can be called to check if http.StatusNotModified is returned.")
	}
