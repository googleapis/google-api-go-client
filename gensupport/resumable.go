...
// reportProgress calls a user-supplied callback to report that the number of uploaded bytes
// has increase by size bytes from off.  If size is 0, the callback is not called.
func (rx *ResumableUpload) reportProgress(off, size int64) {
	if size == 0 {
		return
	}
	rx.mu.Lock()
	rx.progress = off + size
	rx.mu.Unlock()
	if rx.Callback != nil {
		rx.Callback(off + size)
	}
}
...
func (rx *ResumableUpload) transferChunk(ctx context.Context) (*http.Response, error) {
	...
	if res.StatusCode == statusResumeIncomplete || res.StatusCode == http.StatusOK {
		rx.reportProgress(off, int64(size))
	}
