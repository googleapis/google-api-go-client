...
// Create: Creates a new VPC Flow Logs configuration. If a configuration with
...
// Filter sets the optional parameter "filter": Lists the `VpcFlowLogsConfig`s
// that match the filter expression. A filter expression filters the resources
// listed in the response. The expression must be of the form ` ` where
// operators: `<`, `>`, `<=`, `>=`, `!=`, `=`, `:` are supported (colon `:`
// represents a HAS operator which is roughly synonymous with equality). can
...
// target resource: - Configurations at the VPC network level
// target_resource.network:* - Configurations for a VPC network called `vpc-1`
// target_resource.network = vpc-1
...
// exact same settings already exists, the creation fails.
//
//   - name: Identifier. Unique name of the configuration using the form:
//     `projects/{project_id}/locations/global/vpcFlowLogs/{vpc_flow_log}`.
