...
type LongRunningRecognizeMetadata struct {
	// LastUpdateTime: Output only. Time of the most recent processing update.
	LastUpdateTime string `json:"lastUpdateTime,omitempty"`
	// ProgressPercent: Output only. Approximate percentage of audio processed thus
	// far. Guaranteed to be 100 when the audio is fully processed and the results
	// are available.
...
type LongRunningRecognizeResponse struct {
	// Results: Output only. Sequential list of transcription results corresponding
	// to sequential portions of audio.
...
type SpeechRecognitionAlternative struct {
	...
	// non-streaming result or, of a streaming result where `is_final=true`. This
...
type SpeechRecognitionResult struct {
	...
	// (up to the maximum specified in `max_alternatives`). These alternatives are
	...
	// ChannelTag: Output only. For multi-channel audio, this is the channel number
...
type WordInfo struct {
	...
	// `confidence` was not set.
	Confidence float64 `json:"confidence,omitempty"`
	// EndOffset: Output only. Time offset relative to the beginning of the audio,
	// and corresponding to the end of the spoken word. This field is only set if
	...
	// `diarization_config.enable_speaker_diarization` = `true` and only in the top
	// alternative.
	SpeakerTag int64 `json:"speakerTag,omitempty"`
	// StartOffset: Output only. Time offset relative to the beginning of the
	// audio, and corresponding to the start of the spoken word. This field is only
	// set if `enable_word_time_offsets=true` and only in the top hypothesis. This
