...
func TestConfigureResumableMedia(t *testing.T) {
	...
	const (
		...
		minChunk = 256 * 1 << 10 // GCS requirement: chunks are multiples of 256K
	)
	...
	h := func(r io.Reader, contentLength int64, mediaType string, chunkSize int64, isFakeRdr bool) {
		rx := ResumableUpload{}
		rx.Configure(r)
		if isFakeRdr != rx.FakeRdr {
			t.Errorf("FakeRdr = %v, want %v", rx.FakeRdr, isFakeRdr)
		}
		if _, ok := rx.Media.(FakeReaderAt); ok != rx.FakeRdr {
			t.Errorf("FakeRdr flag = %v, want %v", rx.FakeRdr, ok)
		}
