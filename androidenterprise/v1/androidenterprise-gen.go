...
// EnrollmentTokenDuration sets the optional parameter
// "enrollmentToken.duration": [Optional] The length of time the enrollment
// token is valid, ranging from 1 minute to `Durations.MAX_VALUE`
// (https://developers.google.com/protocol-buffers/docs/reference/java/com/google/protobuf/util/Durations.html#MAX_VALUE),
// approximately 10,000 years. If not specified, the default duration is 1
// hour.
func (c *EnterprisesCreateEnrollmentTokenCall) EnrollmentTokenDuration(enrollmentTokenDuration string) *EnterprisesCreateEnrollmentTokenCall {
	c.urlParams_.Set("enrollmentToken.duration", enrollmentTokenDuration)
	return c
}

// EnrollmentTokenEnrollmentTokenType sets the optional parameter
// "enrollmentToken.enrollmentTokenType": [Required] The type of the enrollment
// token.
//
// Possible values:
//
...
//	"userlessDevice" - The enrollment token is for a userless device.
//	"userDevice" - The enrollment token is for a user device.
func (c *EnterprisesCreateEnrollmentTokenCall) EnrollmentTokenEnrollmentTokenType(enrollmentTokenEnrollmentTokenType string) *EnterprisesCreateEnrollmentTokenCall {
	c.urlParams_.Set("enrollmentToken.enrollmentTokenType", enrollmentTokenEnrollmentTokenType)
	return c
}

// EnrollmentTokenToken sets the optional parameter "enrollmentToken.token":
...
func (c *EnterprisesCreateEnrollmentTokenCall) EnrollmentTokenToken(enrollmentTokenToken string) *EnterprisesCreateEnrollmentTokenCall {
	c.urlParams_.Set("enrollmentToken.token", enrollmentTokenToken)
	return c
}
...
func (c *EnterprisesCreateEnrollmentTokenCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "androidenterprise.enterprises.createEnrollmentToken", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
