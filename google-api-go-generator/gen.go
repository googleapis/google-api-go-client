...
func (meth *Method) generateCode() {
	...
	if meth.supportsMediaUpload() {
		...
		p("\tprotocol_  string\n")
		p("\tuploadOpts_ []googleapi.UploadOption\n")
		p("\tresumable_  *googleapi.ResumableUpload")
	}
	p("}\n")
	...
	if meth.supportsMediaUpload() {
		...
		pn("func (c *%s) Media(r io.Reader, opt ...googleapi.UploadOption) *%s {", callName, callName)
		pn("c.media_ = r")
		pn(`c.protocol_ = "multipart"`)
		pn("c.uploadOpts_ = opt")
		pn("return c")
		pn("}")
		pn("\n// ResumableMedia specifies the media to upload in chunks and can be cancelled with ctx.")
		pn("// At most one of Media and ResumableMedia may be set.")
		pn("func (c *%s) ResumableMedia(ctx context.Context, r io.Reader, opt ...googleapi.UploadOption) *%s {", callName, callName)
		pn("c.ctx_ = ctx")
		pn("c.media_ = r")
	...
	if meth.supportsMediaUpload() && httpMethod != "GET" {
		...
		pn(`  var cancel func()`)
		pn("  cancel, _ = googleapi.ConditionallyIncludeMedia(c.media_, c.uploadOpts_, &body, &ctype)")
	...
	if meth.supportsMediaUpload() {
		pn(`if c.protocol_ == "resumable" {`)
		pn(" c.resumable_ = &googleapi.ResumableUpload{")
		pn("  Client: c.s.client,")
		pn("  UserAgent: c.s.userAgent(),")
		pn(" }")
		pn(" mediaType, err := c.resumable_.Configure(c.media_, c.uploadOpts_...)")
		pn(" if err != nil {")
		pn("  return nil, err")
		pn(" }")
		pn(` req.Header.Set("X-Upload-Content-Type", mediaType)`)
	...
	if meth.supportsMediaUpload() {
		pn(`if c.protocol_ == "resumable" {`)
		pn(` c.resumable_.URI = res.Header.Get("Location")`)
		pn(" res, err = c.resumable_.Upload(c.ctx_)")
		pn(" if err != nil { return %serr }", nilRet)
