...
func TestResumableMedia(t *testing.T) {
	...
	t.Log("Resumable upload, buffered chunker, full content")
	...
	// bufio.Reader does not support io.ReaderAt, so content buffering is required
	// even though content length is known up-front. Also specify short
	// content. Chunking is expected.
	t.Log("Resumable upload, buffered chunker, short content")
