...
// Usage example:
//
//	import "google.golang.org/api/merchantapi/datasources/v1"
//	...
...
package merchantapi // import "google.golang.org/api/merchantapi/datasources/v1"
...
const apiId = "merchantapi:datasources_v1"
const apiName = "merchantapi"
const apiVersion = "datasources_v1"
...
type PrimaryProductDataSource struct {
	...
	// restriction.
	FeedLabel string `json:"feedLabel,omitempty"`
	// LegacyLocal: Optional. Immutable. Determines whether the products of this
	// data source are **only** targeting local destinations. Legacy local products
	// are prefixed with `local~` in the product resource ID. For example,
...
func (c *AccountsDataSourcesCreateCall) doRequest(alt string) (*http.Response, error) {
	...
	urls := googleapi.ResolveRelative(c.s.BasePath, "datasources/v1/{+parent}/dataSources")
...
func (c *AccountsDataSourcesDeleteCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "datasources/v1/{+name}")
	urls += "?" + c.urlParams_.Encode()
...
func (c *AccountsDataSourcesFetchCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "datasources/v1/{+name}:fetch")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
