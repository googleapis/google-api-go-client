...
func TestResumableMedia(t *testing.T) {
	...
	// Specify short content.
	t.Log("Resumable upload, io.ReaderAt, short content")
	handler = testResumableMediaHelper(t, received, strings.NewReader(data), googleapi.SetMediaType("text/html; charset=utf-8"), googleapi.SetContentSize(dataLen-1))
	verifyResumableMediaRequest(t, handler.reqs[1], handler.bodies[1],
		"text/html; charset=utf-8",
		fmt.Sprintf("bytes %v-%v/%v", 0, dataLen-2, dataLen-1),
		dataLen-1,
		data[:dataLen-1])
	verifyResumableMediaRequest(t, handler.r, handler.body,
		"text/html; charset=utf-8",
	...
	handler = testResumableMediaHelper(t, received, bufio.NewReader(strings.NewReader(data)),
		googleapi.SetContentSize(dataLen-1))
	verifyResumableMediaRequest(t, handler.reqs[1], handler.bodies[1],
		"text/plain; charset=utf-8",
		fmt.Sprintf("bytes %v-%v/*", 0, chunkSize-1),
		chunkSize,
		data[:chunkSize])
	verifyResumableMediaRequest(t, handler.r, handler.body,
		"text/plain; charset=utf-8",
