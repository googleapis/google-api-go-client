...
func (t *Type) MapElementType() *Type {
	if t.Kind() != MapKind {
		panicf("MapElementType() called on %s", prettyJSON(t.m))
	}
...
func (t *Type) ArrayElementType() *Type {
	if t.Kind() != ArrayKind {
		panicf("ArrayElementType() called on %s", prettyJSON(t.m))
	}
