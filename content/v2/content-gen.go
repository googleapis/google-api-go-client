...
// Usage example:
//
//   import "code.google.com/p/google-api-go-client/content/v2"
//   ...
...
type DatafeedStatus struct {
	...
	ItemsValid int64 `json:"itemsValid,omitempty"`
...
type Product struct {
	...
	// price.
	UnitPricingBaseMeasure string `json:"unitPricingBaseMeasure,omitempty"`
...
type ProductInstallment struct {
	...
	Months uint64 `json:"months,omitempty,string"`
}
...
func (c *AccountsDeleteCall) Do() error {
	...
	req.URL.Path = strings.Replace(req.URL.Path, "{merchantId}", strconv.FormatUint(c.merchantId, 10), 1)
	req.URL.Path = strings.Replace(req.URL.Path, "{accountId}", strconv.FormatUint(c.accountId, 10), 1)
	googleapi.SetOpaque(req.URL)
	req.Header.Set("User-Agent", "google-api-go-client/0.5")
	res, err := c.s.client.Do(req)
	if err != nil {
...
func (c *DatafeedsDeleteCall) Do() error {
	...
	req, _ := http.NewRequest("DELETE", urls, body)
	req.URL.Path = strings.Replace(req.URL.Path, "{merchantId}", strconv.FormatUint(c.merchantId, 10), 1)
	req.URL.Path = strings.Replace(req.URL.Path, "{datafeedId}", strconv.FormatUint(c.datafeedId, 10), 1)
	googleapi.SetOpaque(req.URL)
	req.Header.Set("User-Agent", "google-api-go-client/0.5")
	res, err := c.s.client.Do(req)
	if err != nil {
...
func (c *InventorySetCall) Do() (*InventorySetResponse, error) {
	...
	req, _ := http.NewRequest("POST", urls, body)
	req.URL.Path = strings.Replace(req.URL.Path, "{merchantId}", strconv.FormatUint(c.merchantId, 10), 1)
	req.URL.Path = strings.Replace(req.URL.Path, "{storeCode}", url.QueryEscape(c.storeCode), 1)
