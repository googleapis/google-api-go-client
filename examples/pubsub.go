...
func listTopics(service *pubsub.Service, argv []string) {
	...
	for {
		query := service.Projects.Topics.List(fmt.Sprintf("projects/%s", argv[0])).PageToken(nextPageToken)
...
func deleteTopic(service *pubsub.Service, argv []string) {
	...
	_, err := service.Projects.Topics.Delete(topicName).Do()
	if err != nil {
...
func listSubscriptions(service *pubsub.Service, argv []string) {
	var nextPageToken string = ""
	for {
		query := service.Projects.Subscriptions.List(fmt.Sprintf("projects/%s", argv[0])).PageToken(nextPageToken)
...
func deleteSubscription(service *pubsub.Service, argv []string) {
	checkArgs(argv, 3)
	name := fullSubName(argv[0], argv[2])
	_, err := service.Projects.Subscriptions.Delete(name).Do()
	if err != nil {
...
func connectIRC(service *pubsub.Service, argv []string) {
	...
	for {
		...
		if len(parts) > 0 && parts[0] == "PING" {
			...
			_, err := service.Projects.Topics.Publish(topicName, publishRequest).Do()
			if err != nil {
...
func pullMessages(service *pubsub.Service, argv []string) {
	...
	for {
		...
		for _, receivedMessage := range pullResponse.ReceivedMessages {
			...
			_, err = service.Projects.Subscriptions.Acknowledge(subName, ackRequest).Do()
			if err != nil {
