...
type Domain struct {
	...
	// Engine networks to which the instance is connected. Network can be
	// added using UpdateDomain later. Domain is only available on network
	// part of authorized_networks. Caller needs to make sure that CIDR
	// subnets do not overlap between networks, else domain creation will
	// fail.
...
// https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc731335(v%3dws.10) If the trust is being changed, it will be placed into the UPDATING state, which indicates that the resource is being reconciled. At this point, Get will reflect an intermediate
// state.
type Trust struct {
