...
func (meth *Method) generateCode() {
	...
	if meth.supportsMediaUpload() {
		...
		pn(" if !opts.ChunkSizeSet {")
		pn("   chunkSize = %d", defaultUploadChunkSize)
		pn(" }")
		...
		comment = "ResumableMedia specifies the media to upload in chunks and can be canceled with ctx. " +
			"ResumableMedia is deprecated in favour of Media. " +
			"At most one of Media and ResumableMedia may be set. " +
		...
		pn(" c.resumableBuffer_ = gensupport.NewResumableBuffer(rdr, %d)", defaultUploadChunkSize)
		pn(" c.media_ = nil")
