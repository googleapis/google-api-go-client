...
// intervals depend on the MetricKind of the metric value. The end time
// must not be earlier than the start time. When writing data points,
// the start time must not be more than 25 hours in the past and the end
// time must not be more than five minutes in the future. For GAUGE
// metrics, the startTime value is technically optional; if no value is
// specified, the start time defaults to the value of the end time, and
// the interval represents a single point in time. If both start and end
...
// CUMULATIVE metrics, the start time and end time must specify a a
...
// the parent container in which the alerting policy will be written,
// not the name of the created policy. |name| must be a host project of
// a workspace, otherwise INVALID_ARGUMENT error will return. The
// alerting policy that is returned will have a name that contains a
// normalized representation of this name as a prefix but adds a suffix
// of the form /alertPolicies/[ALERT_POLICY_ID], identifying the policy
// in the container.
...
// projects/[PROJECT_ID_OR_NUMBER]/monitoredResourceDescriptors/[RESOURCE
// _TYPE] The [RESOURCE_TYPE] is a predefined type, such as
// cloudsql_database.
...
// is:
// projects/[PROJECT_ID_OR_NUMBER]/notificationChannelDescriptors/[CHANNE
// L_TYPE]
...
// channel. The resulting channel's name will have a normalized version
// of this field as a prefix, but will add
