...
type Queue struct {
	...
	// queue that were created using Cloud Tasks. Optionally, retry settings can be
	// set on individual tasks and override the queue-level retry settings for the
	// task. * For tasks created using the App Engine SDK: the queue-level retry
...
// BatchCreate: Creates a batch of tasks and adds them to a queue. This call is
// not atomic. All tasks must be for the same queue. A maximum of 100 tasks can
// be created in a single batch.
//
...
func (c *ProjectsLocationsQueuesTasksBatchCreateCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v2beta3/{+parent}/tasks:batchCreate")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
...
func (c *ProjectsLocationsQueuesTasksBatchDeleteCall) doRequest(alt string) (*http.Response, error) {
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v2beta3/{+parent}/tasks:batchDelete")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
