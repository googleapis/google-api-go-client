...
type AttachedDisk struct {
	...
	//
	// If you are creating a new disk, this field encrypts the disk using a
	// customer-supplied encryption key. If you are attaching an existing
	// disk that is already encrypted, this field decrypts the disk using
	// the customer-supplied encryption key.
	//
	// If you encrypt a new disk using a customer-supplied encryption key,
	// you must provide the same key again when attempting to use this
	// resource at a later time (e.g. to create a snapshot or image from the
	// disk or to attach the disk to a virtual machine).
	//
	// If no customer-supplied encryption key is provided at creation, then
...
type Disk struct {
	...
	// encryption key.
	//
	// If you encrypt the disk using a customer-supplied encryption key, and
	// you want to use the disk later (e.g. to create a disk snapshot or an
	// image, or to attach the disk to a virtual machine), you must provide
	// the same key in your request. If you provide an incorrect key, or no
...
// a template for how individual VMs should be checked for health, via
// HTTPS.
...
type Image struct {
	...
	// encryption key.
	//
	// If you encrypt an image using a customer-supplied encryption key, and
	// you want to use the image later (e.g. to create a disk from the
	// image), you must provide the same key used encrypt the image in your
	// future request. If you provide an incorrect key, or no key, the
	// operation will fail.
	//
	// Customer-supplied encryption keys do not protect access to metadata
	// of the disk.
...
type Snapshot struct {
	...
	// customer-supplied encryption key.
	//
	// If you encrypt a snapshot using a customer-supplied encryption key,
	// and you want to use the snapshot later (e.g. to create a disk from
	// the snapshot), you must provide the same key used encrypt the
	// snapshot in your future request. If you provide an incorrect key, or
	// no key, the request will fail.
	//
	// Customer-supplied encryption keys do not protect access to metadata
	// of the disk.
	//
