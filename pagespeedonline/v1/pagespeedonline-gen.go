...
const basePathTemplate = "https://www.UNIVERSE_DOMAIN/pagespeedonline/v1/"
...
type Result struct {
	...
	// FormattedResults: Localized PageSpeed results. Contains a ruleResults entry
	// for each PageSpeed rule instantiated and run by the server.
	...
	// InvalidRules: List of rules that were specified in the request, but which
	// the server did not know how to instantiate.
	InvalidRules []string `json:"invalidRules,omitempty"`
	// Kind: Kind of result.
	Kind string `json:"kind,omitempty"`
	// PageStats: Summary statistics for the page, such as number of JavaScript
	// bytes, number of HTML bytes, etc.
	PageStats *ResultPageStats `json:"pageStats,omitempty"`
	// ResponseCode: Response code for the document. 200 indicates a normal page
	// load. 4xx/5xx indicates an error.
	ResponseCode int64 `json:"responseCode,omitempty"`
	// Score: The PageSpeed Score (0-100), which indicates how much faster a page
	// could be. A high score indicates little room for improvement, while a lower
	...
	// NullFields is a list of field names (e.g. "CaptchaResult") to include in API
...
// ResultFormattedResults: Localized PageSpeed results. Contains a ruleResults
...
type ResultFormattedResults struct {
	...
	// RuleResults: Dictionary of formatted rule results, with one entry for each
...
// ResultFormattedResultsRuleResults: The enum-like identifier for this rule.
// For instance "EnableKeepAlive" or "AvoidCssImport". Not localized.
type ResultFormattedResultsRuleResults struct {
	// LocalizedRuleName: Localized name of the rule, intended for presentation to
	// a user.
	LocalizedRuleName string `json:"localizedRuleName,omitempty"`
	// RuleImpact: The impact (unbounded floating point value) that implementing
	// the suggestions for this rule would have on making the page faster. Impact
	// is comparable between rules to determine which rule's suggestions would have
	// a higher or lower impact on making a page faster. For instance, if enabling
	...
	// UrlBlocks: List of blocks of URLs. Each block may contain a heading and a
	// list of URLs. Each URL may optionally include additional details.
	UrlBlocks []*ResultFormattedResultsRuleResultsUrlBlocks `json:"urlBlocks,omitempty"`
	// ForceSendFields is a list of field names (e.g. "LocalizedRuleName") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "LocalizedRuleName") to include in
...
type ResultFormattedResultsRuleResultsUrlBlocks struct {
	...
	// Urls: List of entries that provide information about URLs in the url block.
	// Optional.
...
// ResultFormattedResultsRuleResultsUrlBlocksHeader: Heading to be displayed
...
type ResultFormattedResultsRuleResultsUrlBlocksHeader struct {
	...
	// Format: A localized format string with $N placeholders, where N is the
	// 1-indexed argument number, e.g. 'Minifying the following $1 resources would
	// save a total of $2 bytes'.
...
type ResultFormattedResultsRuleResultsUrlBlocksHeaderArgs struct {
	// Type: Type of argument. One of URL, STRING_LITERAL, INT_LITERAL, BYTES, or
	// DURATION.
...
type ResultFormattedResultsRuleResultsUrlBlocksUrls struct {
	// Details: List of entries that provide additional details about a single URL.
	// Optional.
	Details []*ResultFormattedResultsRuleResultsUrlBlocksUrlsDetails `json:"details,omitempty"`
	// Result: A format string that gives information about the URL, and a list of
...
type ResultFormattedResultsRuleResultsUrlBlocksUrlsDetails struct {
	...
	// Format: A localized format string with $N placeholders, where N is the
	// 1-indexed argument number, e.g. 'Unnecessary metadata for this resource adds
	// an additional $1 bytes to its download size'.
	Format string `json:"format,omitempty"`
	// ForceSendFields is a list of field names (e.g. "Args") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
...
// ResultFormattedResultsRuleResultsUrlBlocksUrlsResult: A format string that
// gives information about the URL, and a list of arguments for that format
// string.
type ResultFormattedResultsRuleResultsUrlBlocksUrlsResult struct {
	...
	// Format: A localized format string with $N placeholders, where N is the
	// 1-indexed argument number, e.g. 'Minifying the resource at URL $1 can save
	// $2 bytes'.
	Format string `json:"format,omitempty"`
	// ForceSendFields is a list of field names (e.g. "Args") to unconditionally
	// include in API requests. By default, fields with empty or default values are
...
type ResultPageStats struct {
	// CssResponseBytes: Number of uncompressed response bytes for CSS resources on
	// the page.
	CssResponseBytes int64 `json:"cssResponseBytes,omitempty,string"`
	// FlashResponseBytes: Number of response bytes for flash resources on the
	// page.
	FlashResponseBytes int64 `json:"flashResponseBytes,omitempty,string"`
	// HtmlResponseBytes: Number of uncompressed response bytes for the main HTML
	// document and all iframes on the page.
	HtmlResponseBytes int64 `json:"htmlResponseBytes,omitempty,string"`
	// ImageResponseBytes: Number of response bytes for image resources on the
	// page.
	...
	// NumberJsResources: Number of JavaScript resources referenced by the page.
	...
	// NumberStaticResources: Number of static (i.e. cacheable) resources on the
	// page.
	NumberStaticResources int64 `json:"numberStaticResources,omitempty"`
	// OtherResponseBytes: Number of response bytes for other resources on the
	// page.
	OtherResponseBytes int64 `json:"otherResponseBytes,omitempty,string"`
	// TextResponseBytes: Number of uncompressed response bytes for text resources
	// not covered by other statistics (i.e non-HTML, non-script, non-CSS
	// resources) on the page.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CssResponseBytes") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// returns a PageSpeed score, a list of suggestions to make that page faster,
...
// "filter_third_party_resources": Indicates if third party resources should be
...
// Rule sets the optional parameter "rule": A PageSpeed rule to run; if none
// are given, all rules are run
...
// Screenshot sets the optional parameter "screenshot": Indicates if binary
// data containing a screenshot should be included
...
// Strategy sets the optional parameter "strategy": The analysis strategy to
// use
//
// Possible values:
//
...
// *Result.ServerResponse.Header or (if a response was returned at all) in
