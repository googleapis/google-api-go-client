...
type WebApp struct {
	...
	// one element.
	Icons []*WebAppImage `json:"icons,omitempty"`

	// Name: The name of the web application as it is usually displayed to
	// the user (e.g., amongst a list of other applications, or as a label
	// for an icon). Required.
	Name string `json:"name,omitempty"`

	// StartUrl: String that represents the start URL, i.e. the URL that
	// should load when the user opens the application. Required.
...
// WebAppImage: Each image object represents an image that is used as
// part of a web application, suitable to use in various contexts
// depending on the semantics of the member that is using the object
// (e.g., an icon that is part of an application menu, etc.).
type WebAppImage struct {
	// ImageData: The actual bytes of the image.
	ImageData string `json:"imageData,omitempty"`

	// Purposes: The purposes this image may be used for. Optional.
...
func (s *WebAppImage) MarshalJSON() ([]byte, error) {
	type NoMethod WebAppImage
	raw := NoMethod(*s)
	return gensupport.MarshalJSON(raw, s.ForceSendFields, s.NullFields)
}
