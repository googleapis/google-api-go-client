...
type ApplicationReport struct {
	...
	// SigningKeyCertFingerprints: Deprecated. Use signingKeyCerts instead. The
	// SHA-1 hash of each android.content.pm.Signature
...
type EnrollmentToken struct {
	...
	// GoogleAuthenticationOptions: Optional. Options related to Google
	// authentication during the enrollment.
	GoogleAuthenticationOptions *GoogleAuthenticationOptions `json:"googleAuthenticationOptions,omitempty"`
...
// GoogleAuthenticationOptions: Options for Google authentication during the
// enrollment.When triggering the enrollment with a SigninDetail, these options
// are enforced after the user completes third-party sign-in and an
// EnrollmentToken is created. If this token's authentication_requirement is
// set to REQUIRED, these options interact with the
// SigninDetail.googleAuthenticationOptions that initiated the flow in the
...
// will be prompted to sign in again. - If required_account_email is set on
// this token and the user signed in with a different email earlier, an error
// will occur and the user will be asked to sign in again with the correct
// account.
type GoogleAuthenticationOptions struct {
	// AuthenticationRequirement: Optional. Specifies whether user should
	// authenticate with Google during enrollment. If this is set to any value
	// other than AUTHENTICATION_REQUIREMENT_UNSPECIFIED, the enterprise-level
	// setting googleAuthenticationSettings is ignored for devices enrolled with
	// this token.
	//
	// Possible values:
	...
	// belongs to is used to determine whether the user needs to authenticate with
	// Google during enrollment.
	...
	// the user must use during enrollment. This field can only be set if
	// AuthenticationRequirement is set to REQUIRED.
...
func (s GoogleAuthenticationOptions) MarshalJSON() ([]byte, error) {
	type NoMethod GoogleAuthenticationOptions
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}
...
type SigninDetail struct {
	...
	// GoogleAuthenticationOptions: Optional. Options related to Google
	// authentication during the enrollment.
	GoogleAuthenticationOptions *SigninDetailGoogleAuthenticationOptions `json:"googleAuthenticationOptions,omitempty"`
	// QrCode: A JSON string whose UTF-8 representation can be used to generate a
	// QR code to enroll a device with this enrollment token. To enroll a device
	// using NFC, the NFC record must contain a serialized java.util.Properties
...
// SigninDetailGoogleAuthenticationOptions: Options for Google authentication
// during the enrollment.These options control whether the Google
// authentication screen is shown, and whether it can be skipped, at the start
// of the sign-in flow. More requirements can be enforced by
// EnrollmentToken.googleAuthenticationOptions on the EnrollmentToken that is
// created later.
type SigninDetailGoogleAuthenticationOptions struct {
	// AuthenticationRequirement: Optional. Specifies whether user should
	// authenticate with Google during enrollment. If this is set to any value
	// other than AUTHENTICATION_REQUIREMENT_UNSPECIFIED, the enterprise-level
	// setting googleAuthenticationSettings is ignored for devices enrolled with
...
func (s SigninDetailGoogleAuthenticationOptions) MarshalJSON() ([]byte, error) {
	type NoMethod SigninDetailGoogleAuthenticationOptions
	return gensupport.MarshalJSON(NoMethod(s), s.ForceSendFields, s.NullFields)
}
