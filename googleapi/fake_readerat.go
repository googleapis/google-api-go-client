...
// fakeReaderAt implements io.ReaderAt interface, but with a different contract.
...
//
// Also provided is SizeAt call which reads from and buffers the underlying stream,
// and return the same result that ReadAt() would have returned if called, but
// without requiring the caller to provide a []byte to copy the data
// into. Furthermore, the EOF semantics is stricter than for the normal io.ReaderAt:
// fakeReaderAt is guaranteed to return io.EOF along with the last byte of content
// from the underlying stream. In other words, if ReadAt() or SizeAt() return err =
// nil, that means that there is at least one more byte to be read.
//
