...
func (meth *Method) generateCode() {
	...
	pn(`var reqHeaders = map[string]string{"User-Agent":c.s.userAgent()}`)
	if httpMethod == "GET" {
		pn(`if c.ifNoneMatch_ != "" {`)
		pn(` reqHeaders["If-None-Match"] =  c.ifNoneMatch_`)
		pn("}")
	}
	...
	if ba := args.bodyArg(); ba != nil && httpMethod != "GET" {
		...
		pn(`reqHeaders["Content-Type"] = "application/json"`)
	}
	...
	if meth.supportsMediaUpload() {
		...
		pn(`  reqHeaders["Content-Type"] = ctype`)
		pn("  body = combined")
		pn("}")
		pn(`if c.resumableBuffer_ != nil && c.mediaType_ != ""{`)
		pn(` reqHeaders["X-Upload-Content-Type"] = c.mediaType_`)
		pn("}")
	}
