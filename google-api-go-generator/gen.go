...
func (s *Schema) writeSchemaMarshal(forceSendFieldName string) {
	comment := fmt.Sprintf("MarshalJSON implements json.Marshaler. It encodes s as a JSON struct in the usual way, but additionally respects s.%s.", forceSendFieldName)
	s.api.p("%s", asComment("", comment))
