...
type ResumableUpload struct {
	...
	Limited bool
	// Limited specifies whether Media is actually a FakeReaderAt
	MediaType string
...
func (rx *ResumableUpload) transferChunks(ctx context.Context) (*http.Response, error) {
	...
	for {
		...
		var reqSize int64
		if rx.Limited { // we have LimitedReaderAt
		...
		if rx.ContentLength <= 0 {
			req.Header.Set("Content-Range", fmt.Sprintf("bytes %v-%v/%v", start, start+reqSize-1, "*"))
		} else {
