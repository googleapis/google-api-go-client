...
type BackgroundJobLogEntry struct {
	...
	// This state will be used when job is not yet finished.
	//   "SUCCEEDED" - Success.
	//   "FAILED" - Error.
...
type ColumnEntity struct {
	...
	// is empty if set values is not required
...
type ConstraintEntity struct {
	...
	// constraint. eg: if the constraint is a FOREIGN_KEY, this represents
	...
	// constraint. eg: if the constraint is a FOREIGN_KEY, this represents
	// the list of full name of the referenced table by the foreign key.
	ReferenceTable string `json:"referenceTable,omitempty"`

	// TableColumns: Table columns used as part of the Constraint for e.g.
	...
	// Type: Type of constraint - e.g. unique, primary key, foreign key
...
type DatabaseEngineInfo struct {
	...
	// Version: Required. Engine named version, for e.g. 12.c.1
...
type EntityMapping struct {
	...
	// logged along the with the reason preventing them to do so.
...
type ImportMappingRulesRequest struct {
	...
	//   "IMPORT_RULES_FILE_FORMAT_HARBOUR_BRIDGE_SESSION_FILE" - Harbour
	// Bridge Session file
...
type IndexEntity struct {
	...
	// TableColumns: Table columns used as part of the Index for e.g. B-TREE
	// index should list the columns which constitutes the index.
	TableColumns []string `json:"tableColumns,omitempty"`

	// Type: Type of index - e.g. B-TREE
	Type string `json:"type,omitempty"`
...
type RulesFile struct {
	...
	// converted. This is used mainly so future logs of the import rules job
	// will contain this detail and can therefore be searched by it later
...
// parent entity DatabaseInstance (for database engines which supports
// it). For some database engines the term schema and user can be used
...
type TriggerEntity struct {
	...
	// TriggerType: Indicates when the trigger fires, e.g. BEFORE STATEMENT,
	// AFTER EACH ROW
	TriggerType string `json:"triggerType,omitempty"`

	// TriggeringEvents: The DML, DDL, or database events that fires the
	// trigger, e.g. INSERT, UPDATE
...
type VpcPeeringConfig struct {
	...
	// VpcName: Required. Fully qualified name of the VPC DMS will peer to.
...
// Apply: Apply draft tree onto a specific destination database
//
//   - name: Name of the conversion workspace resource to apply draft to
...
//     whose revisions are listed. in the form of:
//     projects/{project}/locations/{location}/conversionWorkspaces/{conver
//     sion_workspace}.
...
// DescribeDatabaseEntities: Use this method to describe the database
// entities tree for a specific conversion workspace and a specific tree
// type. The DB Entities are not a resource like conversion workspace or
// mapping rule, and they can not be created, updated or deleted like
// one. Instead they are simple data objects describing the structure of
// the client database.
//
//   - conversionWorkspace: Name of the conversion workspace resource
//     whose DB entities are described in the form of:
//     projects/{project}/locations/{location}/conversionWorkspaces/{conver
//     sion_workspace}.
...
// this value. If unspecified, at most 50 sets will be returned.
...
// Rollback: Rollbacks a conversion workspace to the last committed
// spanshot.
//
// - name: Name of the conversion workspace resource to rollback to.
...
// SearchBackgroundJobs: Use this method to search/list the background
// jobs for a specific conversion workspace. The background jobs are not
// a resource like conversion workspace or mapping rule, and they can
// not be created, updated or deleted like one. Instead they are a way
// to expose the data plane jobs log.
//
//   - conversionWorkspace: Name of the conversion workspace resource whos
...
// If supplied, will only return jobs that completed until (not
...
// unspecified, at most 100 jobs will be returned. The maximum value is
...
// connections that will be returned. The maximum value is 1000; values
