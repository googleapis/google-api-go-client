...
type Action struct {
	...
	// pipeline to fail. This flag allows execution of other actions to continue
	// instead.
	//   "RUN_IN_BACKGROUND" - This flag allows an action to continue running in
	// the background while executing subsequent actions. This is useful to provide
	...
	// run. This flag indicates that this action must be run even if the pipeline
	// has already failed. This is useful for actions that copy output files off of
	...
	// filesystem. This has the effect of causing the container to be executed with
	// `CAP_SYS_ADMIN` and exposes `/dev/fuse` to the container, so use it only for
	// containers you trust.
	//   "PUBLISH_EXPOSED_PORTS" - Exposes all ports specified by `EXPOSE`
	// statements in the container. To discover the host side port numbers, consult
	// the `ACTION_STARTED` event in the operation metadata.
	//   "DISABLE_IMAGE_PREFETCH" - All container images are typically downloaded
	// before any actions are executed. This helps prevent typos in URIs or issues
	// like lack of disk space from wasting large amounts of compute resources. If
	// set, this flag prevents the worker from downloading the image until just
	...
	// Name: An optional name for the container. The container hostname will be set
	// to this name, making it useful for inter-container communication. The name
