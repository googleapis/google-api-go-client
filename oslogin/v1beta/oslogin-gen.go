...
func NewUsersService(s *Service) *UsersService {
	...
	rs.SshPublicKey = NewUsersSshPublicKeyService(s)
...
type UsersService struct {
	...
	SshPublicKey *UsersSshPublicKeyService
...
func NewUsersSshPublicKeyService(s *Service) *UsersSshPublicKeyService {
	rs := &UsersSshPublicKeyService{s: s}
	return rs
}

type UsersSshPublicKeyService struct {
	s *Service
}
...
// method id "oslogin.users.sshPublicKey.create":

type UsersSshPublicKeyCreateCall struct {
	s            *Service
	parent       string
	sshpublickey *SshPublicKey
	urlParams_   gensupport.URLParams
...
//
// - parent: The unique ID for the user in format `users/{user}`.
func (r *UsersSshPublicKeyService) Create(parent string, sshpublickey *SshPublicKey) *UsersSshPublicKeyCreateCall {
	c := &UsersSshPublicKeyCreateCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.parent = parent
	c.sshpublickey = sshpublickey
	return c
}
...
// https://developers.google.com/gdata/docs/2.0/basics#PartialResponse
// for more information.
func (c *UsersSshPublicKeyCreateCall) Fields(s ...googleapi.Field) *UsersSshPublicKeyCreateCall {
	c.urlParams_.Set("fields", googleapi.CombineFields(s))
	return c
}
...
// pending HTTP request will be aborted if the provided context is
// canceled.
func (c *UsersSshPublicKeyCreateCall) Context(ctx context.Context) *UsersSshPublicKeyCreateCall {
	c.ctx_ = ctx
	return c
}
...
// Header returns an http.Header that can be modified by the caller to
// add HTTP headers to the request.
func (c *UsersSshPublicKeyCreateCall) Header() http.Header {
	if c.header_ == nil {
		c.header_ = make(http.Header)
	}
	return c.header_
...
func (c *UsersSshPublicKeyCreateCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := make(http.Header)
	reqHeaders.Set("x-goog-api-client", "gl-go/"+gensupport.GoVersion()+" gdcl/20211113")
	for k, v := range c.header_ {
		reqHeaders[k] = v
	...
	c.urlParams_.Set("alt", alt)
	c.urlParams_.Set("prettyPrint", "false")
	urls := googleapi.ResolveRelative(c.s.BasePath, "v1beta/{+parent}/sshPublicKey")
	urls += "?" + c.urlParams_.Encode()
	req, err := http.NewRequest("POST", urls, body)
	if err != nil {
		return nil, err
...
// Do executes the "oslogin.users.sshPublicKey.create" call.
...
// to check whether the returned error was because
// http.StatusNotModified was returned.
func (c *UsersSshPublicKeyCreateCall) Do(opts ...googleapi.CallOption) (*SshPublicKey, error) {
	gensupport.SetOptions(c.urlParams_, opts...)
	res, err := c.doRequest("json")
	if res != nil && res.StatusCode == http.StatusNotModified {
		if res.Body != nil {
