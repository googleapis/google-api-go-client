...
type CustomFieldDefServicer interface {
	List(teamId string) *CustomFieldDefListCall
}
...
type JobsServicer interface {
	Get(teamId string, jobId uint64) *JobsGetCall
	Insert(teamId string, address string, lat float64, lng float64, title string, job *Job) *JobsInsertCall
	List(teamId string) *JobsListCall
	Patch(teamId string, jobId uint64, job *Job) *JobsPatchCall
	Update(teamId string, jobId uint64, job *Job) *JobsUpdateCall
}
...
type LocationServicer interface {
	List(teamId string, workerEmail string, startTimestampMs uint64) *LocationListCall
}
...
type ScheduleServicer interface {
	Get(teamId string, jobId uint64) *ScheduleGetCall
	Patch(teamId string, jobId uint64, schedule *Schedule) *SchedulePatchCall
	Update(teamId string, jobId uint64, schedule *Schedule) *ScheduleUpdateCall
}
...
type WorkerServicer interface {
	List(teamId string) *WorkerListCall
}
