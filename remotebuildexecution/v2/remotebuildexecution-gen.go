...
// BuildBazelRemoteExecutionV2ActionCacheUpdateCapabilities: Describes the
// server/instance capabilities for updating the action cache.
type BuildBazelRemoteExecutionV2ActionCacheUpdateCapabilities struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "UpdateEnabled") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// BuildBazelRemoteExecutionV2BatchReadBlobsRequest: A request message for
...
type BuildBazelRemoteExecutionV2BatchReadBlobsRequest struct {
	...
	// ForceSendFields is a list of field names (e.g. "Digests") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Digests") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// BuildBazelRemoteExecutionV2BatchReadBlobsResponse: A response message for
// ContentAddressableStorage.BatchReadBlobs.
...
// BuildBazelRemoteExecutionV2BatchUpdateBlobsRequest: A request message for
...
// BuildBazelRemoteExecutionV2BatchUpdateBlobsResponse: A response message for
// ContentAddressableStorage.BatchUpdateBlobs.
...
// BuildBazelRemoteExecutionV2BatchUpdateBlobsResponseResponse: A response
...
// BuildBazelRemoteExecutionV2CacheCapabilities: Capabilities of the remote
// cache system.
type BuildBazelRemoteExecutionV2CacheCapabilities struct {
	// ActionCacheUpdateCapabilities: Capabilities for updating the action cache.
	...
	// CachePriorityCapabilities: Supported cache priority range for both CAS and
	// ActionCache.
	...
	// uploaded/downloaded using batch methods. A value of 0 means no limit is set,
	// although in practice there will always be a message size limitation of the
	...
	// bytestream resources. Servers MUST support identity/no-compression, even if
	// it is not listed here. Note that this does not imply which if any
	// compressors are supported by the server at the gRPC level.
	//
	// Possible values:
	//   "IDENTITY" - No compression. Servers and clients MUST always support this,
	// and do not need to advertise it.
	//   "ZSTD" - Zstandard compression.
	SupportedCompressor []string `json:"supportedCompressor,omitempty"`
	// SymlinkAbsolutePathStrategy: Whether absolute symlink targets are supported.
	//
	// Possible values:
	//   "UNKNOWN" - Invalid value.
	//   "DISALLOWED" - Server will return an `INVALID_ARGUMENT` on input symlinks
	// with absolute targets. If an action tries to create an output symlink with
	// an absolute target, a `FAILED_PRECONDITION` will be returned.
	...
	// "ActionCacheUpdateCapabilities") to unconditionally include in API requests.
	// By default, fields with empty or default values are omitted from API
	// requests. See https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields
	// for more details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ActionCacheUpdateCapabilities")
...
type BuildBazelRemoteExecutionV2ExecuteRequest struct {
	...
	ActionDigest *BuildBazelRemoteExecutionV2Digest `json:"actionDigest,omitempty"`
	// ExecutionPolicy: An optional policy for execution of the action. The server
	// will have a default policy if this is not provided.
	ExecutionPolicy *BuildBazelRemoteExecutionV2ExecutionPolicy `json:"executionPolicy,omitempty"`
	// ResultsCachePolicy: An optional policy for the results of this execution in
	// the remote cache. The server will have a default policy if this is not
	// provided. This may be applied to both the ActionResult and the associated
	// blobs.
	ResultsCachePolicy *BuildBazelRemoteExecutionV2ResultsCachePolicy `json:"resultsCachePolicy,omitempty"`
	// SkipCacheLookup: If true, the action will be executed even if its result is
	// already present in the ActionCache. The execution is still allowed to be
	// merged with other in-flight executions of the same action, however -
	...
	// cache upon completion, and services SHOULD overwrite any existing entries
	// that may exist. This allows skip_cache_lookup requests to be used as a
	// mechanism for replacing action cache entries that reference outputs no
	// longer available or that are poisoned in any way. If false, the result may
...
// BuildBazelRemoteExecutionV2ExecutionCapabilities: Capabilities of the remote
// execution system.
type BuildBazelRemoteExecutionV2ExecutionCapabilities struct {
	// DigestFunction: Remote execution may only support a single digest function.
	//
	// Possible values:
	//   "UNKNOWN" - It is an error for the server to return this value.
	//   "SHA256" - The SHA-256 digest function.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DigestFunction") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// BuildBazelRemoteExecutionV2ExecutionPolicy: An `ExecutionPolicy` can be used
// to control the scheduling of the action.
type BuildBazelRemoteExecutionV2ExecutionPolicy struct {
	// Priority: The priority (relative importance) of this action. Generally, a
	// lower value means that the action should be run sooner than actions having a
...
// BuildBazelRemoteExecutionV2FindMissingBlobsRequest: A request message for
...
type BuildBazelRemoteExecutionV2FindMissingBlobsRequest struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BlobDigests") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// BuildBazelRemoteExecutionV2FindMissingBlobsResponse: A response message for
// ContentAddressableStorage.FindMissingBlobs.
type BuildBazelRemoteExecutionV2FindMissingBlobsResponse struct {
	// MissingBlobDigests: A list of the blobs requested *not* present in the
	// storage.
	MissingBlobDigests []*BuildBazelRemoteExecutionV2Digest `json:"missingBlobDigests,omitempty"`

	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "MissingBlobDigests") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "MissingBlobDigests") to include
...
type BuildBazelRemoteExecutionV2GetTreeResponse struct {
	...
	// NextPageToken: If present, signifies that there are more results which the
	// client can retrieve by passing this as the page_token in a subsequent
	// request. If empty, signifies that this is the last page of results.
...
// BuildBazelRemoteExecutionV2PriorityCapabilities: Allowed values for priority
// in ResultsCachePolicy and ExecutionPolicy Used for querying both cache and
...
type BuildBazelRemoteExecutionV2PriorityCapabilities struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Priorities") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// BuildBazelRemoteExecutionV2PriorityCapabilitiesPriorityRange: Supported
// range of priorities, including boundaries.
type BuildBazelRemoteExecutionV2PriorityCapabilitiesPriorityRange struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "MaxPriority") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// BuildBazelRemoteExecutionV2ResultsCachePolicy: A `ResultsCachePolicy` is
// used for fine-grained control over how action outputs are stored in the CAS
// and Action Cache.
type BuildBazelRemoteExecutionV2ResultsCachePolicy struct {
	// Priority: The priority (relative importance) of this content in the overall
	// cache. Generally, a lower value means a longer retention time or other
	// advantage, but the interpretation of a given value is server-dependent. A
	// priority of 0 means a *default* value, decided by the server. The particular
	// semantics of this field is up to the server. In particular, every server
	// will have their own supported range of priorities, and will decide how these
	// map into retention/eviction policy.
	Priority int64 `json:"priority,omitempty"`
	// ForceSendFields is a list of field names (e.g. "Priority") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
...
type BuildBazelRemoteExecutionV2ServerCapabilities struct {
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "CacheCapabilities") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CacheCapabilities") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// BuildBazelRemoteExecutionV2WaitExecutionRequest: A request message for
// WaitExecution.
...
type BuildBazelSemverSemVer struct {
	...
	// Prerelease: The pre-release version. Either this field or major/minor/patch
	// fields must be filled. They are mutually exclusive. Pre-release versions are
...
// Get: Retrieve a cached execution result. Implementations SHOULD ensure that
// any blobs referenced from the ContentAddressableStorage are available at the
...
//   - hash: The hash. In the case of SHA-256, it will always be a lowercase hex
//     string exactly 64 characters long.
//   - instanceName: The instance of the execution system to operate against. A
//     server may support multiple instances of the execution system (with their
//     own workers, storage, caches, etc.). The server MAY require use of this
...
// InlineOutputFiles sets the optional parameter "inlineOutputFiles": A hint to
// the server to inline the contents of the listed output files. Each path
// needs to exactly match one file path in either `output_paths` or
...
// InlineStderr sets the optional parameter "inlineStderr": A hint to the
// server to request inlining stderr in the ActionResult message.
...
// InlineStdout sets the optional parameter "inlineStdout": A hint to the
// server to request inlining stdout in the ActionResult message.
...
// *BuildBazelRemoteExecutionV2ActionResult.ServerResponse.Header or (if a
...
// Update: Upload a new execution result. In order to allow the server to
// perform access control based on the type of action, and to assist with
// client debugging, the client MUST first upload the Action that produced the
// result, along with its Command, into the `ContentAddressableStorage`. Server
...
// "resultsCachePolicy.priority": The priority (relative importance) of this
...
// is server-dependent. A priority of 0 means a *default* value, decided by the
...
// Execute: Execute an action remotely. In order to execute an action, the
// client must first upload all of the inputs, the Command to run, and the
// Action into the ContentAddressableStorage. It then calls `Execute` with an
...
// non-canonical input is received. It MAY also proceed without verifying some
// or all of the requirements, such as for performance reasons. If the server
// does not verify the requirement, then it will treat the `Action` as distinct
// from another logically equivalent action if they hash differently. Returns a
// stream of google.longrunning.Operation messages describing the resulting
// execution, with eventual `response` ExecuteResponse. The `metadata` on the
// operation is of type ExecuteOperationMetadata. If the client remains
// connected after the first response is returned after the server, then
// updates are streamed as if the client had called WaitExecution until the
// execution completes or the request reaches an error. The operation can also
// be queried using Operations API. The server NEED NOT implement other methods
// or functionality of the Operations API. Errors discovered during creation of
// the `Operation` will be reported as gRPC Status errors, while errors that
// occurred while running the action will be reported in the `status` field of
// the `ExecuteResponse`. The server MUST NOT set the `error` field of the
// `Operation` proto. The possible errors include: * `INVALID_ARGUMENT`: One or
// more arguments are invalid. * `FAILED_PRECONDITION`: One or more errors
...
// support a queue), the action could not be started. The client should retry.
// * `INTERNAL`: An internal error occurred in the execution engine or the
// worker. * `DEADLINE_EXCEEDED`: The execution timed out. * `CANCELLED`: The
// operation was cancelled by the client. This status is only possible if the
// server implements the Operations API CancelOperation method, and it was
// called for the current execution. In the case of a missing input or command,
// the server SHOULD additionally send a PreconditionFailure error detail
...
// most one execution of the action. The server MAY execute the action multiple
// times, potentially in parallel. These redundant executions MAY continue to
// run, even if the operation is completed.
//
//   - instanceName: The instance of the execution system to operate against. A
//     server may support multiple instances of the execution system (with their
//     own workers, storage, caches, etc.). The server MAY require use of this
...
// BatchRead: Download many blobs at once. The server may enforce a limit of
// the combined total size of blobs to be downloaded using this API. This limit
...
// Bytestream `Read` request on each individual blob, in parallel. The requests
// may succeed or fail independently. Errors: * `INVALID_ARGUMENT`: The client
// attempted to read more than the server supported limit. Every error on
// individual read will be returned in the corresponding digest status.
//
//   - instanceName: The instance of the execution system to operate against. A
//     server may support multiple instances of the execution system (with their
//     own workers, storage, caches, etc.). The server MAY require use of this
...
// Any non-2xx status code is an error. Response headers are in either
// *BuildBazelRemoteExecutionV2BatchReadBlobsResponse.ServerResponse.Header or
...
// BatchUpdate: Upload many blobs at once. The server may enforce a limit of
// the combined total size of blobs to be uploaded using this API. This limit
// may be obtained using the Capabilities API. Requests exceeding the limit
// should either be split into smaller chunks or uploaded using the ByteStream
// API, as appropriate. This request is equivalent to calling a Bytestream
// `Write` request on each individual blob, in parallel. The requests may
// succeed or fail independently. Errors: * `INVALID_ARGUMENT`: The client
// attempted to upload more than the server supported limit. Individual
// requests may return the following errors, additionally: *
...
// Any non-2xx status code is an error. Response headers are in either
// *BuildBazelRemoteExecutionV2BatchUpdateBlobsResponse.ServerResponse.Header
...
// FindMissing: Determine if blobs are present in the CAS. Clients can use this
// API before uploading blobs to determine which ones are already present in
// the CAS and do not need to be uploaded again. Servers SHOULD increase the
// lifetimes of the referenced blobs if necessary and applicable. There are no
...
// Any non-2xx status code is an error. Response headers are in either
// *BuildBazelRemoteExecutionV2FindMissingBlobsResponse.ServerResponse.Header
// or (if a response was returned at all) in error.(*googleapi.Error).Header.
// Use googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// GetTree: Fetch the entire directory tree rooted at a node. This request must
// be targeted at a Directory stored in the ContentAddressableStorage (CAS).
// The server will enumerate the `Directory` tree recursively and return every
// node descended from the root. The GetTreeRequest.page_token parameter can be
// used to skip ahead in the stream (e.g. when retrying a partially completed
// and aborted request), by setting it to a value taken from
...
// PageSize sets the optional parameter "pageSize": A maximum page size to
// request. If present, the server will request no more than this many items.
// Regardless of whether a page size is specified, the server may place its own
// limit on the number of items to be returned and require the client to
// retrieve more items using a subsequent request.
func (c *BlobsGetTreeCall) PageSize(pageSize int64) *BlobsGetTreeCall {
	c.urlParams_.Set("pageSize", fmt.Sprint(pageSize))
	return c
}

// PageToken sets the optional parameter "pageToken": A page token, which must
// be a value received in a previous GetTreeResponse. If present, the server
// will use that token as an offset, returning only that page and the ones that
// succeed it.
...
// Any non-2xx status code is an error. Response headers are in either
// *BuildBazelRemoteExecutionV2GetTreeResponse.ServerResponse.Header or (if a
// response was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// WaitExecution: Wait for an execution operation to complete. When the client
...
// operation. The server MAY choose to stream additional updates as execution
...
// configuration of the remote endpoint. Only the capabilities of the services
// supported by the endpoint will be returned: * Execution + CAS + Action Cache
// endpoints should return both CacheCapabilities and ExecutionCapabilities. *
// Execution only endpoints should return ExecutionCapabilities. * CAS + Action
// Cache only endpoints should return CacheCapabilities.
//
//   - instanceName: The instance of the execution system to operate against. A
//     server may support multiple instances of the execution system (with their
//     own workers, storage, caches, etc.). The server MAY require use of this
...
// Any non-2xx status code is an error. Response headers are in either
// *BuildBazelRemoteExecutionV2ServerCapabilities.ServerResponse.Header or (if
