...
func (r *UrlService) Get(shortUrl string) *UrlGetCall {
	c := &UrlGetCall{s: r.s, urlParams_: urlValues()}
	c.shortUrl = shortUrl
	return c
}
...
func (r *UrlService) Insert(url *Url) *UrlInsertCall {
	c := &UrlInsertCall{s: r.s, urlParams_: urlValues()}
	c.url = url
	return c
}
...
func (r *UrlService) List() *UrlListCall {
	c := &UrlListCall{s: r.s, urlParams_: urlValues()}
	return c
}
...
func (c *UrlListCall) StartToken(startToken string) *UrlListCall {
	c.urlParams_.Set("startToken", fmt.Sprintf("%v", startToken))
	return c
}
