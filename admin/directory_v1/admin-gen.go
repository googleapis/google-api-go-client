...
type Alias struct {
	...
	// PrimaryEmail: User's primary email (Read-only)
...
type ChromeOsDevice struct {
	...
	// OrgUnitPath: OrgUnit of the device (Read-only)
...
type Group struct {
	// AdminCreated: Is the group created by admin *
...
type User struct {
	...
	// CreationTime: User's Google account creation time.
	...
	// LastLoginTime: User's last login time.
...
func (r *UsersService) Undelete(userKey string) *UsersUndeleteCall {
