...
const basePathTemplate = "https://gameservices.UNIVERSE_DOMAIN/"
...
// AuthorizationLoggingOptions: Authorization-related information used by Cloud
// Audit Logging.
...
type Condition struct {
	...
	// (go/security-realms). When used with IN, the condition indicates "any of the
	// request's realms match one of the given values; with NOT_IN, "none of the
	// realms match any of the given values". Note that a value can be: -
	...
	// 'self' [DEPRECATED] (i.e., allow connections from clients that are in the
	...
	// this request. When used with IN, the condition indicates that one of the
	// approvers associated with the request matches the specified principal, or is
	// a member of the specified group. Approvers can only grant additional access,
	// and are thus only used in a strictly positive context (e.g. ALLOW/IN or
	// DENY/NOT_IN).
	...
	// ALLOW/IN or DENY/NOT_IN). Multiple justifications, e.g., a Buganizer ID and
	// a manually-entered reason, are normal and supported.
	...
	// credentials type, so the conditions can only be used in a "positive" context
	...
	// Sys: Trusted attributes supplied by any service that owns resources and uses
...
// CounterOptions: Increment a streamz counter with the specified metric and
// field names. Metric names should start with a '/', generally be
// lowercase-only, and end in "_count". Field names should not contain an
// initial slash. The actual exported metric names will have "/iam/policy"
// prepended. Field names correspond to IAM request parameters and field values
// are their respective values. Supported field names: - "authority", which is
// "[token]" if IAMContext.token is present, otherwise the value of
...
// CustomField: Custom fields. These can be used to create a counter with
...
type DataAccessOptions struct {
	...
	// immediately after the authorization check. If client chooses to write one
	// and it fails, client may either fail open (allow the operation to continue)
	// or fail closed (handle as a DENY outcome).
	...
	// check time and only returning ALLOW to the application if it succeeds. If a
	// matching Rule has this directive, but the client has not indicated that it
	// will honor such requirements, then the IAM check will result in
...
type Rule struct {
	...
	Description string `json:"description,omitempty"`
	// In: If one or more 'in' clauses are specified, the rule matches if the
	...
	// the PRINCIPAL/AUTHORITY_SELECTOR is in none of the entries. The format for
	// in and not_in entries can be found at in the Local IAM documentation (see
	// go/local-iam#features).
	NotIn []string `json:"notIn,omitempty"`
	// Permissions: A permission is a string of form '..' (e.g.,
	// 'storage.buckets.list'). A value of '*' matches all permissions, and a verb
	// part of '*' (e.g., 'storage.buckets.*') matches all verbs.
