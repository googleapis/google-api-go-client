...
// Example demonstrates loading objects from Google Cloud Storage into
...
func Example() {
	...
	if err != nil {
		log.Fatalf("Unable to get bigquery scope: %v", err)
	}

	var (
		project    = flag.Arg(0)
		bucket     = flag.Arg(1)
		objPrefix  = flag.Arg(2)
		datasetId  = flag.Arg(3)
		schemaFile = flag.Arg(4)
	)
	badRecords, err := strconv.ParseInt(flag.Arg(5), 10, 64)
	if err != nil {
