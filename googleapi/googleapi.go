...
// UploadParameters is used to optionally configure resumable upload.
// It is used in ResumableMedia method available on certain objects types
// in Google Cloud Storage and Google Drive APIs.
type UploadParameters struct {
	...
	// io.ReaderAt interface). When setting chunk size, note the memory implications:
	// one full chunk will be buffered in memory.
	ChunkSize int64
...
//  - Media is set to an appropriate io.ReaderAt, which is possibly FakeReaderAt.
//  - FakeRdr is set depending on whether Media is FakeReaderAt.
...
//  - whether reader is also an UploadParameters struct
// The ultimate goal of this configuration is to ensure that resumable upload will be done
// in the most efficient manner whenever possible, while falling back the to less efficient
// method when required (with buffering content and chunking). For example, the most
// common case of user passing os.File as reader is handled efficiently - buffering
...
func (rx *ResumableUpload) transferChunks(ctx context.Context) (*http.Response, error) {
	...
	for {
		...
		var reqSize int64
		if rx.FakeRdr && rx.ContentLength <= 0 { // we have FakeReaderAt
