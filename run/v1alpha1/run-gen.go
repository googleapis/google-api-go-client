...
type Container struct {
	// Args: (Optional) Arguments to the entrypoint. The docker image's CMD is used
	...
	// EnvFrom: (Optional) List of sources to populate environment variables in the
	...
	Name string `json:"name,omitempty"`
	// Ports: (Optional) List of ports to expose from the container. Only a single
	// port can be specified. The specified ports must be listening on all
	// interfaces (0.0.0.0) within the container to be accessible. If omitted, a
	// port number will be chosen and passed to the container through the PORT
	...
	// should be populated. File will use the contents of terminationMessagePath to
	...
	// Only supports SecretVolumeSources. Pod volumes to mount into the container's
	// filesystem.
	VolumeMounts []*VolumeMount `json:"volumeMounts,omitempty"`
	// WorkingDir: (Optional) Container's working directory. If not specified, the
	// container runtime's default will be used, which might be configured in the
	// container image.
...
type ContainerPort struct {
	// ContainerPort: (Optional) Port number the container listens on. This must be
...
type EnvVar struct {
	...
	// supports secret_key_ref. Source for the environment variable's value. Cannot
...
type GRPCAction struct {
	...
	// https://github.com/grpc/grpc/blob/master/doc/health-checking.md). If this is
...
// HTTPGetAction: Not supported by Cloud Run HTTPGetAction describes an action
...
// HTTPHeader: Not supported by Cloud Run HTTPHeader describes a custom header
...
type InstanceSpec struct {
	...
	// may be active relative to StartTime before the system will actively try to
	// mark it failed and kill associated containers. If set to zero, the system
	// will never attempt to kill an instance based on time. Otherwise, value must
	...
	// disallow a number of fields on this Container. Only a single container may
	// be provided.
	...
	// Instances are never restarted and all failures are permanent. Cannot be used
	// if backoffLimit is set. +optional
	RestartPolicy string `json:"restartPolicy,omitempty"`
	// ServiceAccountName: Optional. Email address of the IAM service account
	// associated with the instance of a Job. The service account represents the
	// identity of the running instance, and determines what permissions the
	// instance has. If not provided, the instance will use the project's default
	// service account. +optional
	ServiceAccountName string `json:"serviceAccountName,omitempty"`
	// TerminationGracePeriodSeconds: Optional. Optional duration in seconds the
	// instance needs to terminate gracefully. Value must be non-negative integer.
	// The value zero indicates delete immediately. The grace period is the
	// duration in seconds after the processes running in the instance are sent a
	// termination signal and the time when the processes are forcibly halted with
	// a kill signal. Set this value longer than the expected cleanup time for your
	// process. +optional
	...
	// ForceSendFields is a list of field names (e.g. "ActiveDeadlineSeconds") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
type InstanceStatus struct {
	...
	// Index: Required. Index of the instance, unique per Job, and beginning at 0.
	...
	// Instances are restarted according the restartPolicy configured in the Job
	// template. +optional
...
// InstanceTemplateSpec: InstanceTemplateSpec describes the data an instance
// should have when created from a template.
type InstanceTemplateSpec struct {
	// Spec: Optional. Specification of the desired behavior of the instance. More
	// info:
...
// Job: Job represents the configuration of a single job. A job an immutable
...
type JobCondition struct {
	...
	// Type: Required. Type is used to communicate the status of the reconciliation
	// process. See also:
	// https://github.com/knative/serving/blob/main/docs/spec/errors.md#error-conditions-and-reporting
	// Types include: * "Completed": True when the Job has successfully completed.
...
type JobSpec struct {
	...
	// system tries to terminate it. If set to zero, the system will never attempt
	// to terminate the job based on time. Otherwise, the value must be positive
	// integer. +optional
	ActiveDeadlineSeconds int64 `json:"activeDeadlineSeconds,omitempty,string"`
	// BackoffLimit: Optional. Specifies the number of retries per instance, before
	// marking this job failed. If set to zero, instances will never retry on
	// failure. +optional
	BackoffLimit int64 `json:"backoffLimit,omitempty"`
	// Completions: Optional. Specifies the desired number of successfully finished
	// instances the job should be run with. Setting to 1 means that parallelism is
	// limited to 1 and the success of that instance signals the success of the
	// job. More info:
	// https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/
	// +optional
	Completions int64 `json:"completions,omitempty"`
	// Parallelism: Optional. Specifies the maximum desired number of instances the
	// job should run at any given time. Must be <= completions. The actual number
	// of instances running in steady state will be less than this number when
	...
	// finishes, it is eligible to be automatically deleted. When the Job is being
	// deleted, its lifecycle guarantees (e.g. finalizers) will be honored. If this
	// field is set to zero, the Job won't be automatically deleted. +optional
...
type JobStatus struct {
	...
	// CompletionTime: Optional. Represents time when the job was completed. It is
	// not guaranteed to be set in happens-before order across separate operations.
	// It is represented in RFC3339 form and is in UTC. +optional
	CompletionTime string `json:"completionTime,omitempty"`
	...
	// specified within .Spec.Template.Spec.Container.Image. The digest is resolved
	// during the creation of the Job. This field holds the digest value regardless
	...
	// +optional
	StartTime string `json:"startTime,omitempty"`
	// Succeeded: Optional. The number of instances which reached phase Succeeded.
	// +optional
	Succeeded int64 `json:"succeeded,omitempty"`
...
type ListJobsResponse struct {
	...
	// another request to the endpoint that served this list to retrieve the next
	// set of available objects. Continuing a list may not be possible if the
	// server configuration has changed or more than a few minutes have passed. The
	// metadata.resourceVersion field returned when using this field will be
...
type ObjectMeta struct {
	...
	// which the object belongs to. This is used to distinguish resources with same
	// name and namespace in different clusters. This field is not set anywhere
	...
	// DeletionGracePeriodSeconds: (Optional) Not supported by Cloud Run Number of
	// seconds allowed for this object to gracefully terminate before it will be
	// removed from the system. Only set when deletionTimestamp is also set. May
	// only be shortened. Read-only.
	DeletionGracePeriodSeconds int64 `json:"deletionGracePeriodSeconds,omitempty"`
	// DeletionTimestamp: (Optional) Not supported by Cloud Run DeletionTimestamp
	// is RFC 3339 date and time at which this resource will be deleted. This field
	// is set by the server when a graceful deletion is requested by the user, and
	// is not directly settable by a client. The resource is expected to be deleted
	...
	// deletionTimestamp is set, this value may not be unset or be set further into
	// the future, although it may be shortened or the resource may be deleted
	// prior to this time. For example, a user may request that a pod is deleted in
	// 30 seconds. The Kubelet will react by sending a graceful termination signal
	// to the containers in the pod. After that 30 seconds, the Kubelet will send a
	// hard termination signal (SIGKILL) to the container and after cleanup, remove
	// the pod from the API. In the presence of network partitions, this object may
	// still exist after this timestamp, until an administrator or automated
	// process can determine the resource is fully terminated. If not set, graceful
	// deletion of the object has not been requested. Populated by the system when
	// a graceful deletion is requested. Read-only. More info:
	// https://git.k8s.io/community/contributors/devel/api-conventions.md#metadata
	...
	// deletionTimestamp of the object is non-nil, entries in this list can only be
	// removed. +patchStrategy=merge
	Finalizers []string `json:"finalizers,omitempty"`
	// GenerateName: (Optional) Not supported by Cloud Run GenerateName is an
	// optional prefix, used by the server, to generate a unique name ONLY IF the
	// Name field has not been provided. If this field is used, the name returned
	// to the client will be different than the name passed. This value will also
	// be combined with a unique suffix. The provided value has the same validation
	// rules as the Name field, and may be truncated by the length of the suffix
	// required to make the value unique on the server. If this field is specified
	// and the generated name exists, the server will NOT return a 409 - instead,
	...
	// client should retry (optionally after the time indicated in the Retry-After
	...
	// OwnerReferences: (Optional) Not supported by Cloud Run List of objects that
	// own this object. If ALL objects in the list have been deleted, this object
	...
	// value to disable conflict-detection. They may only be valid for a particular
	...
	// Read-only. More info: https://kubernetes.io/docs/user-guide/identifiers#uids
...
// Probe: Not supported by Cloud Run Probe describes a health check to be
...
type Probe struct {
	...
	// following should be specified. Exec specifies the action to take. A field
	// inlined from the Handler message.
	Exec *ExecAction `json:"exec,omitempty"`
	// FailureThreshold: (Optional) Minimum consecutive failures for the probe to
	// be considered failed after having succeeded. Defaults to 3. Minimum value is
	// 1.
	...
	// be considered successful after having failed. Must be 1 if set.
	...
	// out. Defaults to 1 second. Minimum value is 1. Maximum value is 3600. Must
...
type ResourceRequirements struct {
	...
	// maximum amount of compute resources allowed. The values of the map is string
	...
	// the minimum amount of compute resources required. If Requests is omitted for
...
// SecretEnvSource: Not supported by Cloud Run SecretEnvSource selects a Secret
// to populate the environment variables with. The contents of the target
// Secret's Data field will represent the key-value pairs as environment
// variables.
...
// SecretVolumeSource: The secret's value will be presented as the content of a
// file whose name is defined in the item path. If no items are defined, the
// name of the file is the secret_name. The contents of the target Secret's
...
type SecretVolumeSource struct {
	...
	// fetch from Cloud Secret Manager and the path will be the name of the file
	// exposed in the volume. When items are defined, they must specify a key and a
	// path. If unspecified, each key-value pair in the Data field of the
...
// TCPSocketAction: Not supported by Cloud Run TCPSocketAction describes an
// action based on opening a socket
type TCPSocketAction struct {
	// Host: (Optional) Optional: Host name to connect to, defaults to the pod IP.
	Host string `json:"host,omitempty"`
	// Port: Number or name of the port to access on the container. Number must be
	// in the range 1 to 65535. Name must be an IANA_SVC_NAME. This field is
...
type VolumeMount struct {
	...
	// Name: The name of the volume. There must be a corresponding Volume with the
	// same name.
	Name string `json:"name,omitempty"`
...
//
//   - name: The name of the job to retrieve. For Cloud Run (fully managed),
//     replace {namespace_id} with the project ID or number.
