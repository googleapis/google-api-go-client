...
const basePathTemplate = "https://sts.UNIVERSE_DOMAIN/"
...
// bound of what a principal may access. It includes a list of access boundary
// rules that each defines the resource that may be allowed as well as
// permissions that may be used on those resources.
...
// upper bound of IAM permissions on a single resource.
...
// Security Token Service supports, but that are not supported by standard
// OAuth 2.0 token exchange endpoints, as defined in
...
// an upper bound of IAM permissions on a single resource.
...
type GoogleIdentityStsV1betaExchangeTokenRequest struct {
	// Audience: The full resource name of the identity provider. For example,
	// `//iam.googleapis.com/projects//locations/global/workloadIdentityPools//provi
	// ders/`. Required when exchanging an external credential for a Google access
	// token.
	...
	// `urn:ietf:params:oauth:token-type:access_token`, which indicates an OAuth
	// 2.0 access token.
	...
	// access token issued by Google. If the token is an OIDC JWT, it must use the
	// JWT format defined in RFC 7523 (https://tools.ietf.org/html/rfc7523), and
	// the `subject_token_type` must be either
	// `urn:ietf:params:oauth:token-type:jwt` or
	// `urn:ietf:params:oauth:token-type:id_token`. The following headers are
	...
	// `iss`: The issuer of the token. The issuer must provide a discovery document
	// at the URL `/.well-known/openid-configuration`, where `` is the value of
	...
	// "exp": 1517966704, "aud":
	// "//iam.googleapis.com/projects/1234567890123/locations/global/workloadIdentit
	...
	// the same information as a request to the AWS `GetCallerIdentity()`
	...
	// send the request, formatted as an ISO8601 Basic
	...
	// For example:
	// //iam.googleapis.com/projects//locations/global/workloadIdentityPools//provid
	// ers/
	...
	// "Authorization", "value":
	// "AWS4-HMAC-SHA256+Credential=$credential,+SignedHeaders=host;x-amz-date;x-goo
	// g-cloud-target-resource,+Signature=$signature"}, {"key":
	// "x-goog-cloud-target-resource", "value":
	// "//iam.googleapis.com/projects//locations/global/workloadIdentityPools//provi
	// ders/"}, {"key": "host", "value": "sts.amazonaws.com"} . ], "method":
	// "POST", "url":
	// "https://sts.amazonaws.com?Action=GetCallerIdentity&Version=2011-06-15" }
...
type GoogleIdentityStsV1betaExchangeTokenResponse struct {
	...
	// reserves the right to change the token size and the maximum length at any
	// time.
...
// the Security Token Service supports, but that are not supported by standard
// OAuth 2.0 token exchange endpoints, as defined in
// https://tools.ietf.org/html/rfc8693.
...
// Token: Exchanges a credential for a Google OAuth 2.0 access token. The token
...
// *GoogleIdentityStsV1betaExchangeTokenResponse.ServerResponse.Header or (if a
