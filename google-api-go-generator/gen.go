...
func (meth *Method) generateCode() {
	...
	if meth.supportsMediaUpload() {
		...
		pn("// At most one of Media and ResumableMedia may be set.")
		pn("// The media to upload can be specified via any simple io.Reader (including os.File),")
		pn("// or, as a special case for extra configuration, a googleapi.UploadParameters struct")
		pn("// value or struct pointer can be used. Refer to the struct's documentation for additional details.")
		pn("func (c *%s) ResumableMedia(ctx context.Context, r io.Reader) *%s {", callName, callName)
		pn("c.ctx_ = ctx")
		pn("c.media_ = r")
	...
	if meth.supportsMediaUpload() {
		pn("var rx *googleapi.ResumableUpload")
		pn(`if c.protocol_ == "resumable" {`)
		...
		pn(" }")
		pn(" rx.Configure(c.media_)")
		pn(` req.Header.Set("X-Upload-Content-Type", rx.MediaType)`)
	...
	if meth.supportsMediaUpload() {
		pn(`if c.protocol_ == "resumable" {`)
		pn(` rx.URI = res.Header.Get("Location")`)
